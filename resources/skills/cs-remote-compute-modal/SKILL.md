---
name: cs-remote-compute-modal
description: "Run GPU jobs on the user's own Modal account via host.compute.create('byoc:modal', ...). Covers the create→submit→wait_for_notification flow, the compute_provider kernel for env setup, image/volume resolution, and the two approval cards. Load once you've decided to dispatch to Modal. OpenScience: record concrete outputs through science_artifact provenance."
---

<!-- Generated by OpenScience science skill materializer; source=resources/skills/vendor/academicforge/skills/claude-science/remote-compute-modal -->

# OpenScience Adapter

This is a first-class OpenScience materialization of the upstream AcademicForge Claude Science skill `remote-compute-modal`. Use it as domain workflow knowledge inside Science Mode.
...
- Source path: `resources/skills/vendor/academicforge/skills/claude-science/remote-compute-modal`
- Original description: Run GPU jobs on the user's own Modal account via host.compute.create('byoc:modal', ...). Covers the create→submit→wait_for_notification flow, the compute_provider kernel for env setup, image/volume resolution, and the two approval cards. Load once you've decided to dispatch to Modal.
...
```python
# repl tool
print(host.skills.read('cs-remote-compute-modal', 'envs/proteomics_jax_gpu.py')['content'])
```
