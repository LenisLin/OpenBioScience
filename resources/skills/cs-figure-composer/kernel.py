...
def panel_task(outline, letter, fig_label="Figure", rules_ref="(load `cs-figure-style`)"):
    p = next(q for q in outline["panels"] if q["letter"]==letter)
...
- Environment `figures`, Python/matplotlib. Load `cs-figure-style`, call `apply_figure_style()`,
...
def compose_crops(outline, dpi=300, gutter_mm=4, pad_px=4):
    ...
    ``cs-figure-style.panel_crops`` for the PIL-composed case where no live
...
**Design rules:** {rules_path or '(load `cs-figure-style`)'}{prev_line}
