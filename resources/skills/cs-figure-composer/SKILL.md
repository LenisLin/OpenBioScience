---
name: cs-figure-composer
description: "Compose one publication-grade multi-panel figure. Entry from a one-line claim + data files, OR from an existing figure via `derive_outline_prompt` (you read the PNG). Runs a per-figure loop: outline (12-col grid, per-panel ask + label_budget) → render each panel with `panel_task` (loading `cs-figure-style`), one at a time or parallelized → tile + stamp letters with `compose_figure` → adversarial composite self-review with two-tier feedback (Tier-1 outline_revisions / Tier-2 per-panel violations) → regen affected panels, ≤3 rounds. Helpers: panel_task / compose_figure / compose_crops / composite_review_task / derive_outline_prompt. For one standalone plot use `cs-figure-style`; for whole-paper figure ordering use `cs-paper-narrative`. OpenScience: record concrete outputs through science_artifact provenance."
---

<!-- Generated by OpenScience science skill materializer; source=resources/skills/vendor/academicforge/skills/claude-science/figure-composer -->

# OpenScience Adapter

This is a first-class OpenScience materialization of the upstream AcademicForge Claude Science skill `figure-composer`. Use it as domain workflow knowledge inside Science Mode.
...
**Step 0.** Load `cs-figure-style` alongside this skill — that is the
...
sub-agent (see §2), the maker loads `cs-figure-style` first.
...
`cs-figure-composer` is the **outer tier**: make ONE multi-panel figure good. The
**inner tier** is `cs-figure-style` (every panel maker loads it — and load it
yourself, since you write the outline and, on a single-agent platform, render
the panels too). The **outermost tier** is `cs-paper-narrative` — if this figure is
...
Outline rules (cs-figure-style §7.1):
...
load `cs-figure-style`, call `apply_figure_style()`, render at exactly w×h px with
...
render against `cs-figure-style`, each writing its own PNG:
...
L)` prompt, loads `cs-figure-style` itself, and writes `panel_<letter>.png` — then
...
Run the `cs-figure-style` §9.2 perceptual checklist on each crop (contrast,
