...
| `bio-environment-manager`         | 官方环境不满足需求时，指导 agent 创建/派生/注册 user conda env，并通过 `bio_runtime` 暴露 `environmentRef`              |
| `bio-analysis-script-authoring`   | R/Python 脚本写作规范；contract header、固定输出目录、session/package info、禁止脚本内安装或隐式下载                   |
...
- 新增 `bio-environment-manager` skill，并规划 environment registration MCP；真实 conda/mamba/pip/R/GitHub 安装由 agent 在 MCP 外层执行。
- 用户环境注册后由 `bio_runtime` list/resolve/probe 暴露为 `environmentRef`。
- 官方环境 immutable；包来源不做固定白名单，但必须记录来源、版本、commit/tag 和风险。
- 用户环境第一版只给 agent/runtime 可见，不做前台 UI 管理。
- 新增 `bio-analysis-script-authoring` skill only；不做 `bio_script` MCP、runner、scheduler 或复杂执行状态机。
...
- 定义 environment registration MCP 的 register/index schema，并把 user env 接入 `bio_runtime` list/resolve/probe。
