...
对于完整文章复现，scRNA-seq 链路不再作为所有 omics 文献复现的最上游入口。上游应先走 `bio-omics-reproduction-planning`，完成 paper/data/code/method/environment/task 的规划后，再按 modality 路由到本文定义的 scRNA-seq skills。对应规划文档：
...
| --- | --- | --- | --- | --- |
| `bio-scrna-reproduction` | modality router | scRNA-seq 文章/demo 复现下游入口；在 `bio-omics-reproduction-planning` 已完成上游规划后，连接 single-cell data、environment、analysis、artifact；判断当前任务进入哪条子流程 | `research_evidence`、`science_artifact`、`bio_runtime`、`bio_source` | none |
