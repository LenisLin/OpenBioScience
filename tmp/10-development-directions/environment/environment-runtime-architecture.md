...
- 官方环境统一安装在 `<OPENBIOSCIENCE_RUNTIME_ROOT>/envs`。
...
- `sc-py-singlecell` 已完成 CPU-friendly Scanpy/AnnData/LIANA/CellPhoneDB/GEOparse/pysradb 导入验证；CUDA/PyTorch/scVI 不再作为 P0 默认依赖，后续应拆到独立 GPU/scVI 扩展环境。
