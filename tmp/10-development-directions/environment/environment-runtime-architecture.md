...
| 用户自定义环境 | advanced 用户 / agent runtime | User 级 conda env | 隔离、agent 外层创建、MCP 注册、probe 通过后可用 |
...
- 真实 conda/mamba/pip/R/GitHub 安装由 agent 在 MCP 外层执行。
- MCP 只负责注册、index 和返回结构化元数据。
- `bio_runtime` 负责 list/resolve/probe 注册后的 `environmentRef`。
- 用户环境仅对 agent/runtime 可见，不做前台 UI 管理。
- 不新增 orchestrator 或复杂环境状态机。
...
- 每个环境的标准 probe 合约。
- 用户环境注册/index 与 `bio_runtime` list/resolve/probe。
...
3. 新增 user environment registration MCP，并让 `bio_runtime` 查询、解析和 probe user env。
4. 将 bio tools、workflow、未来 job runner 改为通过 `environmentRef` 获取运行环境。
5. 暂不增加环境状态 UI；首版状态通过 agent/runtime 日志和 `bio_runtime` 暴露。
