...
const APPEARANCE_CACHE_KEY = '__deeporganiser_theme';
...
const useTheme = (): [Theme | null, (activeId: string) => Promise<void>, string | null] => {
  ...
  const select = useCallback(async (selectedId: string) => {
    await setActiveTheme(selectedId);
    setActive(resolveActiveTheme(selectedId, BUILTIN_THEMES, getSystemPrefersDark()));
