...
interface UseResizableSplitOptions {
  ...
  unit?: 'ratio' | 'px';
  /** Called when the pointer is released after dragging past maxWidth by this distance. */
  onDragPastMax?: () => void;
  /** Extra drag distance needed to trigger onDragPastMax. Defaults to 48px. */
  dragPastMaxThresholdPx?: number;
  /** Called when the pointer is released after dragging past minWidth by this distance. */
  onDragPastMin?: () => void;
  /** Extra drag distance needed to trigger onDragPastMin. Defaults to 48px. */
  dragPastMinThresholdPx?: number;
}
  ...
  const handleDragStart = useCallback(
    (reverse = false) =>
      (event: React.PointerEvent<HTMLDivElement>) => {
        ...
        const dragPastMaxThreshold = Math.max(1, dragPastMaxThresholdPx);
        const dragPastMinThreshold = Math.max(1, dragPastMinThresholdPx);
        let hasDraggedPastMax = false;
        let hasDraggedPastMin = false;
        ...
        const updateDragPastMax = (clientX: number) => {
          if (!onDragPastMax) {
            return;
          }
          const deltaX = reverse ? startX - clientX : clientX - startX;
          const maxDeltaPx = isPx ? maxWidth - startRatio : ((maxWidth - startRatio) / 100) * containerWidth;
          hasDraggedPastMax = deltaX > maxDeltaPx + dragPastMaxThreshold;
        };
        const updateDragPastMin = (clientX: number) => {
          if (!onDragPastMin) {
            return;
          }
          const deltaX = reverse ? startX - clientX : clientX - startX;
          const minDeltaPx = isPx ? minWidth - startRatio : ((minWidth - startRatio) / 100) * containerWidth;
          hasDraggedPastMin = deltaX < minDeltaPx - dragPastMinThreshold;
        };
        ...
        const finishDrag = (e?: PointerEvent | MouseEvent | FocusEvent) => {
          ...
          if (e && 'clientX' in e && typeof e.clientX === 'number') {
            updateDragPastMax(e.clientX);
            updateDragPastMin(e.clientX);
            finalRatio = computeRatio(e.clientX);
            latestRatio = finalRatio;
          }

          cleanupListeners?.();
          const dragPastCallback = hasDraggedPastMin ? onDragPastMin : hasDraggedPastMax ? onDragPastMax : undefined;
          if (dragPastCallback) {
            setSplitRatioState(startRatio);
            dispatchSplitResizeEvent(startRatio);
            dragPastCallback();
            return;
          }
    ...
    [
      ...
      isPx,
      onDragPastMax,
      dragPastMaxThresholdPx,
      onDragPastMin,
      dragPastMinThresholdPx,
    ]
  );
