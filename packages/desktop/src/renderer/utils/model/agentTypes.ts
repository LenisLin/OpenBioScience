...
import { isLegacyLocalRuntimeAgent } from '@/common/config/legacyIdentifiers';
...
/** Type of an agent. */
export type AgentType = 'acp' | 'remote' | 'openclaw-gateway' | 'nanobot';
...
export type AgentMetadata = {
  ...
  backend?: string;
  /** Top-level runtime discriminant: "acp" | "remote" | "nanobot" | … */
  ...
  /** Native mode id that DeepOrganiser's legacy `yolo` / `yoloNoSandbox`
...
function filterRuntimeAgents(agents: AgentMetadata[]): AgentMetadata[] {
...
export async function fetchDetectedAgents(): Promise<AgentMetadata[]> {
  try {
    ...
    if (Array.isArray(agents)) {
      return filterRuntimeAgents(agents as AgentMetadata[]);
    }
  } catch {
    // fallback to empty
  }
