...
import {
  ...
  findMentionAtCaret,
  type MentionRange,
} from '@/renderer/pages/guid/utils/larkProjectMention';
...
type GuidLarkProjectPanelProps = {
  ...
  onDraftDescriptionReady?: (description: string) => void;
};
...
const GuidLarkProjectPanel: React.FC<GuidLarkProjectPanelProps> = ({
  ...
  const insertMention = useCallback(
    (contact: ILarkAutomationContact) => {
      ...
      const insertion = buildMentionInsertion(currentText, contact.name, mentionRange, fallbackEnd);
      const nextContacts = contacts.some((item) => item.openId === contact.openId) ? contacts : [...contacts, contact];
      editor.innerHTML = renderDescriptionHtml(
        insertion.text,
        nextContacts.filter((item) => selectedOpenIds.includes(item.openId) || item.openId === contact.openId)
      );
      setMentionQuery(null);
      setMentionRange(null);
      setMentionActiveIndex(0);
      setCaretTextOffset(editor, insertion.caretOffset);
    },
  ...
  const handleCreateTasklist = useCallback(async () => {
    ...
    try {
      ...
      if (result.tasklistGuid) {
        const projectDescription = editorRef.current?.innerText?.trim() || '';
        ...
        if (projectDescription) {
          onDraftDescriptionReady?.(projectDescription);
        }
      }
  ...
  }, [loadProjects, onContextChange, onDraftDescriptionReady, resetModal, selectedContacts, tasklistName, t]);
