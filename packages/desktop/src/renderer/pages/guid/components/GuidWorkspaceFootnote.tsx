...
const GuidWorkspaceFootnote: React.FC<GuidWorkspaceFootnoteProps> = ({
  ...
  const handleBrowseWorkspace = useCallback(() => {
    ...
    ipcBridge.dialog.showOpen
      ...
      .catch((error) => {
        ...
        if (!workspaceDir) onDeactivate?.();
      });
  }, [onActivate, onDeactivate, onSelectWorkspace, workspaceDir]);
  ...
  }, [active, closeDropdown, onActivate, onDeactivate, open, openDropdown, workspaceDir]);
      ...
      {workspaceDir ? (
        <>
          <Tooltip content={workspaceDir} position='top'>
            <div className={`${styles.workspacePill} ${active ? styles.workspacePillActive : ''}`}>
              <button
        ...
        <>
          <button
            ref={triggerRef as React.RefObject<HTMLButtonElement>}
            className={`${styles.workspaceEmptyBtn} ${active ? styles.workspaceEmptyBtnActive : ''}`}
