...
const PREFERRED_HOME_AGENT_BACKENDS = new Set(['claude', 'codex', 'opencode']);
const DEFAULT_HOME_AGENT_BACKEND = 'codex';

function isPreferredHomeAgent(agent: AvailableAgent): boolean {
  if (agent.enabled === false || agent.available === false) return false;
  return PREFERRED_HOME_AGENT_BACKENDS.has(agent.backend || agent.agent_type);
}

function findDefaultHomeAgent(agents: AvailableAgent[] | undefined): AvailableAgent | undefined {
  return agents?.find((agent) => !agent.is_preset && isPreferredHomeAgent(agent));
}
...
/**
 * Hook that manages agent selection and availability.
 */
export const useGuidAgentSelection = ({
  ...
  const [selectedAgentKey, _setSelectedAgentKey] = useState<string>(() => {
    try {
      const saved = configService.get('guid.lastSelectedAgent');
      return saved && saved !== LEGACY_LOCAL_RUNTIME_ID ? saved : DEFAULT_HOME_AGENT_BACKEND;
    } catch {
      return DEFAULT_HOME_AGENT_BACKEND;
    }
  });
  ...
  const selectedAgent: string = ((): string => {
    if (selectedAgentKey.startsWith('custom:')) {
      const firstCliAgent = findDefaultHomeAgent(availableAgents) || availableAgents?.find((a) => !a.is_preset);
      return firstCliAgent ? getAgentKey(firstCliAgent) : DEFAULT_HOME_AGENT_BACKEND;
    }
  ...
  useEffect(() => {
    ...
    // exposed as `avatar` so compact selectors render the glyph directly
    // instead of mistaking it for a logo URL.
    const normalisedDetected: AvailableAgent[] = availableAgentsData
      .filter((agent) => isSupportedNewConversationAgent(agent) && !agent.is_preset)
      .map((a) => {
  ...
  useLayoutEffect(() => {
    ...
    if (resetAssistant) {
      ...
      if (currentIsPreset) {
        const firstCliAgent = findDefaultHomeAgent(availableAgents) || availableAgents.find((a) => !a.is_preset);
        const fallbackKey = firstCliAgent ? getAgentKey(firstCliAgent) : DEFAULT_HOME_AGENT_BACKEND;
      ...
      try {
        ...
        if (savedKey) {
          if (savedKey.startsWith('custom:')) {
            const firstAgent = findDefaultHomeAgent(availableAgents) || availableAgents[0];
            const fallbackKey = firstAgent ? getAgentKey(firstAgent) : DEFAULT_HOME_AGENT_BACKEND;
            _setSelectedAgentKey(fallbackKey);
            configService.set('guid.lastSelectedAgent', fallbackKey).catch((error) => {
