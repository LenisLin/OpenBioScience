import React, { useEffect, useMemo, useRef, useState } from 'react';
import { useTranslation } from 'react-i18next';

type ResearchHubStatus = 'operational' | 'syncing' | 'review';

type ResearchHub = {
  id: string;
  lat: number;
  lon: number;
  metricEn: string;
  metricZh: string;
  nameEn: string;
  nameZh: string;
  regionEn: string;
  regionZh: string;
  status: ResearchHubStatus;
};

type ProjectedResearchHub = ResearchHub & {
  label: string;
  metric: string;
  projX: number;
  projY: number;
  rawX: number;
  rawY: number;
  rawZ: number;
  region: string;
  z: number;
};

type GlobePoint = {
  opacity: number;
  origX: number;
  origY: number;
  origZ: number;
};

type LoginResearchGlobeProps = {
  onSelectHub: (hubId: string) => void;
  selectedHubId: string;
};

const HUBS: ResearchHub[] = [
  {
    id: 'westlake',
    nameZh: '西湖大学',
    nameEn: 'Westlake University',
    lat: 30.25,
    lon: 120.16,
    regionZh: '杭州 · 研究协调中心',
    regionEn: 'Hangzhou · Research coordination center',
    status: 'operational',
    metricZh: '课题 92%',
    metricEn: 'Programs 92%',
  },
  {
    id: 'beijing',
    nameZh: '清华大学实验室',
    nameEn: 'Tsinghua University Lab',
    lat: 39.9,
    lon: 116.4,
    regionZh: '北京 · 交叉学科实验室',
    regionEn: 'Beijing · Interdisciplinary lab',
    status: 'review',
    metricZh: '评审 7 项',
    metricEn: '7 reviews',
  },
  {
    id: 'shanghai',
    nameZh: '上海科研数据中心',
    nameEn: 'Shanghai Research Data Center',
    lat: 31.23,
    lon: 121.47,
    regionZh: '上海 · 数据与文献中心',
    regionEn: 'Shanghai · Data and literature center',
    status: 'operational',
    metricZh: '文献 1.8K',
    metricEn: '1.8K papers',
  },
  {
    id: 'singapore',
    nameZh: '新加坡国立大学',
    nameEn: 'National University of Singapore',
    lat: 1.35,
    lon: 103.82,
    regionZh: '新加坡 · 亚洲研究中心',
    regionEn: 'Singapore · Asia research center',
    status: 'syncing',
    metricZh: '合作中',
    metricEn: 'Collaborating',
  },
  {
    id: 'boston',
    nameZh: 'MIT 研究实验室',
    nameEn: 'MIT Research Laboratory',
    lat: 42.36,
    lon: -71.06,
    regionZh: '波士顿 · 计算科学实验室',
    regionEn: 'Boston · Computational science lab',
    status: 'operational',
    metricZh: '实验 24 项',
    metricEn: '24 experiments',
  },
  {
    id: 'london',
    nameZh: '伦敦大学学院',
    nameEn: 'University College London',
    lat: 51.5,
    lon: -0.12,
    regionZh: '伦敦 · 学术评议中心',
    regionEn: 'London · Academic review center',
    status: 'review',
    metricZh: '评议 12 项',
    metricEn: '12 reviews',
  },
  {
    id: 'geneva',
    nameZh: '日内瓦开放科学中心',
    nameEn: 'Geneva Open Science Center',
    lat: 46.2,
    lon: 6.14,
    regionZh: '日内瓦 · 科研治理中心',
    regionEn: 'Geneva · Research governance center',
    status: 'syncing',
    metricZh: '治理同步',
    metricEn: 'Governance sync',
  },
  {
    id: 'tokyo',
    nameZh: '东京大学研究中心',
    nameEn: 'University of Tokyo Research Center',
    lat: 35.68,
    lon: 139.65,
    regionZh: '东京 · 模型科学中心',
    regionEn: 'Tokyo · Model science center',
    status: 'operational',
    metricZh: '仿真 38 次',
    metricEn: '38 simulations',
  },
];

const CONNECTIONS = [
  ['westlake', 'shanghai'],
  ['westlake', 'beijing'],
  ['westlake', 'singapore'],
  ['singapore', 'tokyo'],
  ['tokyo', 'shanghai'],
  ['beijing', 'geneva'],
  ['geneva', 'london'],
  ['london', 'boston'],
  ['boston', 'westlake'],
] as const;

const STATUS_COPY: Record<ResearchHubStatus, { en: string; zh: string }> = {
  operational: { zh: '运行中', en: 'operational' },
  syncing: { zh: '同步中', en: 'syncing' },
  review: { zh: '待确认', en: 'to confirm' },
};

function normalizeLon(lon: number): number {
  let next = lon;
  while (next < -180) next += 360;
  while (next > 180) next -= 360;
  return next;
}

function isLand(lat: number, lon: number): boolean {
  const l = normalizeLon(lon);

  if (lat >= 12 && lat <= 78 && l >= -168 && l <= -50) {
    if (lat > 52 && lat < 64 && l > -94 && l < -76) return false;
    if (lat > 15 && lat < 30 && l > -98 && l < -82) return false;
    if (lat > 25) return l > -126 && l < -60;
    return l > -116 && l < -86;
  }

  if (lat >= -56 && lat < 12 && l >= -82 && l <= -34) {
    if (lat < -20) {
      const width = ((lat + 56) / 36) * 26;
      return l >= -67 - width && l <= -67 + width;
    }
    return lat < 5 ? l >= -81 && l <= -35 : l >= -77 && l <= -48;
  }

  if (lat >= -35 && lat <= 37 && l >= -18 && l <= 51) {
    if (lat > 12 && lat < 30 && l > 32 && l < 43) return false;
    if (l > 43) return false;
    if (lat > 5) return l >= -17 && l <= 34;
    const progress = (lat + 35) / 40;
    return l >= -10 - progress * 5 && l <= 22 + progress * 16;
  }

  if (lat > 35 && lat <= 72 && l >= -10 && l <= 40) {
    if (lat > 50 && lat < 61 && l > -11 && l < 2) return true;
    if (lat > 55 && l > 4 && l < 32) return true;
    if (lat > 40 && lat < 47 && l > 27 && l < 42) return false;
    return l > -5 && l < 40;
  }

  if (lat >= 5 && lat <= 76 && l > 35 && l <= 180) {
    if (lat > 50) return l > 35 && l < 172;
    if (lat > 12 && lat <= 30 && l > 34 && l < 60) return l < 60 - (lat - 12) * 0.45;
    if (lat > 8 && lat < 25 && l > 68 && l < 88) return lat > 8 + Math.abs(l - 78) * 1.1;
    if (lat > 8 && lat < 23 && l > 95 && l < 110) return true;
    if (lat > 33 && lat < 41 && l > 124 && l < 131) return true;
    return l > 55 && l < 142;
  }

  if (lat > 30 && lat < 46 && l > 129 && l < 146) return true;
  if (lat >= -39 && lat <= -10 && l >= 113 && l <= 154) return true;
  if (lat >= 60 && lat <= 83 && l >= -73 && l <= -10) return true;
  if (lat >= -26 && lat < -12 && l > 43 && l < 51) return true;
  if (lat > -9 && lat < 8 && l > 95 && l < 150) return true;

  return false;
}

function latLonToCartesian(lat: number, lon: number, radius: number) {
  const phi = (90 - lat) * (Math.PI / 180);
  const theta = (lon + 180) * (Math.PI / 180);

  return {
    x: -radius * Math.sin(phi) * Math.sin(theta),
    y: radius * Math.cos(phi),
    z: radius * Math.sin(phi) * Math.cos(theta),
  };
}

function buildGlobePoints(): GlobePoint[] {
  const points: GlobePoint[] = [];

  for (let lat = -75; lat <= 75; lat += 2.1) {
    const cosLat = Math.cos(lat * (Math.PI / 180));
    const lonStep = cosLat > 0.08 ? 2.15 / cosLat : 28;

    for (let lon = -180; lon < 180; lon += lonStep) {
      if (!isLand(lat, lon)) continue;

      const point = latLonToCartesian(lat, lon, 1);
      const opacity = 0.34 + ((Math.sin(lat * 12.9898 + lon * 78.233) + 1) / 2) * 0.38;
      points.push({
        origX: point.x,
        origY: point.y,
        origZ: point.z,
        opacity,
      });
    }
  }

  for (let index = 0; index < 130; index += 1) {
    const phi = Math.acos(-1 + (2 * index) / 130);
    const theta = Math.sqrt(130 * Math.PI) * phi;

    points.push({
      origX: Math.sin(phi) * Math.cos(theta),
      origY: Math.cos(phi),
      origZ: Math.sin(phi) * Math.sin(theta),
      opacity: 0.06 + (index % 9) * 0.012,
    });
  }

  return points;
}

const LoginResearchGlobe: React.FC<LoginResearchGlobeProps> = ({ onSelectHub, selectedHubId }) => {
  const { i18n } = useTranslation();
  const isChinese = i18n.language.startsWith('zh');

  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const containerRef = useRef<HTMLDivElement | null>(null);
  const hubsRef = useRef<ProjectedResearchHub[]>([]);
  const hoverRef = useRef<ProjectedResearchHub | null>(null);
  const onSelectHubRef = useRef(onSelectHub);
  const selectedHubIdRef = useRef(selectedHubId);

  const rotationX = useRef(0.25);
  const rotationY = useRef(1.18);
  const velocityX = useRef(0.0012);
  const velocityY = useRef(0.00012);
  const isDragging = useRef(false);
  const previousMousePosition = useRef({ x: 0, y: 0 });
  const pointsRef = useRef<GlobePoint[]>([]);

  const [hoveredHub, setHoveredHub] = useState<ProjectedResearchHub | null>(null);
  const [tooltipPos, setTooltipPos] = useState({ x: 0, y: 0 });

  const localizedHubs = useMemo<ProjectedResearchHub[]>(
    () =>
      HUBS.map((hub) => ({
        id: hub.id,
        label: isChinese ? hub.nameZh : hub.nameEn,
        lat: hub.lat,
        lon: hub.lon,
        metric: isChinese ? hub.metricZh : hub.metricEn,
        metricEn: hub.metricEn,
        metricZh: hub.metricZh,
        nameEn: hub.nameEn,
        nameZh: hub.nameZh,
        projX: 0,
        projY: 0,
        rawX: 0,
        rawY: 0,
        rawZ: 0,
        region: isChinese ? hub.regionZh : hub.regionEn,
        regionEn: hub.regionEn,
        regionZh: hub.regionZh,
        status: hub.status,
        z: 0,
      })),
    [isChinese]
  );

  useEffect(() => {
    hubsRef.current = localizedHubs;
    setHoveredHub(null);
  }, [localizedHubs]);

  useEffect(() => {
    onSelectHubRef.current = onSelectHub;
  }, [onSelectHub]);

  useEffect(() => {
    selectedHubIdRef.current = selectedHubId;
  }, [selectedHubId]);

  useEffect(() => {
    pointsRef.current = buildGlobePoints();
  }, []);

  useEffect(() => {
    const canvas = canvasRef.current;
    const container = containerRef.current;
    if (!canvas || !container) return;

    const ctx = canvas.getContext('2d');
    if (!ctx) return;

    const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    let animationFrameId = 0;
    let width = 0;
    let height = 0;
    let dpr = window.devicePixelRatio || 1;

    const updateSize = () => {
      width = container.clientWidth;
      height = container.clientHeight;
      dpr = window.devicePixelRatio || 1;
      canvas.width = Math.max(1, Math.floor(width * dpr));
      canvas.height = Math.max(1, Math.floor(height * dpr));
      canvas.style.width = `${width}px`;
      canvas.style.height = `${height}px`;
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    };

    const projectFactory = (cx: number, cy: number, radius: number) => {
      const cosX = Math.cos(rotationX.current);
      const sinX = Math.sin(rotationX.current);
      const cosY = Math.cos(rotationY.current);
      const sinY = Math.sin(rotationY.current);

      return (x3d: number, y3d: number, z3d: number) => {
        const x1 = x3d * cosY - z3d * sinY;
        const z1 = x3d * sinY + z3d * cosY;
        const y2 = y3d * cosX - z1 * sinX;
        const z2 = y3d * sinX + z1 * cosX;

        return {
          scale: 1 - (z2 / radius) * 0.2,
          x: cx + x1,
          y: cy - y2,
          z: z2,
        };
      };
    };

    const drawArc = (
      project: ReturnType<typeof projectFactory>,
      radius: number,
      fromHub: ProjectedResearchHub,
      toHub: ProjectedResearchHub
    ) => {
      const peak = radius * 0.16;
      let previous = project(fromHub.rawX, fromHub.rawY, fromHub.rawZ);

      for (let step = 1; step <= 20; step += 1) {
        const t = step / 20;
        const mt = 1 - t;
        const midX = (fromHub.rawX + toHub.rawX) / 2;
        const midY = (fromHub.rawY + toHub.rawY) / 2;
        const midZ = (fromHub.rawZ + toHub.rawZ) / 2;
        const midLength = Math.max(1, Math.sqrt(midX * midX + midY * midY + midZ * midZ));
        const control = {
          x: (midX / midLength) * (radius + peak),
          y: (midY / midLength) * (radius + peak),
          z: (midZ / midLength) * (radius + peak),
        };
        const currentPoint = {
          x: mt * mt * fromHub.rawX + 2 * mt * t * control.x + t * t * toHub.rawX,
          y: mt * mt * fromHub.rawY + 2 * mt * t * control.y + t * t * toHub.rawY,
          z: mt * mt * fromHub.rawZ + 2 * mt * t * control.z + t * t * toHub.rawZ,
        };
        const current = project(currentPoint.x, currentPoint.y, currentPoint.z);
        const alpha = current.z > radius * 0.22 ? 0.02 : 0.09 + Math.max(0, -current.z / radius) * 0.12;

        ctx.beginPath();
        ctx.moveTo(previous.x, previous.y);
        ctx.lineTo(current.x, current.y);
        ctx.strokeStyle = `rgba(20, 20, 19, ${alpha})`;
        ctx.lineWidth = 0.7;
        ctx.stroke();
        previous = current;
      }
    };

    const render = () => {
      if (!width || !height) {
        animationFrameId = requestAnimationFrame(render);
        return;
      }

      const radius = Math.min(width, height) * 0.31;
      const cx = width / 2;
      const cy = height / 2;

      ctx.clearRect(0, 0, width, height);

      if (!isDragging.current && !reduceMotion) {
        rotationY.current += velocityX.current;
        rotationX.current += velocityY.current;
        velocityX.current = velocityX.current * 0.96 + 0.0011 * 0.04;
        velocityY.current = velocityY.current * 0.96 + 0.00008 * 0.04;
      }

      rotationX.current = Math.max(-Math.PI / 2.5, Math.min(Math.PI / 2.5, rotationX.current));

      const project = projectFactory(cx, cy, radius);
      const glow = ctx.createRadialGradient(cx, cy, radius * 0.2, cx, cy, radius * 1.55);
      glow.addColorStop(0, 'rgba(182, 155, 74, 0.11)');
      glow.addColorStop(0.5, 'rgba(20, 20, 19, 0.035)');
      glow.addColorStop(1, 'rgba(20, 20, 19, 0)');
      ctx.fillStyle = glow;
      ctx.fillRect(0, 0, width, height);

      const sphere = ctx.createRadialGradient(cx - radius * 0.25, cy - radius * 0.28, radius * 0.1, cx, cy, radius);
      sphere.addColorStop(0, 'rgba(255, 255, 255, 0.8)');
      sphere.addColorStop(0.44, 'rgba(244, 242, 235, 0.62)');
      sphere.addColorStop(0.78, 'rgba(214, 219, 221, 0.32)');
      sphere.addColorStop(1, 'rgba(20, 20, 19, 0.06)');

      ctx.beginPath();
      ctx.arc(cx, cy, radius, 0, Math.PI * 2);
      ctx.fillStyle = sphere;
      ctx.shadowColor = 'rgba(20, 20, 19, 0.08)';
      ctx.shadowBlur = 24;
      ctx.shadowOffsetY = 16;
      ctx.fill();
      ctx.shadowColor = 'transparent';
      ctx.shadowBlur = 0;
      ctx.shadowOffsetY = 0;

      ctx.beginPath();
      ctx.arc(cx, cy, radius, 0, Math.PI * 2);
      ctx.fillStyle = '#faf8f5';
      ctx.fill();

      pointsRef.current.forEach((point) => {
        const projected = project(point.origX * radius, point.origY * radius, point.origZ * radius);
        const edgeFade = Math.min(1, Math.abs(projected.z / radius) / 0.22);
        const isFront = projected.z <= 0;
        ctx.beginPath();
        ctx.arc(projected.x, projected.y, (isFront ? 1.08 : 0.75) * projected.scale, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(20, 20, 19, ${point.opacity * (isFront ? 0.55 : 0.1) * edgeFade})`;
        ctx.fill();
      });

      const projectedHubs = hubsRef.current.map((hub) => {
        const raw = latLonToCartesian(hub.lat, hub.lon, radius * 1.04);
        const projected = project(raw.x, raw.y, raw.z);
        return {
          ...hub,
          projX: projected.x,
          projY: projected.y,
          rawX: raw.x,
          rawY: raw.y,
          rawZ: raw.z,
          z: projected.z,
        };
      });
      hubsRef.current = projectedHubs;

      CONNECTIONS.forEach(([fromId, toId]) => {
        const fromHub = projectedHubs.find((hub) => hub.id === fromId);
        const toHub = projectedHubs.find((hub) => hub.id === toId);
        if (fromHub && toHub) drawArc(project, radius, fromHub, toHub);
      });

      const pulseT = (Date.now() % 1200) / 1200;
      projectedHubs.forEach((hub) => {
        const isFront = hub.z <= radius * 0.2;
        const selected = selectedHubIdRef.current === hub.id;
        const opacity = isFront ? 1 : 0.25;

        ctx.beginPath();
        ctx.arc(hub.projX, hub.projY, selected ? 4 : 3, 0, Math.PI * 2);
        ctx.fillStyle = selected ? 'rgba(182, 155, 74, 0.96)' : `rgba(20, 20, 19, ${0.64 * opacity})`;
        ctx.fill();

        ctx.beginPath();
        ctx.arc(hub.projX, hub.projY, selected ? 9 + pulseT * 7 : 6, 0, Math.PI * 2);
        ctx.strokeStyle = selected
          ? `rgba(182, 155, 74, ${0.55 * (1 - pulseT)})`
          : `rgba(20, 20, 19, ${0.22 * opacity})`;
        ctx.lineWidth = selected ? 1.1 : 0.8;
        ctx.stroke();

        if (!isFront) return;

        ctx.font = '600 10px Inter, ui-sans-serif, sans-serif';
        const textX = hub.projX + 10;
        const textY = hub.projY + 4;
        const metrics = ctx.measureText(hub.label);
        ctx.fillStyle = 'rgba(250, 248, 245, 0.72)';
        ctx.fillRect(textX - 3, textY - 11, metrics.width + 6, 15);
        ctx.fillStyle = selected ? '#141413' : 'rgba(20, 20, 19, 0.64)';
        ctx.fillText(hub.label, textX, textY);
      });

      ctx.beginPath();
      ctx.arc(cx, cy, radius, 0, Math.PI * 2);
      ctx.strokeStyle = 'rgba(20, 20, 19, 0.1)';
      ctx.lineWidth = 1;
      ctx.stroke();

      ctx.beginPath();
      ctx.arc(cx - radius * 0.12, cy - radius * 0.12, radius * 0.92, -Math.PI * 0.76, -Math.PI * 0.08);
      ctx.strokeStyle = 'rgba(255, 255, 255, 0.52)';
      ctx.lineWidth = 1.5;
      ctx.stroke();

      animationFrameId = requestAnimationFrame(render);
    };

    const findHover = (mouseX: number, mouseY: number) => {
      let found: ProjectedResearchHub | null = null;

      for (const hub of hubsRef.current) {
        const distance = Math.sqrt((mouseX - hub.projX) ** 2 + (mouseY - hub.projY) ** 2);
        if (hub.z <= 32 && distance < 14) {
          found = hub;
          break;
        }
      }

      hoverRef.current = found;
      setHoveredHub(found);
      if (found) setTooltipPos({ x: found.projX, y: found.projY });
      canvas.style.cursor = found ? 'pointer' : 'grab';
    };

    const handleMouseDown = (event: MouseEvent) => {
      isDragging.current = true;
      canvas.style.cursor = 'grabbing';
      const rect = canvas.getBoundingClientRect();
      previousMousePosition.current = {
        x: event.clientX - rect.left,
        y: event.clientY - rect.top,
      };
    };

    const handleMouseMove = (event: MouseEvent) => {
      const rect = canvas.getBoundingClientRect();
      const mouseX = event.clientX - rect.left;
      const mouseY = event.clientY - rect.top;

      if (isDragging.current) {
        const deltaX = mouseX - previousMousePosition.current.x;
        const deltaY = mouseY - previousMousePosition.current.y;
        velocityX.current = deltaX * 0.0032;
        velocityY.current = deltaY * 0.0032;
        rotationY.current += velocityX.current;
        rotationX.current += velocityY.current;
        previousMousePosition.current = { x: mouseX, y: mouseY };
        return;
      }

      findHover(mouseX, mouseY);
    };

    const handleMouseUp = () => {
      isDragging.current = false;
      canvas.style.cursor = hoverRef.current ? 'pointer' : 'grab';
    };

    const handleClick = () => {
      if (hoverRef.current) {
        onSelectHubRef.current(hoverRef.current.id);
      }
    };

    updateSize();
    const resizeObserver = new ResizeObserver(updateSize);
    resizeObserver.observe(container);
    render();

    canvas.addEventListener('mousedown', handleMouseDown);
    window.addEventListener('mousemove', handleMouseMove);
    window.addEventListener('mouseup', handleMouseUp);
    canvas.addEventListener('click', handleClick);

    return () => {
      cancelAnimationFrame(animationFrameId);
      resizeObserver.disconnect();
      canvas.removeEventListener('mousedown', handleMouseDown);
      window.removeEventListener('mousemove', handleMouseMove);
      window.removeEventListener('mouseup', handleMouseUp);
      canvas.removeEventListener('click', handleClick);
    };
  }, []);

  const statusCopy = hoveredHub ? STATUS_COPY[hoveredHub.status] : null;

  return (
    <div
      ref={containerRef}
      aria-label={isChinese ? '科研协作地图' : 'Research collaboration map'}
      className='login-globe'
      role='img'
    >
      <div className='login-globe__halo login-globe__halo--inner' aria-hidden='true' />
      <div className='login-globe__halo login-globe__halo--outer' aria-hidden='true' />
      <canvas ref={canvasRef} className='login-globe__canvas' />

      {hoveredHub && statusCopy && (
        <div
          className='login-globe__tooltip'
          style={{
            left: `${tooltipPos.x + 14}px`,
            top: `${tooltipPos.y - 12}px`,
          }}
        >
          <div className='login-globe__tooltip-header'>
            <span>{hoveredHub.label}</span>
            <i className={`login-globe__status login-globe__status--${hoveredHub.status}`} />
          </div>
          <p>{hoveredHub.region}</p>
          <dl>
            <div>
              <dt>{isChinese ? '状态' : 'Status'}</dt>
              <dd>{isChinese ? statusCopy.zh : statusCopy.en}</dd>
            </div>
            <div>
              <dt>{isChinese ? '指标' : 'Metric'}</dt>
              <dd>{hoveredHub.metric}</dd>
            </div>
          </dl>
        </div>
      )}
    </div>
  );
};

export default LoginResearchGlobe;
