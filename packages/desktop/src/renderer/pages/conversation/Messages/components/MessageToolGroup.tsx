...
const getCommandLikeResult = (
  content: IMessageToolGroupProps['message']['content'][number]
): CommandLikeResult | undefined => {
  const record = toRecord(content.result_display);
  const input = (() => {
  ...
  const command = getStringValue(record, ['command', 'cmd']) || getStringValue(input, ['command', 'cmd']);
  const stdout = getStringValue(record, ['stdout', 'std_out']);
  const stderr = getStringValue(record, ['stderr', 'std_err', 'error']);
  const output =
    getStringValue(record, ['output', 'result', 'text']) ||
    (typeof content.result_display === 'string' ? content.result_display : undefined);
  const exitCode = getStringValue(record, ['exitCode', 'exit_code', 'code', 'statusCode']);
  const looksCommandLike =
    Boolean(command) ||
    Boolean(stdout) ||
    Boolean(stderr) ||
    /^(bash|shell|exec|execute|run|command|terminal)$/i.test(content.name || '');

  if (!looksCommandLike) return undefined;
  return { command, stdout, stderr, output, exitCode };
};

const CommandResultDisplay: React.FC<{ result: CommandLikeResult }> = ({ result }) => (
  <div className='message-tool-command-result'>
    {result.command && <pre className='message-tool-command-result__command'>$ {result.command}</pre>}
    {result.stdout && <pre className='message-tool-command-result__output'>{result.stdout}</pre>}
    {!result.stdout && result.output && <pre className='message-tool-command-result__output'>{result.output}</pre>}
    {result.stderr && <pre className='message-tool-command-result__stderr'>{result.stderr}</pre>}
    {result.exitCode !== undefined && <div className='message-tool-command-result__meta'>exit {result.exitCode}</div>}
  </div>
);
...
const ToolResultDisplay: React.FC<{
  ...
  const commandResult = getCommandLikeResult(content);
  if (commandResult) {
    return <CommandResultDisplay result={commandResult} />;
  }
