...
const iconForToolItem = (item: NormalizedToolCall): ActivityIconSpec => {
  if (item.status === 'error' || item.status === 'canceled') return statusIconForActivity(item.status);
  if (item.imagePath) return { name: 'imageFiles' };

  const text = normalizeToolText(`${item.kind || ''} ${item.name || ''} ${item.description || ''}`);
  ...
  if (/(bash|shell|exec|execute|run|command|terminal)/.test(text)) {
    return iconForCommandIntent(`${item.name || ''} ${item.description || ''}`, item.status);
  }
  if (/(mcp|tool)/.test(text)) return { name: 'tool' };
  if (/(todo|plan|tasklist)/.test(text)) return { name: 'listCheckbox' };
  return statusIconForActivity(item.status);
};

const ToolItemIcon: React.FC<{ item: NormalizedToolCall }> = ({ item }) => {
  const icon = iconForToolItem(item);
  return <AgentStatusIcon className='tool-status-icon' name={icon.name} size={15} spin={icon.spin} />;
};
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className='flex flex-row color-#86909C gap-12px items-center'>
        <ToolItemIcon item={item} />
        <button
          type='button'
          className={
            'tool-item-detail__label flex-1 min-w-0' +
            (expanded ? ' break-all' : ' truncate') +
            (hasDetail ? ' cursor-pointer hover:color-#4E5969' : ' tool-item-detail__label--static')
          }
        ...
        {hasDetail && (
          <button
            type='button'
            className='tool-item-detail__toggle flex-shrink-0 cursor-pointer hover:color-#4E5969 transition-colors'
    ...
    <div className='tool-group-summary'>
      ...
      >
        <span className='tool-group-summary__icon'>
          {hasRunning ? (
            <AgentStatusIcon name='loading' size={15} spin />
          ) : (
            <AgentStatusIcon name='listCheckbox' size={15} />
          )}
        </span>
        ...
        <span className={`tool-group-summary__arrow${showMore ? ' tool-group-summary__arrow--open' : ''}`}>
          {showMore ? <IconDown style={{ fontSize: 12 }} /> : <IconRight style={{ fontSize: 12 }} />}
        </span>
      </button>
      {showMore && (
