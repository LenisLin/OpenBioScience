...
import { getMermaidTheme } from '../../theme';
...
const extractCodeText = (children: React.ReactNode): string => {
  ...
  if (React.isValidElement<{ children?: React.ReactNode }>(children)) return extractCodeText(children.props.children);
  return '';
};

const MarkdownCode: React.FC<MarkdownCodeProps> = ({ children, className, node, ...props }) => {
  const startLine = node?.position?.start?.line;
  const endLine = node?.position?.end?.line;
  const isInline = typeof startLine === 'number' && startLine === endLine;
  ...
  if (language === 'mermaid') {
    return <MermaidBlock code={code} showOpenInPanelButton={false} />;
  }
  ...
  return (
    <div data-language={language} data-streamdown='code-block'>
      ...
      <pre className={className} data-language={language} data-streamdown='code-block-body'>
        <code>{code}</code>
      </pre>
    </div>
  );
    ...
    <div className='flex flex-col w-full h-full overflow-hidden'>
      ...
      >
        {viewMode === 'source' ? (
          ...
          <div
            className='deeporganiser-markdown'
            style={{
              wordWrap: 'break-word',
