...
const ENABLED_CONVERSATION_KEY = 'deeporganiser:e2e-message-stream-conversation-id';
...
declare global {
  interface Window {
    __DEEPORGANISER_E2E_MESSAGE_STREAM__?: StreamRegistry;
  }
}
...
const AcpE2EStreamInjector: React.FC<{ conversationId: string }> = ({ conversationId }) => {
  ...
  useEffect(() => {
    ...
    const registry = (window.__DEEPORGANISER_E2E_MESSAGE_STREAM__ ??= { controllers: {} });
    ...
    return () => {
      if (window.__DEEPORGANISER_E2E_MESSAGE_STREAM__) {
        delete window.__DEEPORGANISER_E2E_MESSAGE_STREAM__.controllers[conversationId];
      }
    };
