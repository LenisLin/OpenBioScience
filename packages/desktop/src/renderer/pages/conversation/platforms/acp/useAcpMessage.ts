...
export const useAcpMessage = (conversation_id: string, options?: { skipWarmup?: boolean }): UseAcpMessageReturn => {
  ...
  const persistedContextUsageRef = useRef('');
  ...
  const assistantTextTurnRef = useRef<{
    lastTextId?: string;
    lastTextMsgId?: string;
    lastTextCreatedAt?: number;
  }>({});
    ...
    (message: IResponseMessage) => {
      ...
      switch (message.type) {
        ...
        case 'finish':
          {
            ...
            const finalText = assistantTextTurnRef.current;
            if (finalText.lastTextId) {
              addOrUpdateMessage({
                id: finalText.lastTextId,
                msg_id: finalText.lastTextMsgId,
                conversation_id,
                type: 'text',
                position: 'left',
                status: 'finish',
                created_at: finalText.lastTextCreatedAt ?? message.created_at ?? Date.now(),
                content: {
                  content: '',
                  finality: 'final',
                },
        ...
        case 'content': {
          ...
          setThought({ subject: '', description: '' });
          if (transformedMessage?.type === 'text' && transformedMessage.position === 'left') {
            const previousTextId = assistantTextTurnRef.current.lastTextId;
            if (previousTextId && previousTextId !== transformedMessage.id) {
              addOrUpdateMessage({
                id: previousTextId,
                msg_id: assistantTextTurnRef.current.lastTextMsgId,
                conversation_id,
                type: 'text',
                position: 'left',
                status: 'finish',
                created_at: assistantTextTurnRef.current.lastTextCreatedAt ?? Date.now(),
                content: {
                  content: '',
                  finality: 'progress',
                },
            ...
            assistantTextTurnRef.current = {
              lastTextId: transformedMessage.id,
              lastTextMsgId: transformedMessage.msg_id,
              lastTextCreatedAt: transformedMessage.created_at,
            };
            addOrUpdateMessage({
              ...transformedMessage,
              content: {
                ...transformedMessage.content,
                finality: transformedMessage.content.finality ?? 'progress',
              },
            });
            break;
          }
        ...
        case 'acp_context_usage': {
          ...
          if (usageData && typeof usageData.used === 'number') {
            const nextTokenUsage: TokenUsageData = { total_tokens: usageData.used };
            const nextContextLimit =
              typeof usageData.size === 'number' && usageData.size > 0 ? usageData.size : undefined;
            ...
            const signature = `${nextTokenUsage.total_tokens}:${nextContextLimit ?? 0}`;
            if (persistedContextUsageRef.current !== signature) {
              persistedContextUsageRef.current = signature;
              void ipcBridge.conversation.update.invoke({
                id: conversation_id,
                updates: {
                  extra: {
                    last_token_usage: nextTokenUsage,
                    ...(nextContextLimit ? { last_context_limit: nextContextLimit } : {}),
  ...
  useEffect(() => {
    ...
    setContextLimit(0);
    setSlashCommands([]);
    persistedContextUsageRef.current = '';
    hasContentInTurnRef.current = false;
    turnFinishedRef.current = false;
    hasThinkingMessageRef.current = false;
    activeThinkingRef.current = null;
  ...
  // Mirrors the local runtime pattern: warmup first, then fetch.
