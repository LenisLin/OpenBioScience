...
const AcpSendBox: React.FC<{
  ...
  const resolvedContextLimit = useMemo(
    () => (context_limit > 0 ? context_limit : getModelContextLimit(model_info?.current_model_id)),
    [context_limit, model_info?.current_model_id]
  );
  const supportsCompactContext = useMemo(
...
Please check your local CLI tool authentication status`,
  ...
  const handleCompactContext = useCallback(() => {
    if (!supportsCompactContext || isBusy) return;
    void executeCommand({ input: '/compact', files: [] });
  }, [executeCommand, isBusy, supportsCompactContext]);
  ...
  const handleGuideQueuedCommand = useCallback(
    async (item: ConversationCommandQueueItem) => {
      pause();
      remove(item.id, { pause: true });

      try {
        if (runtimeView.isProcessing || isBusy || teamRuntime?.loading) {
          await effectiveHandleStop();
        }
        await executeCommand({ input: item.input, files: item.files });
        resume();
      } catch (error) {
        console.error('[AcpSendBox] Failed to guide queued command:', error);
        restore(item, { pause: true });
        Message.warning(
          t('conversation.commandQueue.guideFailed', {
            defaultValue: 'The guided command could not start. It has been returned to the queue.',
          })
        );
      }
    },
  ...
  return (
    <div className='max-w-800px w-full mx-auto flex flex-col mt-auto mb-16px'>
      <CommandQueuePanel
        ...
        onGuide={(item) => void handleGuideQueuedCommand(item)}
        onReorder={reorder}
        onRemove={remove}
        onClear={clear}
      />
      <div className='relative z-30 pointer-events-none'>
        <RunningAgentProgress runtimeView={runtimeView.view} />
      </div>
      ...
      <SendBox
        ...
        sendButtonPrefix={
          <ContextUsageIndicator
            ...
            context_limit={resolvedContextLimit}
            size={18}
            onCompact={supportsCompactContext ? handleCompactContext : undefined}
            disabled={teamRuntime?.loading ?? isBusy}
            showWhenEmpty
          />
        }
        prefix={
