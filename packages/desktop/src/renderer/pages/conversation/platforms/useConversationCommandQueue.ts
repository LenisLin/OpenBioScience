...
const readStoredQueueState = (conversation_id: string): string | null => {
  ...
  const legacySessionValue = window.sessionStorage.getItem(storageKey);
  if (legacySessionValue) {
    window.localStorage.setItem(storageKey, legacySessionValue);
...
const readPersistedQueueState = (conversation_id: string): ConversationCommandQueueState => {
  ...
  try {
    const stored = readStoredQueueState(conversation_id);
    if (!stored) {
      return createDefaultQueueState();
    }
...
export const useConversationCommandQueue = ({
  ...
  const remove = useCallback(
    (commandId: string, options: { pause?: boolean } = {}) => {
      if (!enabled) {
        return;
      }
      ...
      void updateState((state) => {
        ...
        return {
          items: nextItems,
          isPaused: shouldPause && nextItems.length > 0,
  ...
  const restore = useCallback(
    (item: ConversationCommandQueueItem, options: { pause?: boolean } = {}) => {
      if (!enabled) {
        return;
      }
      ...
      waitingForTurnCompletionRef.current = false;
      waitingForTeamUpgradeHandoffRef.current = false;
      logCommandQueue(conversation_id, 'restored-command', {
        item: summarizeQueuedCommand(item),
        paused: shouldPause,
      });
      void updateState((state) => ({
