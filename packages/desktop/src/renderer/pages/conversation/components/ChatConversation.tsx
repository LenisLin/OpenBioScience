...
import { useConversationLoopGoal } from './useConversationLoopGoal';
...
const normalizeLegacyLocalRuntimeConversation = (
  ...
  const rawConversation = conversation as unknown as { type?: string; extra?: Record<string, unknown> } | undefined;
  if (!conversation || rawConversation?.type !== LEGACY_LOCAL_RUNTIME_ID) return conversation;
  return {
    ...conversation,
    type: 'acp',
    extra: {
...
const ChatConversation: React.FC<{
  ...
  hideSendBox?: boolean;
}> = ({ conversation: rawConversation, hideSendBox }) => {
  ...
  if (!conversation || !teamConversationRuntime) return chatContent;

  return (
    <TeamPermissionProvider
      team_id={teamConversationRuntime.team.id}
      isLeaderAgent={teamConversationRuntime.isLeaderAgent}
      leaderConversationId={getTeamLeaderConversationId(teamConversationRuntime, conversation.id)}
      allConversationIds={teamConversationRuntime.allConversationIds}
    >
      {chatContent}
    </TeamPermissionProvider>
  );
