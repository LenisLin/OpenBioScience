...
const ChatLayout: React.FC<{
  ...
  /** Optional default width for the right workspace/project panel. */
  workspaceDefaultWidthPx?: number;
  /** Optional preference key for the right workspace/project panel width. */
  workspaceWidthStorageKey?: string;
  ...
  const {
    ...
    unit: 'px',
    defaultWidth: props.workspaceDefaultWidthPx ?? DEFAULT_WORKSPACE_PANEL_PX,
    minWidth: MIN_WORKSPACE_PANEL_PX,
    maxWidth: MAX_WORKSPACE_PANEL_PX,
    storageKey: props.workspaceWidthStorageKey ?? 'chat-workspace-width-px',
  ...
  const {
    ...
    onDragPastMax: closePreview,
    dragPastMaxThresholdPx: Math.max(48, Math.round(containerWidth * 0.06)),
  });
