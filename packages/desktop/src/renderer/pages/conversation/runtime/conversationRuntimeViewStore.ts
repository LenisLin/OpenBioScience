...
const summarizeView = (view: ConversationRuntimeView): Record<string, unknown> => ({
  ...
  activeStartedAt: view.activeStartedAt,
  state: view.state,
...
const viewFromRuntimeSummary = (
  ...
  const isProcessing = pendingLocalSend || isCancelling || runtime.is_processing;
  const continuesSameTurn = previous.activeTurnId !== null && previous.activeTurnId === activeTurnId;
  const activeStartedAt = isProcessing
    ? continuesSameTurn
      ? (previous.activeStartedAt ?? Date.now())
      : previous.localSubmitting && previous.activeStartedAt
        ? previous.activeStartedAt
        : Date.now()
    : null;
  const localStopping =
...
export const localSendStartedConversationRuntimeView = (
  ...
  const view: ConversationRuntimeView = {
    ...base,
    activeStartedAt: base.activeStartedAt ?? Date.now(),
