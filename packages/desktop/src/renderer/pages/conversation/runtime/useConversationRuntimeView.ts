...
export const useConversationRuntimeView = (conversation_id: string): UseConversationRuntimeViewReturn => {
  ...
  const rehydrateRuntimeView = useCallback(() => {
    if (!conversation_id) {
      return;
    }
  ...
  useEffect(() => {
    const request = rehydrateRuntimeView();
    return () => {
      request?.cancel();
    };
  }, [rehydrateRuntimeView]);

  useEffect(() => {
    if (!conversation_id || typeof window === 'undefined' || typeof document === 'undefined') {
      return;
    }

    const rehydrateIfVisible = () => {
    ...
    window.addEventListener('focus', rehydrateIfVisible);
    document.addEventListener('visibilitychange', rehydrateIfVisible);

    return () => {
      window.removeEventListener('focus', rehydrateIfVisible);
      document.removeEventListener('visibilitychange', rehydrateIfVisible);
    };
  }, [conversation_id, rehydrateRuntimeView]);

  useEffect(() => {
    if (!conversation_id || !view.hydrated) {
      return;
    }
    ...
    const needsRuntimeSync =
      ...
      view.localStopping ||
      view.state === 'starting';
    if (!needsRuntimeSync || typeof window === 'undefined') {
      return;
    }
