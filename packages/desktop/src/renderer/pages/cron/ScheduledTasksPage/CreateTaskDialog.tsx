...
function getPromptInitialValue(job: ICronJob): string {
...
function getAgentKeyFromJob(job: ICronJob, cliAgents: { backend?: string; agent_type: string }[]): string | undefined {
  ...
  if (config) {
    ...
    if (config.backend === LEGACY_LOCAL_RUNTIME_ID || config.preset_agent_type === LEGACY_LOCAL_RUNTIME_ID) return 'cli:codex';
...
const CreateTaskDialog: React.FC<CreateTaskDialogProps> = ({
  ...
  useEffect(() => {
    if (!visible) return;
    if (editJob) {
      const agentConfig = editJob.metadata?.agent_config;
      ...
      setAdvancedOpen(
        Boolean(
          ...
          (agentConfig?.config_options && Object.keys(agentConfig.config_options).length > 0)
        )
      );
      ...
      form.setFieldsValue({
        ...
        prompt: getPromptInitialValue(editJob),
        agent: agentKey,
      });
      // Populate advanced settings from editJob
      setModelId(agentConfig?.model_id);
      setConfigOptions(agentConfig?.config_options);
      setWorkspace(agentConfig?.workspace);
    } else {
      form.resetFields();
      setFrequency('manual');
      setTime('09:00');
  ...
  const resolvedBackend = useMemo(() => {
    ...
    return agentId === LEGACY_LOCAL_RUNTIME_ID ? 'codex' : agentId;
  ...
  const geminiCurrentModel = useMemo<TProviderWithModel | undefined>(() => {
    if (resolvedBackend !== 'gemini' || !model_id) return undefined;
  ...
  const acpCachedModelInfo = useMemo<AcpModelInfo | null>(() => {
    if (!resolvedBackend || resolvedBackend === 'gemini') return null;
              ...
              {cliAgents.length > 0 && (
                <OptGroup label={t('conversation.dropdown.cliAgents')}>
                  {cliAgents.map((agent) => {
                    ...
                    const disabled = agentKey === LEGACY_LOCAL_RUNTIME_ID;
                    return (
                      <Option key={`cli:${agentKey}`} value={`cli:${agentKey}`} disabled={disabled}>
                        <div
                          className='flex items-center gap-8px'
                          title={disabled ? t('cron.page.form.agentDeprecated', { defaultValue: 'This runtime has been replaced by Codex.' }) : undefined}
                        >
                          {logo ? (
                          ...
                          {disabled && (
                            <span className='text-12px text-t-tertiary'>
                              {t('cron.page.form.agentDeprecated', { defaultValue: 'Replaced by Codex' })}
                            </span>
                          )}
                        </div>
                      </Option>
              ...
              <div className='mt-12px grid gap-x-16px gap-y-16px md:grid-cols-2'>
                ...
                <div className={advancedFieldCount === 1 ? 'md:col-span-2' : ''}>
                  ...
                  <WorkspaceFolderSelect
                    ...
                    recentLabel={t('common.recent', { defaultValue: 'Recent' })}
                    chooseDifferentLabel={t('guid.workspace.specifyWorkspace', {
