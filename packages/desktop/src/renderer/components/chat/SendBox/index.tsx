...
const SendBox: React.FC<{
  ...
  const inputPlaceholder =
    ...
    t('conversation.sendbox.placeholder', { defaultValue: 'Ask, attach, or type /' });
  ...
  const sendButton = (
    <Button
      ...
      icon={<SendBoxIcon name='send' size={18} title={t('common.send', { defaultValue: 'Send' })} />}
      onClick={() => {
        sendMessageHandler();
      }}
  ...
  const stopButton = (
    <Button
      shape='circle'
      type='secondary'
      className='sendbox-stop-button'
      icon={<span className='sendbox-stop-button__glyph' aria-hidden='true' />}
      onClick={stopHandler}
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        {isUploading && (
          <div className='sendbox-run-status' role='status' aria-live='polite'>
            <span className='sendbox-run-status__dot' aria-hidden='true' />
            <span className='sendbox-run-status__text'>
              {t('common.fileAttach.uploading', { defaultValue: 'Uploading...' })}
            </span>
          </div>
        )}
        <div
