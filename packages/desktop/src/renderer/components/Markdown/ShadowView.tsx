...
const createInitStyle = (
  ...
  :host {
    ${cssVarsDeclaration}
    --markdown-link-ink: var(--brand, #7583b2);
    --markdown-link-hover: color-mix(in srgb, var(--brand, #7583b2) 76%, var(--text-primary, #000000) 24%);
    --markdown-link-accent: #b69b4a;
    --markdown-link-bg: color-mix(in srgb, var(--brand-light, #eff0f6) 76%, transparent);
    --markdown-link-border: color-mix(in srgb, var(--brand, #7583b2) 20%, transparent);
  }
  ...
  * {
  ...
  a{
    color: var(--markdown-link-ink);
    text-decoration-line: underline;
    text-decoration-color: color-mix(in srgb, var(--markdown-link-accent) 72%, transparent);
  ...
  a:hover {
    ...
    background: var(--markdown-link-bg);
    text-decoration-color: var(--markdown-link-accent);
  }
  a:focus-visible {
    outline: 2px solid color-mix(in srgb, var(--markdown-link-accent) 45%, transparent);
    outline-offset: 2px;
  }
  .markdown-local-file-link {
    ...
    min-height: 22px;
    padding: 2px 6px;
    background: color-mix(in srgb, var(--brand-light, #eff0f6) 86%, transparent);
    color: var(--markdown-link-ink);
    border: 1px solid var(--markdown-link-border);
    border-left-color: color-mix(in srgb, var(--markdown-link-accent) 64%, transparent);
    border-radius: 6px;
    box-shadow: inset 0 -1px 0 color-mix(in srgb, var(--markdown-link-accent) 18%, transparent);
    font: inherit;
    font-weight: 500;
    line-height: inherit;
    ...
    --agent-output-icon-ink: var(--markdown-link-accent);
    --agent-output-icon-paper: var(--bg-base);
    transition:
      background-color 0.15s ease,
  ...
  .markdown-local-file-link:hover {
    background: color-mix(in srgb, var(--brand-light, #eff0f6) 100%, var(--bg-base, #ffffff) 6%);
    color: var(--markdown-link-hover);
    border-color: color-mix(in srgb, var(--brand, #7583b2) 30%, transparent);
    border-left-color: var(--markdown-link-accent);
    text-decoration: none;
  }
  .agent-output-icon {
    position: relative;
  ...
  .agent-output-icon__dark {
    display: none !important;
  }
  .agent-status-icon__dark {
    display: none !important;
  }
  ${
    currentTheme === 'dark'
      ? `
  .agent-output-icon__light {
    display: none !important;
  }
  .agent-output-icon__dark {
    display: block !important;
  }
  .agent-status-icon__light {
    display: none !important;
  }
  .agent-status-icon__dark {
    display: block !important;
  ...
  const updateStyles = React.useCallback(
    (shadowRoot: ShadowRoot) => {
      ...
      const cssVars = {
        ...
        '--bg-base': computedStyle.getPropertyValue('--bg-base'),
        ...
        '--brand-light': computedStyle.getPropertyValue('--brand-light'),
        '--brand-hover': computedStyle.getPropertyValue('--brand-hover'),
