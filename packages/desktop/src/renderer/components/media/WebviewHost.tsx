...
export interface WebviewHostProps {
  ...
  partition?: string;
  /** Whether to intercept links/window.open/forms and keep navigation inside this host */
  interceptNavigation?: boolean;
  /** Whether the embedded page may open popup windows */
  allowPopups?: boolean;
  /** Optional user agent override for embedded services that gate Electron defaults */
  userAgent?: string;
  /** Loading label shown when the nav bar is hidden */
  loadingLabel?: string;
  /** Extra class names for root container */
  className?: string;
  /** Extra styles for root container */
  style?: React.CSSProperties;
  /** Called when the embedded page navigates */
  ...
  /** Optional one-shot script command for parent-controlled embedded flows */
  scriptCommand?: { key: number; script: string };
  /** Called when scriptCommand resolves or fails */
  onScriptCommandResult?: (result: unknown) => void;
}
...
function readWidthResult(result: unknown): number {
  if (!result || typeof result !== 'object' || !('width' in result)) return 0;
  return Number((result as { width?: unknown }).width || 0);
}
...
const WebviewHost: React.FC<WebviewHostProps> = ({
  ...
  useEffect(() => {
    const webviewEl = webviewRef.current;
    if (!webviewReady || !webviewEl || !scriptCommand) return;
    webviewEl
      .executeJavaScript(scriptCommand.script)
      .then((result: unknown) => {
        onScriptCommandResult?.(result);
      })
      .catch((error: unknown) => {
        onScriptCommandResult?.({
          ok: false,
  ...
  }, [onScriptCommandResult, scriptCommand, webviewReady]);
  ...
  useEffect(() => {
    ...
    const handleConsoleMessage = (event: Electron.ConsoleMessageEvent) => {
      try {
        ...
        if (event.message.includes('__DEEPORGANISER_WEBVIEW_ZOOM__')) {
        ...
        if (event.message.includes('__DEEPORGANISER_WEBVIEW_ZOOM_RESET__')) {
          setZoomFactor(1);
        }
      } catch {
        // Ignore parse errors
      ...
      if (isStarOfficeUrl(currentUrl)) {
        webviewEl
          .executeJavaScript(
            `
          (function() {
            if (window.__deeporganiserZoomInjected) return true;
            window.__deeporganiserZoomInjected = true;
            window.addEventListener('wheel', function(e) {
              if (!(e.ctrlKey || e.metaKey)) return;
              e.preventDefault();
              console.log('__DEEPORGANISER_WEBVIEW_ZOOM__', JSON.stringify({ deltaY: e.deltaY }));
            }, { passive: false, capture: true });
            window.addEventListener('keydown', function(e) {
              if (!(e.ctrlKey || e.metaKey)) return;
              if (e.key === '0') {
                e.preventDefault();
                console.log('__DEEPORGANISER_WEBVIEW_ZOOM_RESET__');
              }
            }, { capture: true });
            return true;
          })();
      ...
      if (isStarOfficeUrl(currentUrl) && autoFitPendingRef.current) {
        window.setTimeout(() => {
          ...
          `
            ...
            .then((result: unknown) => {
              const stageWidth = readWidthResult(result);
    ...
    const handleDidFailLoad = (event: WebviewFailLoadEvent) => {
      setIsLoading(false);
      onDidFailLoad?.(event.errorCode ?? 0, event.errorDescription ?? '');
    };
    ...
    const resize = () => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (contentRect.width > 0 && contentRect.height > 0) {
          ...
          webviewEl.style.minWidth = `${contentRect.width}px`;
          webviewEl.style.minHeight = `${contentRect.height}px`;
        }
      });
    };
  ...
  return (
    <div ref={containerRef} className={`h-full w-full flex flex-col ${className ?? ''}`} style={style}>
      ...
      {!showNavBar && isLoading && (
        <PageLoadingState label={loadingLabel} className='z-10' />
      )}
      ...
      >
        <webview
          ref={(element) => {
            webviewRef.current = element as unknown as Electron.WebviewTag | null;
          }}
          src={currentUrl}
          className='border-0 absolute inset-0 h-full w-full'
          style={{
            display: 'block',
            width: '100%',
            height: '100%',
