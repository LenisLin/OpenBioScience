...
export const UPDATE_AVAILABLE_EVENT = 'deeporganiser-update-available';
...
const RELEASES_PAGE_URL = 'https://github.com/ResearAI/DeepOrganiser/releases';
...
export const useUpdateNotificationController = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('deeporganiser-open-update-modal', handleWindowOpen);
    ...
    return () => {
      removeOpenListener();
      window.removeEventListener('deeporganiser-open-update-modal', handleWindowOpen);
