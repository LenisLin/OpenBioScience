...
const isHiddenBuiltinServer = (server: IMcpServer) =>
  server.builtin === true &&
  (server.id === BUILTIN_LARK_PROJECT_AGENT_ID || server.name === BUILTIN_LARK_PROJECT_AGENT_NAME);
...
const ModalMcpManagementSection: React.FC<{
  ...
  const visibleMcpServers = useMemo(
    () => mcpServers.filter((server) => !isBuiltinImageGenServer(server) && !isHiddenBuiltinServer(server)),
    [mcpServers]
  );
