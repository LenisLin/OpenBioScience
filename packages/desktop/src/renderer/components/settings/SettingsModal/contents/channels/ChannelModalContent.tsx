...
const ChannelModalContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.setTimeout(() => {
      document.querySelector(`[data-channel-id="${focusChannel}"]`)?.scrollIntoView({
        block: 'center',
        behavior: 'smooth',
      });
    }, 80);
  ...
  const channels: ChannelConfig[] = useMemo(() => {
    const telegramChannel: ChannelConfig = {
      ...
      description: t('settings.channels.telegramDesc', 'Chat with DeepOrganiser assistant via Telegram'),
      status: 'active',
    ...
    const larkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.larkDesc', 'Chat with DeepOrganiser assistant via Lark or Feishu'),
      status: 'active',
    ...
    const dingtalkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.dingtalkDesc', 'Chat with DeepOrganiser assistant via DingTalk'),
      status: 'active',
    ...
    const weixinChannel: ChannelConfig = {
      ...
      description: t('settings.channels.weixinDesc', 'Chat with DeepOrganiser assistant via WeChat'),
      status: 'active',
    ...
    const wecomChannel: ChannelConfig = {
      ...
      description: t(
        ...
        'Chat with DeepOrganiser assistant via WeCom (Enterprise WeChat)'
      ),
    ...
    const extensionTypeSet = new Set(extensionChannels.map((item) => String(item.id).toLowerCase()));
    const comingSoonChannels: ChannelConfig[] = [
      {
        id: 'slack',
        title: t('settings.channels.slackTitle', 'Slack'),
        description: t('settings.channels.slackDesc', 'Chat with DeepOrganiser assistant via Slack'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
      ...
      },
      {
        ...
        description: t('settings.channels.discordDesc', 'Chat with DeepOrganiser assistant via Discord'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
    ...
    ].filter((item) => !extensionTypeSet.has(String(item.id).toLowerCase()));
  ...
  const channelGuideText = t('settings.webui.featureChannelsDesc', {
    defaultValue: 'Connect Telegram, Lark, and DingTalk to interact with DeepOrganiser from IM apps.',
  });
        ...
        <div className='space-y-12px mt-12px'>
          {channels.map((channelConfig) => (
            <ChannelItem
              ...
              highlighted={focusChannel === channelConfig.id}
            />
          ))}
        </div>
      </div>
