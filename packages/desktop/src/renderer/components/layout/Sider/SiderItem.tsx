...
export type SiderItemProps = {
  ...
  inlineActions?: SiderInlineAction[];
...
const SiderItem: React.FC<SiderItemProps> = ({
  ...
  const hasInlineActions = inlineActions && inlineActions.length > 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className='size-24px flex items-center justify-center shrink-0 line-height-0 text-t-primary relative'>
          <span
        ...
        {/* Name with truncation — reserve room for hover actions */}
        <div
          className={classNames('h-24px min-w-0 flex-1 overflow-hidden', {
            'pr-40px': hasInlineActions && hasMenu,
            'pr-32px': (hasInlineActions || hasMenu) && !(hasInlineActions && hasMenu),
            'pr-12px': !hasInlineActions && !hasMenu,
          })}
        >
        ...
        {hasInlineActions && (
          <div
            className={classNames('absolute right-8px top-1/2 -translate-y-1/2 items-center justify-end gap-2px', {
              flex: isMobile,
          ...
          >
            {inlineActions.map((action) => (
              <button
                ...
                title={action.label}
                className={classNames(
                  'flex-center cursor-pointer transition-colors size-22px rd-5px bg-fill-2/90 hover:bg-fill-3',
                  action.danger ? 'text-[rgb(var(--danger-6))]' : 'text-t-secondary hover:text-t-primary'
                )}
                onClick={(e) => {
                  e.stopPropagation();
                  action.onClick();
        ...
        {hasMenu && (
          <div
            className={classNames('absolute top-1/2 -translate-y-1/2 items-center justify-end', {
              'right-36px': hasInlineActions,
              'right-8px': !hasInlineActions,
