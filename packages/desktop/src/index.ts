...
import { BackendLifecycleManager } from '@deeporganiser/web-host';
...
const isEnvEnabled = (name: string): boolean =>
  process.env[name] === '1' || process.env[legacyEnvName(name.replace('DEEPORGANISER_', ''))] === '1';

const isE2ETestMode = isEnvEnabled('DEEPORGANISER_E2E_TEST');
const skipSingleInstanceLock = isE2ETestMode || isEnvEnabled('DEEPORGANISER_MULTI_INSTANCE');
const deepLinkFromArgv = process.argv.find((arg) => isSupportedDeepLinkUrl(arg));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[DeepOrganiser] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    // Prefer additionalData (reliable on all platforms), fallback to argv scan
    const deepLinkUrl =
      (additionalData as { deepLinkUrl?: string })?.deepLinkUrl || argv.find((arg) => isSupportedDeepLinkUrl(arg));
    if (deepLinkUrl) {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        ...
        createWindow: () => {
          console.log(
            '[DeepOrganiser] second-instance received with no active main window, recreating main window'
          );
          createWindow();
        },
      });
...
let disableDeprecatedLocalRuntimePromise: Promise<void> | null = null;
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      method: 'POST',
      headers: {
        'x-deeporganiser-internal': '1',
      },
    }).catch((error) => {
      console.error('[DeepOrganiser] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[DeepOrganiser] runBackendMigrations completed');
    } catch (error) {
      console.error('[DeepOrganiser] Backend migration hook threw:', error);
    }
  })();
};
...
function disableDeprecatedLocalRuntimeOnce(backendPort: number): Promise<void> {
  if (!disableDeprecatedLocalRuntimePromise) {
    disableDeprecatedLocalRuntimePromise = (async () => {
      try {
        const { disableDeprecatedLocalRuntime } = await import('./process/utils/disableDeprecatedLocalRuntime');
        await disableDeprecatedLocalRuntime(backendPort);
      } catch (err) {
        console.warn('[DeepOrganiser] disableDeprecatedLocalRuntime failed:', err);
      }
    })();
  }
  return disableDeprecatedLocalRuntimePromise;
}

function markBackendReady(backendPort: number, source: string): void {
  if (backendStartedOk) return;
  console.log(`[DeepOrganiser] ${source} ready (port=${backendPort})`);
  ...
  void disableDeprecatedLocalRuntimeOnce(backendPort);
  scheduleBackendMigrations();
}

function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  const reason = (process.env.DEEPORGANISER_DEBUG_BACKEND_STARTUP_FAILURE ??
    process.env[legacyEnvName('DEBUG_BACKEND_STARTUP_FAILURE')]) as BackendStartupFailureInfo['reason'] | undefined;
  if (!reason) {
    return null;
  }
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[DeepOrganiser] Ignoring DEEPORGANISER_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[DeepOrganiser] Ignoring unknown DEEPORGANISER_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[DeepOrganiser] Creating main window...');
  ...
  console.log(`[DeepOrganiser] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[DeepOrganiser] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[DeepOrganiser] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[DeepOrganiser] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater = isEnvEnabled('DEEPORGANISER_DISABLE_AUTO_UPDATE') || isE2ETestMode || isCiRuntime;
  if (!disableAutoUpdater) {
    ...
    console.log('[DeepOrganiser] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[DeepOrganiser] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[DeepOrganiser] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[DeepOrganiser] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[DeepOrganiser] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[DeepOrganiser] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[DeepOrganiser] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[DeepOrganiser] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[DeepOrganiser] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[DeepOrganiser] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[DeepOrganiser] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[DeepOrganiser] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[DeepOrganiser] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  if (debugBackendStartupFailure) {
    ...
    // Start DeepOrganiser Core only after initializeProcess(). initStorage may open
  ...
  try {
    ...
    console.error('[DeepOrganiser] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[DeepOrganiser] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`OpenBioScience --webui` or packaged `openbioscience-web` mode that
      ...
      // M6: Switch to @deeporganiser/web-host
      const handle = await startWebHost({
        ...
        // Expose the same backend system-dir env the desktop IPC path
        ...
        backend: {
          ...
          port: (() => {
            ...
            if (!port) {
              throw new Error('[WebUI] Cannot start: DeepOrganiser Core is not running (globalThis.__backendPort unset)');
            }
            return port;
          })(),
        },
      ...
      void (async () => {
        try {
          ...
          if (petEnabled === true) {
            ...
            const petStyle = (await ProcessConfig.get('pet.style')) ?? 'deepscientist';
            const petPersonality = (await ProcessConfig.get('pet.personality')) ?? 'balanced';
            const { createPetWindow, setPetConfirmEnabled, setPetPersonality, setPetStyle } =
              await import('./process/pet/petManager');
            setPetConfirmEnabled(confirmEnabled);
            setPetStyle(petStyle);
            setPetPersonality(petPersonality);
            createPetWindow();
          }
        } catch (error) {
...
// Register the DeepOrganiser deep-link scheme as the default protocol client
...
// macOS: handle DeepOrganiser URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[DeepOrganiser] App initialization failed:', error);
    app.quit();
  });
...
installQuitCleanup({
  ...
  // Stop DeepOrganiser Core subprocess — backend shutdown kills all agent children
...
app.on('will-quit', () => {
  console.log('[DeepOrganiser] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[DeepOrganiser] quit (exitCode=${exitCode})`);
});
