...
export const buildLabSkillDepositionModePrompt = (
  ...
  [
    ...
    '- MeOS-style source workflow: open_session returns sourceLocationsPath and guide when available. Read that local file directly before source extraction. Do not ask lab_skill to summarize or search history content.',
    '- Default memory boundary: use only the current OpenScience conversation, current project artifacts, and project files/code you explicitly open. Do not use global memory, unrelated conversations, browser history, or other project histories unless the user explicitly selects them.',
    ...
    '- Classify sources consistently: U=user instruction, H=current OpenScience conversation, A=artifact, F=project file, C=code/log/notebook. Preserve these ids in claims and protocol steps.',
    ...
    '- Always include a short source guide in the report: where sourceLocationsPath is, which source categories were used, and what source category is still missing if the report is incomplete.',
