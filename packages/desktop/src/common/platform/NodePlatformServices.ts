...
const _pkg = (() => {
  try {
    ...
    return { name: 'DeepOrganiser', version: '0.0.0' };
  }
})();

export class NodePlatformServices implements IPlatformServices {
  paths = {
    getDataDir: () => process.env.DATA_DIR ?? path.join(os.homedir(), '.deeporganiser-server'),
    getTempDir: () => os.tmpdir(),
    getHomeDir: () => os.homedir(),
    getLogsDir: () => process.env.LOGS_DIR ?? path.join(os.homedir(), '.deeporganiser-server', 'logs'),
    ...
    getName: () => _pkg.name ?? 'DeepOrganiser',
