...
export function fromApiConversation<T>(raw: T): T {
  ...
  const isLegacyLocalRuntimeConversation = r.type === LEGACY_LOCAL_RUNTIME_ID;

  if (isLegacyLocalRuntimeConversation) {
    next.type = 'acp';
  }

  if ('model' in r && !isLegacyLocalRuntimeConversation) {
