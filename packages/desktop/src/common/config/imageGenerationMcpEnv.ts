...
import { legacyEnvName } from './legacyIdentifiers';
import type { IProvider } from './storage';

export const IMAGE_GEN_ENV_KEYS = {
  providerId: 'DEEPORGANISER_IMG_PROVIDER_ID',
  platform: 'DEEPORGANISER_IMG_PLATFORM',
  baseUrl: 'DEEPORGANISER_IMG_BASE_URL',
  apiKey: 'DEEPORGANISER_IMG_API_KEY',
  model: 'DEEPORGANISER_IMG_MODEL',
} as const;

const LEGACY_IMAGE_GEN_ENV_KEYS = {
  providerId: legacyEnvName('IMG_PROVIDER_ID'),
  platform: legacyEnvName('IMG_PLATFORM'),
  baseUrl: legacyEnvName('IMG_BASE_URL'),
  apiKey: legacyEnvName('IMG_API_KEY'),
  model: legacyEnvName('IMG_MODEL'),
} as const;
...
function getLegacyField(
  ...
  return {
    providerId: selection?.id || existingEnv?.[IMAGE_GEN_ENV_KEYS.providerId] || existingEnv?.[LEGACY_IMAGE_GEN_ENV_KEYS.providerId],
    platform: selection?.platform || existingEnv?.[IMAGE_GEN_ENV_KEYS.platform] || existingEnv?.[LEGACY_IMAGE_GEN_ENV_KEYS.platform],
    baseUrl: selection?.base_url || existingEnv?.[IMAGE_GEN_ENV_KEYS.baseUrl] || existingEnv?.[LEGACY_IMAGE_GEN_ENV_KEYS.baseUrl],
    model: selection?.use_model || existingEnv?.[IMAGE_GEN_ENV_KEYS.model] || existingEnv?.[LEGACY_IMAGE_GEN_ENV_KEYS.model],
  };
}
...
export function removeImageGenerationEnvKeys(env: Record<string, string>): Record<string, string> {
  const next = { ...env };
  [...Object.values(IMAGE_GEN_ENV_KEYS), ...Object.values(LEGACY_IMAGE_GEN_ENV_KEYS)].forEach((key) => {
    delete next[key];
  });
  return next;
}
