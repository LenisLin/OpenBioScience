...
export interface IConfigStorageRefer {
  ...
  // Skills Market: whether the built-in skills builtin skill is enabled
...
export interface IEnvStorageRefer {
  [key: `${typeof LEGACY_APP_NAMESPACE}.dir`]: {
    workDir: string;
    cacheDir: string;
    logDir?: string;
  };
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'app' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 app / Conversation source, defaults to app */
...
export type TChatConversation =
  ...
  // DeepOrganiser Core backend enums and factory.
...
export interface MedicalEvidenceConfig {
  enabled?: boolean;
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by DeepOrganiser (hide edit/delete in UI) */
  builtin?: boolean;
}
