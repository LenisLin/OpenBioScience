...
export function getConversationTypeForBackend(_backend: string): ICreateConversationParams['type'] {
  return 'acp';
}
...
export function buildAgentConversationParams(input: BuildAgentConversationInput): ICreateConversationParams {
  ...
  const effectiveBackend = backend === LEGACY_LOCAL_RUNTIME_ID ? 'codex' : backend;
  const requestedPresetType = preset_agent_type || effectiveBackend;
  const effectivePresetType = requestedPresetType === LEGACY_LOCAL_RUNTIME_ID ? 'codex' : requestedPresetType;
  const effectivePresetAssistantId = preset_assistant_id || custom_agent_id;
  const type = getConversationTypeForBackend(is_preset ? effectivePresetType : effectiveBackend);
  ...
  if (is_preset) {
    ...
    extra.backend = effectiveBackend as string;
