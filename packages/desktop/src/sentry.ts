...
function isBackendStartupFailureEvent(event: { tags?: Record<string, unknown> }): boolean {
  return event.tags?.['deeporganiser.failure'] === 'backend_startup';
}

function isUserFeedbackEvent(event: { tags?: Record<string, unknown> }): boolean {
  return (
    event.tags?.type === 'user-feedback' ||
    event.tags?.['deeporganiser.installation_integrity.user_report'] === 'true'
  );
}
...
function getInstallPathKind(resourcesPath: unknown): string | undefined {
  ...
  const currentResourcesPath = '\\appdata\\local\\programs\\deeporganiser\\resources';
  const currentProgramFilesPath = '\\program files\\deeporganiser\\resources';
  const currentProgramFilesX86Path = '\\program files (x86)\\deeporganiser\\resources';
  const legacyResourcesPath = `\\appdata\\local\\programs\\${LEGACY_APP_NAMESPACE}\\resources`;
  const legacyProgramFilesPath = `\\program files\\${LEGACY_APP_NAMESPACE}\\resources`;
  const legacyProgramFilesX86Path = `\\program files (x86)\\${LEGACY_APP_NAMESPACE}\\resources`;
  if (normalized.includes(currentResourcesPath) || normalized.includes(legacyResourcesPath)) {
    return 'user_local_programs';
  }
  if (
    normalized.includes(currentProgramFilesPath) ||
    normalized.includes(currentProgramFilesX86Path) ||
    normalized.includes(legacyProgramFilesPath) ||
    normalized.includes(legacyProgramFilesX86Path)
  ) {
...
export async function captureBackendStartupFailure(error: unknown): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.setTag('deeporganiser.failure', 'backend_startup');
    scope.setTag('deeporganiser.backend_startup.reason', failureInfo.reason);
    if (failureInfo.runtime) {
      scope.setTag('deeporganiser.backend_startup.runtime', failureInfo.runtime);
    }
    if (failureInfo.packageArch) {
      scope.setTag('deeporganiser.backend_startup.package_arch', failureInfo.packageArch);
    }
    if (failureInfo.deviceArch) {
      scope.setTag('deeporganiser.backend_startup.device_arch', failureInfo.deviceArch);
    }
    if (failureInfo.expectedDownloadArch) {
      scope.setTag('deeporganiser.backend_startup.expected_download_arch', failureInfo.expectedDownloadArch);
    }
    if (typeof failureInfo.isRosettaTranslated === 'boolean') {
      scope.setTag('deeporganiser.backend_startup.rosetta_translated', getBooleanTagValue(failureInfo.isRosettaTranslated));
    }
    if (typeof details?.stage === 'string') {
      scope.setTag('deeporganiser.backend_startup.stage', details.stage);
    }
    if (failureInfo.incompleteInstallationKind) {
      scope.setTag('deeporganiser.backend_startup.incomplete_installation_kind', failureInfo.incompleteInstallationKind);
    }
    for (const [tag, value] of [
      [
        'deeporganiser.backend_startup.missing_bundled_dir',
        getBooleanTagValue(failureInfo.missingBundledDeepOrganiserCoreDir),
      ],
      ['deeporganiser.backend_startup.missing_runtime_dir', getBooleanTagValue(failureInfo.missingRuntimeDir)],
      ['deeporganiser.backend_startup.missing_binary', getBooleanTagValue(failureInfo.missingBackendBinary)],
      ['deeporganiser.backend_startup.missing_hub_dir', getBooleanTagValue(failureInfo.missingHubDir)],
      ['deeporganiser.backend_startup.missing_pet_states_dir', getBooleanTagValue(failureInfo.missingPetStatesDir)],
      ['deeporganiser.backend_startup.missing_pwa_dir', getBooleanTagValue(failureInfo.missingPwaDir)],
      ['deeporganiser.backend_startup.install_path_kind', getInstallPathKind(details?.resourcesPath)],
      ['deeporganiser.backend_startup.last_update_status', getString(autoUpdateDiagnostics?.lastEvent?.status)],
      [
        'deeporganiser.backend_startup.health_polling_delayed',
        getBooleanTagValue(
          typeof details?.healthCheckPollingDelayed === 'boolean' ? details.healthCheckPollingDelayed : undefined
        ),
      ],
      ['deeporganiser.backend_startup.health_attempts_bucket', getHealthAttemptBucket(details?.healthCheckAttempts)],
      [
        'deeporganiser.backend_startup.health_attempt_deficit_bucket',
        getHealthAttemptBucket(details?.healthCheckAttemptDeficit),
      ],
      [
        'deeporganiser.backend_startup.health_timeout_overrun_bucket',
        getDurationBucket(details?.healthCheckTimeoutOverrunMs),
      ],
      [
        'deeporganiser.backend_startup.health_max_attempt_gap_bucket',
        getDurationBucket(details?.healthCheckMaxAttemptGapMs),
      ],
      [
        'deeporganiser.backend_startup.seconds_since_quit_and_install',
    ...
    if (details) {
      scope.setContext('deeporganiser_core_startup', details);
      scope.setExtra('deeporganiser_core_startup', details);
    }
    scope.setContext('deeporganiser_core_startup_classification', { ...failureInfo });
    scope.setExtra('deeporganiser_core_startup_classification', failureInfo);
    scope.setContext('deeporganiser_core_install_diagnostics', installDiagnostics);
    scope.setExtra('deeporganiser_core_install_diagnostics', installDiagnostics);
...
async function runStartupLogReport(): Promise<void> {
  ...
  Sentry.withScope((scope) => {
    scope.addAttachment({
      filename: 'deeporganiser-logs.log.gz',
      data: pack.gzipped,
