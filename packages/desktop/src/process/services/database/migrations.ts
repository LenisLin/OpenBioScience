...
const migration_v8: IMigration = {
  ...
  up: (db) => {
    ...
    if (!columns.has('source')) {
      db.exec(`ALTER TABLE conversations ADD COLUMN source TEXT CHECK(source IN ('${LEGACY_APP_NAMESPACE}', 'telegram'))`);
...
const migration_v11: IMigration = {
  ...
  up: (db) => {
    ...
    db.exec(`UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('${LEGACY_APP_NAMESPACE}', 'telegram')`);
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        source TEXT CHECK(source IS NULL OR source IN ('${LEGACY_APP_NAMESPACE}', 'telegram')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
...
const migration_v12: IMigration = {
  ...
  up: (db) => {
    ...
    // Clean up any invalid source values before copying
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('${LEGACY_APP_NAMESPACE}', 'telegram', 'lark')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('${LEGACY_APP_NAMESPACE}', 'telegram', 'lark')),
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
      )`);
...
const migration_v14: IMigration = {
  ...
  up: (db) => {
    ...
    // so DROP TABLE will NOT trigger ON DELETE CASCADE on the messages table.
    db.exec(
      `UPDATE conversations SET source = NULL WHERE source IS NOT NULL AND source NOT IN ('${LEGACY_APP_NAMESPACE}', 'telegram', 'lark', 'dingtalk')`
    );
    ...
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('${LEGACY_APP_NAMESPACE}', 'telegram', 'lark', 'dingtalk')),
...
const migration_v20: IMigration = {
  ...
  up: (db) => {
    ...
    try {
      db.exec(`ALTER TABLE teams ADD COLUMN lead_agent_id TEXT NOT NULL DEFAULT '${LEGACY_APP_NAMESPACE}'`);
    } catch {
...
/**
 * Migration v20 -> v21: Add the deprecated local runtime to conversations type CHECK constraint
 */
const migration_v21: IMigration = {
  version: 21,
  name: 'Add deprecated local runtime to conversations type CHECK',
  up: (db) => {
    db.exec(`CREATE TABLE IF NOT EXISTS conversations_new (
        id TEXT PRIMARY KEY,
        user_id TEXT NOT NULL,
        name TEXT NOT NULL,
        type TEXT NOT NULL CHECK(type IN ('gemini', 'acp', 'codex', 'openclaw-gateway', 'nanobot', 'remote', '${LEGACY_LOCAL_RUNTIME_ID}')),
        extra TEXT NOT NULL,
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT,
    ...
    console.log('[Migration v21] Added deprecated local runtime to conversations type CHECK');
  },
  down: (db) => {
    // Remove deprecated local runtime conversations before copying to table with stricter constraint.
    db.prepare('DELETE FROM messages WHERE conversation_id IN (SELECT id FROM conversations WHERE type = ?)').run(
      LEGACY_LOCAL_RUNTIME_ID
    );
    db.prepare('DELETE FROM conversations WHERE type = ?').run(LEGACY_LOCAL_RUNTIME_ID);
    ...
    console.log('[Migration v21] Rolled back: Removed deprecated local runtime from conversations type CHECK');
  },
};
...
const migration_v22: IMigration = {
  ...
  up: (db) => {
    ...
    db.exec(
      `UPDATE cron_jobs SET execution_mode = 'new_conversation' WHERE conversation_id = '${LEGACY_APP_NAMESPACE}' OR conversation_id IS NULL`
    );
