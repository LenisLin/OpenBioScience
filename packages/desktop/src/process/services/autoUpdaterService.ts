...
const FORCE_DEV_AUTO_UPDATE_ENV = 'DEEPORGANISER_FORCE_DEV_AUTO_UPDATE';
const DEBUG_AUTO_UPDATE_CURRENT_VERSION_ENV = 'DEEPORGANISER_DEBUG_AUTO_UPDATE_CURRENT_VERSION';
const envValue = (name: string): string | undefined => process.env[name] ?? process.env[legacyEnvName(name.replace('DEEPORGANISER_', ''))];
...
class AutoUpdaterService extends EventEmitter {
  ...
  private configureDevAutoUpdateDebug(): void {
    if (app.isPackaged || envValue(FORCE_DEV_AUTO_UPDATE_ENV) !== '1') {
      return;
    }
    ...
    const debugCurrentVersion = envValue(DEBUG_AUTO_UPDATE_CURRENT_VERSION_ENV);
  ...
  private ensureDevUpdateConfig(): void {
    try {
      ...
      const devConfig = [
        'provider: generic',
        `url: ${cdnFeedOptions.url}`,
        'updaterCacheDirName: cc.deepscientist.pro',
        '',
      ].join('\n');
   ...
   * while the downloaded archive contains the packaged app (cc.deepscientist.pro). Squirrel.Mac
