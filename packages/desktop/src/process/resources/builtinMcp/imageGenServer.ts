...
function getProviderFromEnv(): TProviderWithModel | null {
  const platform = process.env[IMAGE_GEN_ENV_KEYS.platform] || process.env[legacyEnvName('IMG_PLATFORM')];
  const base_url = process.env[IMAGE_GEN_ENV_KEYS.baseUrl] || process.env[legacyEnvName('IMG_BASE_URL')];
  const api_key = process.env[IMAGE_GEN_ENV_KEYS.apiKey] || process.env[legacyEnvName('IMG_API_KEY')];
  const model = process.env[IMAGE_GEN_ENV_KEYS.model] || process.env[legacyEnvName('IMG_MODEL')];
...
async function main() {
  ...
  server.tool(
    'deeporganiser_image_generation',
...
IMPORTANT: When user provides multiple images, ALWAYS pass ALL images to the image_uris parameter as an array.`,
    ...
    async ({ prompt, image_uris, workspace_dir }) => {
      ...
      const proxy = process.env.DEEPORGANISER_IMG_PROXY || process.env[legacyEnvName('IMG_PROXY')] || undefined;
