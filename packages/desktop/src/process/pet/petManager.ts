...
import type { PetPersonality, PetSize, PetState, PetStyle, PetTransitionLogEntry } from './petTypes';
...
let currentPersonality: PetPersonality = 'balanced';
...
export function createPetWindow(): void {
  ...
  idleTicker.setPersonality(currentPersonality);
  ...
  petWindow.webContents.once('did-finish-load', () => {
    ...
    petWindow.webContents.send('pet:style-changed', currentStyle);
    petWindow.webContents.send('pet:state-changed', stateMachine?.getCurrentState() ?? 'idle');
  });
...
export function setPetStyle(style: PetStyle): void {
  currentStyle = style;
  if (petWindow && !petWindow.isDestroyed()) {
    petWindow.webContents.send('pet:style-changed', style);
    petWindow.webContents.send('pet:state-changed', stateMachine?.getCurrentState() ?? 'idle');
  }
}
...
export function setPetPersonality(personality: PetPersonality): void {
  currentPersonality = personality;
  idleTicker?.setPersonality(personality);
}

export function getPetTransitionLogs(): PetTransitionLogEntry[] {
  return stateMachine?.getTransitionLog() ?? [];
}
 ...
 * currently hosts the main DeepOrganiser window. Falls back to the primary display
...
function registerIpcHandlers(): void {
  ipcMain.on('pet:drag-start', () => {
    ...
    stateMachine?.forceState('dragging', 'force', 'drag start');
...
function endDrag(): void {
  ...
  preDragState = null;
  stateMachine?.forceState(restoreTo, 'force', 'drag end');
  idleTicker?.resetIdle();
  if (petWindow && !petWindow.isDestroyed()) {
