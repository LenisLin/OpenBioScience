...
export type PetStyle = 'deepscientist' | 'classic' | 'paperfold' | 'observatory';

export type PetPersonality = 'calm' | 'balanced' | 'lively';

export type PetTransitionLogReason =
  | 'request-applied'
  | 'request-pending'
  | 'request-rejected'
  | 'pending-applied'
...
export type PetTransitionLogEntry = {
  id: number;
  at: number;
  from: PetState;
  to?: PetState;
