...
import type { PetPersonality, PetSize, PetStyle } from '@process/pet/petTypes';
...
export function initSystemSettingsBridge(): void {
  ...
  ipcBridge.systemSettings.getPetStyle.provider(async () => {
    const value = await ProcessConfig.get('pet.style');
    return value ?? 'deepscientist';
  });

  ipcBridge.systemSettings.setPetStyle.provider(async ({ style }) => {
    await ProcessConfig.set('pet.style', style);
    const { setPetStyle } = await import('@process/pet/petManager');
    setPetStyle(style as PetStyle);
  });

  ipcBridge.systemSettings.getPetPersonality.provider(async () => {
    const value = await ProcessConfig.get('pet.personality');
    return value ?? 'balanced';
  });

  ipcBridge.systemSettings.setPetPersonality.provider(async ({ personality }) => {
    await ProcessConfig.set('pet.personality', personality);
    const { setPetPersonality } = await import('@process/pet/petManager');
    setPetPersonality(personality as PetPersonality);
  });
  ...
  ipcBridge.systemSettings.getPetTransitionLogs.provider(async () => {
    const { getPetTransitionLogs } = await import('@process/pet/petManager');
    return getPetTransitionLogs();
  });
}
