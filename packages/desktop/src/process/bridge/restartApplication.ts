...
export function restartApplication(app: RestartableApp): IAppRestartResult {
  if (!app.isPackaged) {
    console.info('[DeepOrganiser] Restart skipped in development mode; manual restart required');
    return {
      restarted: false,
  ...
  console.info('[DeepOrganiser] Relaunching application to apply changes');
  app.relaunch();
  app.exit(0);
  return {
    restarted: true,
