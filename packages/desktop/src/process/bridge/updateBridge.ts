...
const CDN_HOST = CDN_URL.hostname;
const CDN_ORIGIN = CDN_URL.origin;
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = (process.env.DEEPORGANISER_GITHUB_REPO ?? process.env[legacyEnvName('GITHUB_REPO')])?.trim();
...
const assertAllowedUrl = async (rawUrl: string) => {
  ...
  const isConfiguredHttpUpdateOrigin = parsed.protocol === 'http:' && parsed.origin === CDN_ORIGIN;
  if (parsed.protocol !== 'https:' && !isConfiguredHttpUpdateOrigin) {
