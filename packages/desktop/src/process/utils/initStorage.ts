...
const STORAGE_PATH = {
  config: `${LEGACY_APP_NAMESPACE}-config.txt`,
  chatMessage: `${LEGACY_APP_NAMESPACE}-chat-message.txt`,
  chat: `${LEGACY_APP_NAMESPACE}-chat.txt`,
  env: `.${LEGACY_APP_NAMESPACE}-env`,
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[DeepOrganiser] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[DeepOrganiser] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[DeepOrganiser] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync(`${LEGACY_APP_NAMESPACE}.dir`);
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const chatHistoryDir = `${LEGACY_APP_NAMESPACE}-chat-history`;
  const fullName = path.join(dir, chatHistoryDir, conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, chatHistoryDir));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, chatHistoryDir, conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, `${LEGACY_APP_NAMESPACE}-chat-history`, 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[DeepOrganiser] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
 ...
 * the backend bundled assistant and skill assets) — neither is synced from
...
const initStorage = async () => {
  ...
  const mark = (label: string) =>
    console.log(`[DeepOrganiser:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  //    now live in the backend SQLite catalog (see backend assistant service) and
  ...
  try {
    ...
    console.error('[DeepOrganiser] Failed to ensure assistant dirs:', error);
  }
