...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'deeporganiser');
};

const APP_DATA_DIR_NAME = 'deeporganiser';
const APP_DATA_SYMLINK_NAME = '.deeporganiser';
const APP_CONFIG_SYMLINK_NAME = '.deeporganiser-config';

const migrateLegacyDataPathIfNeeded = (targetPath: string, legacyTargetPath: string): void => {
  if (targetPath === legacyTargetPath || existsSync(targetPath) || !existsSync(legacyTargetPath)) {
    return;
  }

  try {
    mkdirSync(path.dirname(targetPath), { recursive: true });
    renameSync(legacyTargetPath, targetPath);
  } catch {
    // If another process creates the new directory first, leave both paths intact.
    // The caller will continue with the current target path.
  }
};
 ...
 * Release builds use a stable hidden data symlink; dev builds append a dev suffix.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用稳定隐藏数据符号链接，Dev 模式附加 dev 后缀。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, APP_DATA_DIR_NAME);
  migrateLegacyDataPathIfNeeded(dataPath, path.join(rootPath, LEGACY_APP_NAMESPACE));
 ...
 * Release builds use a stable hidden config symlink; dev builds append a dev suffix.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用稳定隐藏配置符号链接，Dev 模式附加 dev 后缀。
 */
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[DeepOrganiser] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[DeepOrganiser] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[DeepOrganiser] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[DeepOrganiser] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
