 ...
 * every assistant (built-in and user). Treat the legacy default as "no explicit
 * choice" and promote it to the current default, so users who never touched the
 * agent picker don't find all their assistants pointing at a backend that is no
 * longer there on boot.
...
const CURRENT_DEFAULT_PRESET_AGENT_TYPE = 'codex';
 ...
 * backend manifest at
 * the backend built-in assistant manifest
...
async function backendSupportsAssistantDefinitions(): Promise<boolean> {
  try {
    ...
    console.warn('[DeepOrganiser] Failed to probe unified assistant detail support:', error);
    return false;
  }
}
...
async function markAssistantsMigrationDone(configFile: ConfigFile): Promise<void> {
  ...
  try {
    ...
    console.warn('[DeepOrganiser] failed to persist assistants migration flag', err);
  }
}
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[DeepOrganiser] Skipped override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[DeepOrganiser] Failed to apply builtin override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(
      `[DeepOrganiser] Applied ${applied} builtin disabled-state override(s) (skipped ${skipped} retired id(s))`
    );
  } else {
    console.error(
      `[DeepOrganiser] Builtin override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
 ...
 * whatever manifest the running backend ships (e.g. current is `codex`, but
 ...
 * on built-in rows. 404 is treated as
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      ...
      if (isBackendHttpError(reason) && reason.status === 404) {
        skipped += 1;
        console.warn(
          `[DeepOrganiser] Skipped preset_agent_type override for retired built-in '${overrides[i].id}' (no longer in backend manifest)`
        );
        return;
      }
      failed += 1;
      console.error(`[DeepOrganiser] Failed to apply preset_agent_type override for ${overrides[i].id}:`, reason);
    }
  });
  const applied = overrides.length - failed - skipped;
  if (failed === 0) {
    console.log(
      `[DeepOrganiser] Applied ${applied} builtin preset_agent_type override(s) (skipped ${skipped} retired id(s))`
    );
  } else {
    console.error(
      `[DeepOrganiser] Builtin preset_agent_type override partial: ${failed}/${overrides.length} failed, ${skipped} skipped, ${applied} applied`
    );
  }
  return failed;
}
...
async function fetchCurrentBuiltinAgentTypes(): Promise<Map<string, string>> {
  try {
    ...
    console.error('[DeepOrganiser] Failed to fetch current builtin preset_agent_type map:', error);
    return new Map();
  }
}
...
async function uploadLegacyAssistantRules(legacyAssistantIds: Set<string>): Promise<number> {
  ...
  try {
    ...
    console.error('[DeepOrganiser] Failed to read legacy assistant rules dir:', error);
    return 1;
  }
  ...
  results.forEach((r, i) => {
    if (r.status === 'rejected') {
      failed += 1;
      console.error(
        `[DeepOrganiser] Failed to upload legacy rule for '${ruleEntries[i].id}' (${ruleEntries[i].locale}):`,
        r.reason
      );
      return;
    }
  ...
  if (failed === 0) {
    if (uploaded > 0 || skipped > 0) {
      console.log(`[DeepOrganiser] Legacy rule upload: ${uploaded} uploaded, ${skipped} skipped`);
    }
  } else {
    console.error(`[DeepOrganiser] Legacy rule upload partial: ${failed}/${ruleEntries.length} failed`);
  }
  return failed;
}
 ...
 *      across the 'gemini' → current-default migration.
 ...
 *
 * Honors `DEEPORGANISER_SKIP_ELECTRON_MIGRATION=1` so E2E fixtures can seed via
...
export async function migrateAssistantsToBackend(configFile: ConfigFile): Promise<boolean> {
  if (
    process.env.DEEPORGANISER_SKIP_ELECTRON_MIGRATION === '1' ||
    process.env[legacyEnvName('SKIP_ELECTRON_MIGRATION')] === '1'
  ) {
    console.log('[DeepOrganiser] Assistant migration skipped (env flag set)');
    return false;
  }
  ...
  if (userAssistants.length > 0) {
    try {
      ...
      if (result.failed !== 0) {
        console.error(`[DeepOrganiser] Assistant migration partial: ${result.failed} failed`, result.errors);
        return false;
      }
      if (result.imported > 0 || result.skipped > 0) {
        console.log(`[DeepOrganiser] migrated ${result.imported} assistants (skipped ${result.skipped})`);
      }
    } catch (error) {
      console.error('[DeepOrganiser] Assistant migration failed:', error);
      return false;
    }
  }
