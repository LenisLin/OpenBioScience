...
// Override via DEEPORGANISER_CDP_PORT env variable. Set to "0" to disable.
//
...
// Registry file: ~/.deeporganiser-cdp-registry.json
// ---------------------------------------------------------------------------

export const DEFAULT_CDP_PORT = 9230;
export const CDP_PORT_RANGE_START = 9230;
export const CDP_PORT_RANGE_END = 9250;
const CDP_REGISTRY_FILE = path.join(os.homedir(), '.deeporganiser-cdp-registry.json');
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another DeepOrganiser instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active DeepOrganiser instances, trying ${preferredPort}`
  );
  return preferredPort;
}
...
function resolveCdpPortFromEnv(): number | null | undefined {
  const envVal = process.env.DEEPORGANISER_CDP_PORT ?? process.env[legacyEnvName('CDP_PORT')];
