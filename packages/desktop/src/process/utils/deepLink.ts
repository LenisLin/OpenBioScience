...
export const PROTOCOL_SCHEME = 'deeporganiser';

/**
 * Parse a DeepOrganiser deep link URL into action and params.
 * Supports two formats:
 *   1. deeporganiser://add-provider?base_url=xxx&api_key=xxx
 *   2. deeporganiser://provider/add?v=1&data=<base64 JSON>  (one-api / new-api style)
 */
...
export const isSupportedDeepLinkUrl = (url: string): boolean => parseDeepLinkUrl(url) !== null;

let mainWindowRef: BrowserWindow | null = null;
let pendingDeepLinkUrl: string | null =
  process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`)) || null;
