...
import { startWebHost, type WebHostHandle } from '@deeporganiser/web-host';
...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.DEEPORGANISER_PORT ?? process.env[legacyEnvName('PORT')] ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(
    process.env.DEEPORGANISER_ALLOW_REMOTE ||
      process.env.DEEPORGANISER_REMOTE ||
      process.env[legacyEnvName('ALLOW_REMOTE')] ||
      process.env[legacyEnvName('REMOTE')]
  );
  const hostHint = (process.env.DEEPORGANISER_HOST ?? process.env[legacyEnvName('HOST')])?.trim();
...
export async function startDesktopWebUI(opts: { port?: number; allowRemote?: boolean }): Promise<DesktopWebUIHandle> {
  ...
  const handle = await startWebHost({
    app: {
      ...
      // getDataPath() returns the CLI-safe data symlink on macOS to sidestep
