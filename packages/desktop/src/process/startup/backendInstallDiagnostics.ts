...
export function collectBackendInstallDiagnostics(
  ...
  const bundledDirPath = resourcesPath ? pathApi.join(resourcesPath, BUNDLED_DEEPORGANISER_CORE_DIR) : undefined;
  const runtimeDirPath =
    resourcesPath && runtimeKey ? pathApi.join(resourcesPath, BUNDLED_DEEPORGANISER_CORE_DIR, runtimeKey) : undefined;
  const binaryPath =
