...
const BUNDLED_DEEPORGANISER_CORE_DIR = 'bundled-deeporganiser-core';
const BUNDLED_DEEPORGANISER_CORE_ENTRY = `${BUNDLED_DEEPORGANISER_CORE_DIR}/`;
...
function classifyIncompleteInstallation(details: ErrorWithDetails['details']): BackendStartupFailureInfo | undefined {
  ...
  const missingBundledDeepOrganiserCoreDir = !resourcesDirEntries.includes(BUNDLED_DEEPORGANISER_CORE_ENTRY);
  const missingRuntimeDir = details.runtimeDirExists === false && typeof details.runtimeKey === 'string';
  const missingResources = missingBundledDeepOrganiserCoreDir ? [BUNDLED_DEEPORGANISER_CORE_ENTRY] : [];
  if (details.runtimeDirExists === false && typeof details.runtimeKey === 'string') {
    missingResources.push(`${BUNDLED_DEEPORGANISER_CORE_DIR}/${details.runtimeKey}/`);
  }
  ...
  if (missingManagedResourcesDir && typeof details.runtimeKey === 'string') {
    missingResources.push(`${BUNDLED_DEEPORGANISER_CORE_DIR}/${details.runtimeKey}/managed-resources/`);
  }
  ...
  if (missingRuntimeBinary && typeof details.runtimeKey === 'string' && typeof details.binaryName === 'string') {
    missingResources.push(`${BUNDLED_DEEPORGANISER_CORE_DIR}/${details.runtimeKey}/${details.binaryName}`);
  }
  ...
  return {
    incompleteInstallationKind:
      missingBundledDeepOrganiserCoreDir || missingRuntimeDir || missingManagedResourcesDir
        ? 'missing_directory_resources'
        : 'missing_backend_binary',
    missingBackendBinary:
      missingBundledDeepOrganiserCoreDir || missingRuntimeDir || missingManagedResourcesDir || missingRuntimeBinary,
    missingBundledDeepOrganiserCoreDir,
