/**
 * Lifecycle manager for the DeepOrganiser Core subprocess (web-host version).
 *
...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via DEEPORGANISER_{CACHE,WORK,LOG}_DIR env.
...
export class BackendStartupCancelledError extends Error {
  constructor(message = 'DeepOrganiser Core startup cancelled') {
    super(message);
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = readEnv('LOG_LEVEL') || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
...
/**
 * Backend reads DEEPORGANISER_{CACHE,WORK,LOG}_DIR env vars to report system dirs
 * (with legacy aliases for older cores). Inject them so the
...
export function buildSpawnEnv(dirs: BackendDirConfig): NodeJS.ProcessEnv {
  return {
    ...process.env,
    PATH: buildBackendPath(process.env),
    DEEPORGANISER_CACHE_DIR: dirs.cacheDir,
    DEEPORGANISER_WORK_DIR: dirs.workDir,
    DEEPORGANISER_LOG_DIR: dirs.logDir,
    [legacyEnvName('CACHE_DIR')]: dirs.cacheDir,
    [legacyEnvName('WORK_DIR')]: dirs.workDir,
    [legacyEnvName('LOG_DIR')]: dirs.logDir,
  };
}
...
function dedupePathEntries(entries: string[], caseInsensitive: boolean): string[] {
...
export function buildBackendPath(env: NodeJS.ProcessEnv = process.env): string {
...
export function findAvailablePort(
  ...
  if (preferredPort && firstRequestedPort === 0) {
    console.info(`[deeporganiser-core] skipped fetch-blocked backend port ${preferredPort}`);
  }
      ...
      server.listen(requestedPort, '127.0.0.1', () => {
        ...
        server.close(() => {
          ...
          if (resolvedPort > 0 && !isFetchForbiddenPort(resolvedPort)) {
            console.info(`[deeporganiser-core] selected backend port ${resolvedPort} after ${attempt} attempts`);
            resolve(resolvedPort);
            return;
          }
          if (resolvedPort > 0 && remainingAttempts > 1) {
            console.info(`[deeporganiser-core] skipped fetch-blocked backend port ${resolvedPort}`);
...
function parseDeepOrganiserCoreListeningPort(line: string): number | undefined {
...
export class BackendLifecycleManager {
  ...
  async start(
    ...
    try {
      ...
      throw new BackendStartupError(
        'DeepOrganiser Core startup failed while resolving backend binary',
        {
    ...
    console.log(`[deeporganiser-core] starting: ${binaryPath} ${args.join(' ')}`);
    ...
    try {
      ...
      this._status = 'error';
      throw makeStartupError('spawn', 'DeepOrganiser Core process spawn threw before startup', error);
    }
    ...
    const startupFailure = new Promise<never>((_resolve, reject) => {
      ...
      this.childProcess?.once('close', (code, signal) => {
        ...
        if (!pendingStartupExit.startupSettledAtExit) {
          ...
          rejectOnce(
            makeStartupError('early_exit', 'DeepOrganiser Core exited before health check passed', undefined, {
        ...
        if (pendingStartupExit.statusAtExit === 'starting') {
          void Promise.resolve(
            options?.onPendingExit?.(
              makeStartupError('early_exit', 'DeepOrganiser Core exited after startup health timeout', undefined, {
                exitCode: exitCode ?? undefined,
                signal: exitSignal ?? undefined,
              })
            )
          ).catch((error) => {
            console.error('[deeporganiser-core] pending exit handler failed:', error);
          });
        }
      });
    });
    ...
    this.childProcess.stdout?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        ...
        const port = parseDeepOrganiserCoreListeningPort(trimmed);
        if (port !== undefined) {
          this._port = port;
        ...
        if (trimmed) console.log(`[deeporganiser-core] ${line}`);
      }
    });
    ...
    this.childProcess.stderr?.on('data', (data: Buffer) => {
      ...
      for (const line of data.toString().split('\n')) {
        if (line.trim()) console.error(`[deeporganiser-core] ${line}`);
      }
    });
    ...
    if (!health.ok) {
      const healthTimeoutError = makeStartupError(
        'health_timeout',
        'DeepOrganiser Core failed to start within timeout',
        undefined,
        {
          ...health.diagnostics,
        }
      ...
      if (options?.allowPendingOnHealthTimeout && this.childProcess) {
        startupSettled = true;
        console.warn(`[deeporganiser-core] health check timed out; keeping process alive on port ${this._port}`);
        void Promise.resolve(options.onHealthTimeout?.(healthTimeoutError)).catch((error) => {
          console.error('[deeporganiser-core] health timeout handler failed:', error);
        });
    ...
    this.restartCount = 0;
    console.info(
      `[deeporganiser-core] health ready on port ${this._port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${health.diagnostics.healthCheckElapsedMs}, data-dir: ${dbPath}`
    );
    return this._port;
  }
  ...
  private continueWaitingForHealth(
    ...
    void (async () => {
      ...
      console.info(
        `[deeporganiser-core] late health ready on port ${port} after ${health.diagnostics.healthCheckAttempts} attempts, elapsed_ms=${elapsedMs}, data-dir: ${this._lastDbPath}`
      );
      await onReady?.(port);
    })().catch((error) => {
      console.error('[deeporganiser-core] background health wait failed:', error);
    });
  }
  ...
  private handleCrash(code: number | null, signal?: NodeJS.Signals | string | null): void {
    ...
    if (this.restartCount > this.maxRestarts) {
      this._status = 'error';
      console.error('[deeporganiser-core] child exited unexpectedly; restart limit exceeded', crashContext);
      return;
    }

    const delay = Math.pow(2, this.restartCount - 1) * 1000;
    console.warn('[deeporganiser-core] child exited unexpectedly; scheduling restart', {
      ...crashContext,
      delayMs: delay,
    });
    ...
    setTimeout(() => {
      ...
      this.start(this._lastDbPath, this._lastLogDir, this._lastDirs, this._lastOptions, this._port)
        ...
        .catch((error) => {
          this._status = 'error';
          console.error('[deeporganiser-core] restart after crash failed', {
            port: this._port,
            restartCount: this.restartCount,
            maxRestarts: this.maxRestarts,
            delayMs: delay,
