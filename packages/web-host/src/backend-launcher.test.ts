...
describe('buildSpawnArgs', () => {
  ...
  it('respects DEEPORGANISER_LOG_LEVEL override', () => {
    const prev = process.env.DEEPORGANISER_LOG_LEVEL;
    process.env.DEEPORGANISER_LOG_LEVEL = 'trace';
    try {
      const args = buildSpawnArgs({
        port: 1,
        dbPath: '/d',
      ...
      if (prev === undefined) delete process.env.DEEPORGANISER_LOG_LEVEL;
      else process.env.DEEPORGANISER_LOG_LEVEL = prev;
    }
  });
});

describe('buildSpawnEnv', () => {
  it('merges process.env with DeepOrganiser dir vars', () => {
    const env = buildSpawnEnv({
      cacheDir: '/c',
      workDir: '/w',
      logDir: '/l',
    });
    expect(env.DEEPORGANISER_CACHE_DIR).toBe('/c');
    expect(env.DEEPORGANISER_WORK_DIR).toBe('/w');
    expect(env.DEEPORGANISER_LOG_DIR).toBe('/l');
    expect(env[legacyEnvName('CACHE_DIR')]).toBe('/c');
    expect(env[legacyEnvName('WORK_DIR')]).toBe('/w');
    expect(env[legacyEnvName('LOG_DIR')]).toBe('/l');
    const originalEntries = (process.env.PATH ?? '').split(':').filter(Boolean);
    for (const entry of originalEntries) {
      expect(env.PATH).toContain(entry);
    }
  });

  it('prepends common user bin dirs so officecli is visible to the backend', () => {
    if (process.platform === 'win32') {
      expect(buildBackendPath({ USERPROFILE: 'C:\\Users\\alice', PATH: 'C:\\Windows\\System32' })).toContain(
        'C:\\Windows\\System32'
      );
      return;
    }

    const env = { HOME: '/Users/alice', PATH: '/usr/bin:/bin' };

    const backendPath = buildBackendPath(env);

    expect(backendPath.split(':').slice(0, 3)).toEqual([
...
describe('findAvailablePort', () => {
  ...
  it('skips ports blocked by Fetch so health checks can use the selected port', async () => {
    ...
    try {
      ...
      expect(infoSpy).toHaveBeenCalledWith('[deeporganiser-core] skipped fetch-blocked backend port 1720');
      expect(infoSpy).toHaveBeenCalledWith('[deeporganiser-core] selected backend port 40404 after 2 attempts');
    } finally {
      infoSpy.mockRestore();
    }
  });
...
describe('BackendLifecycleManager.start (success path)', () => {
  it('lets DeepOrganiser Core choose the backend port and waits for the reported listening event', async () => {
    vi.mocked(createServer).mockImplementation(() => {
    ...
    const mgr = new BackendLifecycleManager(APP_META_PACKAGED, () => '/abs/path/deeporganiser-core');
  ...
  it('spawns with correct args, waits for /health, reports running', async () => {
    ...
    const resolveBackend = vi.fn(() => '/abs/path/deeporganiser-core');
    ...
    try {
      ...
      expect(spawnCall[0]).toBe('/abs/path/deeporganiser-core');
      ...
      expect(opts.env.DEEPORGANISER_CACHE_DIR).toBe('/c');
      expect(opts.env.DEEPORGANISER_WORK_DIR).toBe('/w');
      expect(opts.env.DEEPORGANISER_LOG_DIR).toBe('/l');
      expect(opts.env[legacyEnvName('CACHE_DIR')]).toBe('/c');
      expect(opts.env[legacyEnvName('WORK_DIR')]).toBe('/w');
      expect(opts.env[legacyEnvName('LOG_DIR')]).toBe('/l');
      expect((spawnCall[2] as { detached?: boolean }).detached).toBe(process.platform !== 'win32');

      expect(fetchSpy).toHaveBeenCalled();
      expect(infoSpy).toHaveBeenCalledWith(
        expect.stringContaining('[deeporganiser-core] health ready on port 55555 after 1 attempts, elapsed_ms=')
      );
    } finally {
      fetchSpy.mockRestore();
      infoSpy.mockRestore();
...
describe('BackendLifecycleManager.start (health timeout)', () => {
  it('captures backend boundary code and stage from early-exit stderr', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.open databasePath=/db/path/deeporganiser-backend.db: failed to initialize application data\n'
      )
    );
  ...
  it('captures backend boundary code when stderr drains after exit but before close', async () => {
    ...
    child.stderr?.emit(
      'data',
      Buffer.from(
        'BOOTSTRAP_DATA_INIT_FAILED stage=database.migration databasePath=/db/path/deeporganiser-backend.db: failed to initialize application data\n'
      )
    );
    child.emit('close', 1, null);
  ...
  it('kills child and reports listen_timeout when DeepOrganiser Core never reports a port', async () => {
    vi.useFakeTimers();
    const child = makeFakeChild();
    vi.mocked(spawn).mockReturnValue(child as unknown as ChildProcess);
  ...
  it('includes startup diagnostics when health check times out', async () => {
    ...
    const expectedRejection = expect(startPromise).rejects.toMatchObject({
      ...
      details: expect.objectContaining({
        ...
        binaryPath: '/abs/path/deeporganiser-core',
        port: 33334,
...
describe('BackendLifecycleManager crash restart', () => {
  ...
  it('logs crash restart scheduling details', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith('[deeporganiser-core] child exited unexpectedly; scheduling restart', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 65303,
      restartCount: 1,
  ...
  it('logs when crash restart limit is exceeded', () => {
    ...
    expect(mgr.status).toBe('error');
    expect(errorSpy).toHaveBeenCalledWith('[deeporganiser-core] child exited unexpectedly; restart limit exceeded', {
      exitCode: 1,
      signal: 'SIGABRT',
      port: 0,
      restartCount: 4,
