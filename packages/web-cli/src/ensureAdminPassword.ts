 ...
 *
 * On first tarball launch, DeepOrganiser Core's SQLite `users` table holds the
...
export type EnsureAdminPasswordOptions = {
   ...
   * by launch context — packaged tarball = `deeporganiser-web resetpass`, in-repo dev
...
export async function ensureAdminPassword(
  ...
  const resetCmd = opts.resetCommand ?? 'deeporganiser-web resetpass';
  ...
  try {
    ...
    deps.warn(`[deeporganiser-web] could not verify admin credentials: ${err instanceof Error ? err.message : String(err)}`);
    return;
  }
  ...
  if (!needsSetup) {
    ...
    deps.log(`[deeporganiser-web] Log in with username "${username}". Forgot the password? Run \`${resetCmd}\`.`);
    return;
  }
  ...
  try {
    ...
    if (!resetRes.ok) {
      deps.warn(`[deeporganiser-web] /api/webui/reset-password returned ${resetRes.status} — run \`${resetCmd}\``);
      return;
    }
    ...
    if (!newPassword) {
      deps.warn(`[deeporganiser-web] /api/webui/reset-password returned no new_password — run \`${resetCmd}\``);
      return;
    }
    const username = await fetchAdminUsername(deps, opts.backendPort);
    deps.log(`[deeporganiser-web] Generated initial admin password: ${newPassword}`);
    deps.log(`[deeporganiser-web] Log in with username "${username}" and change it from the UI.`);
  } catch (err) {
    deps.warn(
      `[deeporganiser-web] failed to seed initial admin password: ${err instanceof Error ? err.message : String(err)}`
    );
  }
}
