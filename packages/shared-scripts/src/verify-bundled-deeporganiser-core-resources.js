...
function backendBinaryName(platform) {
  return platform === 'win32' ? 'deeporganiser-core.exe' : 'deeporganiser-core';
}
...
function bundledPath(runtimeKey, ...parts) {
  return normalize(path.join('bundled-deeporganiser-core', runtimeKey, ...parts));
}
...
function verifyBundledDeepOrganiserCoreResources({ resourcesDir, electronPlatformName, targetArch }) {
  const runtimeKey = `${electronPlatformName}-${targetArch}`;
  const baseDir = path.join(resourcesDir, 'bundled-deeporganiser-core', runtimeKey);
  const checked = [];
  const missing = [];
