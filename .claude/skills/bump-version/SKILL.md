---
name: bump-version
description: Use when bumping the DeepScientist-Pro version: query AionCore release, verify artifacts, update package.json, generate CHANGELOG, branch, commit, push, create PR, auto-merge, tag release.
---

# Bump Version

Automate the DeepScientist-Pro release preparation: query AionCore release → verify artifacts → update versions → generate CHANGELOG → branch → PR → tag.

**Usage:** `/bump-version [version] [flags]`

- `/bump-version` — auto patch + latest AionCore
- `/bump-version 2.2.0` — explicit DeepScientist-Pro version + latest AionCore
...
### Step 3: Determine DeepScientist-Pro Target Version
...
Display: "Bumping DeepScientist-Pro: {current} → {target}"
...
## [{target}](https://github.com/ResearAI/DeepOrganiser/compare/v{previous}...v{target}) ({date YYYY-MM-DD})
...
```
 ...
 2. git pull --rebase
 3. Determine DeepScientist-Pro target version (patch+1 or explicit)
