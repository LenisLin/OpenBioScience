 ...
 *   2. No webui running → spawn a short-lived DeepOrganiser Core against the same
 ...
 *   DEEPORGANISER_DATA_DIR=/x bun run resetpass
...
import { startBackend, stopBackend } from '@deeporganiser/web-host';
 ...
 * See the comment there for why the default is a dedicated standalone WebUI dir.
 */
 ...
 * desktop side). Callers can override with `--port` / `DEEPORGANISER_PORT` to match
 ...
 * pass --port / DEEPORGANISER_PORT if they launched webui on a non-default one.
 */
