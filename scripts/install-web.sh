#!/usr/bin/env bash
# ============================================================================
# DeepOrganiser WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/ResearAI/DeepOrganiser/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/deeporganiser-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/deeporganiser-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/ResearAI/DeepOrganiser/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║     DeepOrganiser WebUI Installer (No Electron)     ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/deeporganiser-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="deeporganiser-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/ResearAI/DeepOrganiser/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is deeporganiser-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/deeporganiser-web" ]]; then
        mv "${extract_temp}/deeporganiser-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing deeporganiser-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/deeporganiser-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/deeporganiser-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/deeporganiser-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/deeporganiser-web"
    local target_path="${INSTALL_DIR}/deeporganiser-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by deeporganiser-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 DeepOrganiser WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/deeporganiser-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start DeepOrganiser WebUI"
        echo "    deeporganiser-web start"
        echo ""
        echo "    # Check version"
        echo "    deeporganiser-web version"
    else
        echo "    # Start DeepOrganiser WebUI (using full path)"
        echo "    ${INSTALL_DIR}/deeporganiser-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/deeporganiser-web ~/.local/bin/deeporganiser-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/ResearAI/DeepOrganiser"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/ResearAI/DeepOrganiser/issues"
    echo ""
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo ""
        echo "    # Remove symlink"
        echo "    rm ${BIN_DIR}/deeporganiser-web"
    fi
