 ...
 * parsing the electron-log file for [DeepOrganiser:ready] / [DeepOrganiser:init] /
 * [DeepOrganiser:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [DeepOrganiser:ready] marks
  ...
  // Parsed from [DeepOrganiser:init] marks
  initTotalMs: number;
  // Parsed from [DeepOrganiser:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'DeepOrganiser-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'DeepOrganiser', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'DeepOrganiser', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'DeepOrganiser', 'logs', `${today}.log`));
  }
...
// Matches: [DeepOrganiser:ready] <label> +<ms>ms
// Matches: [DeepOrganiser:init]  <label> +<ms>ms
// Matches: [DeepOrganiser:process] <label> +<ms>ms
const MARK_REGEX = /\[DeepOrganiser:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[DeepOrganiser] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[DeepOrganiser] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[DeepOrganiser] Showing main window')) marks.logs.showingMainWindow = true;
  }
