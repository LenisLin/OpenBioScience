...
const tarballName = `deeporganiser-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
// 1. Prepare bundled DeepOrganiser Core
console.log('1. Preparing DeepOrganiser Core...');
prepareDeepOrganiserCore({
  projectRoot,
  platform,
  arch,
  version: resolveDeepOrganiserCoreVersion(projectRoot),
});
...
const tarballContentDir = path.join(stagingDir, 'deeporganiser-web');
...
const executableName = platform === 'win32' ? 'deeporganiser-web.exe' : 'deeporganiser-web';
...
// `deeporganiser-web version` match the tarball filename.
...
// 7. Copy bundled DeepOrganiser Core
const backendSrc = path.join(projectRoot, 'resources/bundled-deeporganiser-core', `${platform}-${arch}`);
const backendDest = path.join(tarballContentDir, 'bundled-deeporganiser-core', `${platform}-${arch}`);
if (!fs.existsSync(backendSrc)) {
  throw new Error(`Backend bundle dir missing at ${backendSrc}. Ensure prepareDeepOrganiserCore succeeded.`);
}
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} deeporganiser-web`, {
