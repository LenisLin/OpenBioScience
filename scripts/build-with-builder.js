...
function resolveTargetPlatform(builderArgsText) {
  if (builderArgsText.includes('--win') || builderArgsText.includes('--windows')) return 'win32';
  if (builderArgsText.includes('--linux')) return 'linux';
  if (builderArgsText.includes('--mac') || builderArgsText.includes('--macos')) return 'darwin';
...
const targetPlatform = resolveTargetPlatform(builderArgs);
...
try {
  ...
  // 5. Prepare DeepOrganiser Core binary (for packaged runtime usage)
