...
export INSTALL_DIR="/tmp/deeporganiser-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/deeporganiser-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/deeporganiser-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/deeporganiser-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/deeporganiser-web"
    exit 1
fi
...
VERSION_OUTPUT=$(deeporganiser-web version 2>&1 || echo "")
