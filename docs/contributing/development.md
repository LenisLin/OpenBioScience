...
git clone https://github.com/ResearAI/DeepOrganiser.git
cd DeepOrganiser
...
When you have two clones of the repository (e.g. `DeepOrganiser` and `DeepOrganiser-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `DEEPORGANISER_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`DeepOrganiser-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.deeporganiser-dev-2`, `~/.deeporganiser-config-dev-2`)
...
DeepOrganiser uses **electron-vite** for fast bundling:
