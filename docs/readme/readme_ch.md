...
这组图更适合中文读者快速理解 OpenScience：它从首页入口、四种模式、医学循证、目标模式、知识沉淀，到 Science artifact 和开源入口，串成一个完整的产品故事。

<p align="center">
  <img src="../../resources/readme/xhs/contact-sheet.png" alt="OpenScience 小红书图文总览" width="100%" />
</p>

<table>
<tr>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-01.png" alt="OpenScience 开源版 Claude Science" /><br/>
<sub><b>开源版 Claude Science。</b> 完全开源、本地化部署，支持任意模型和多学科科研。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-02.png" alt="OpenScience 研究项目入口" /><br/>
<sub><b>从聊天框进入研究项目。</b> 文件、证据、运行记录和结果留在同一个地方。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-03.png" alt="OpenScience 四种模式" /><br/>
<sub><b>四种模式。</b> 科学研究、医学循证、知识沉淀和目标模式对应不同科研场景。</sub>
</td>
</tr>
<tr>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-04.png" alt="OpenScience 医学循证模式" /><br/>
<sub><b>医学循证模式。</b> 先看证据边界，再给出可追溯的医学回答。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-05.png" alt="OpenScience 目标模式" /><br/>
<sub><b>目标模式。</b> 继承 DeepScientist 的长期任务能力，让科研目标持续推进。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-06.png" alt="OpenScience 知识沉淀模式" /><br/>
<sub><b>知识沉淀模式。</b> 把做对的对话、SOP 和经验整理成可复用 skill。</sub>
</td>
</tr>
<tr>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-07.png" alt="OpenScience 科研 artifact" /><br/>
<sub><b>结果不是附件。</b> 图、表、报告和 notebook 都保留代码、输入、日志和版本。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-08.png" alt="OpenScience Science artifact 工作流" /><br/>
<sub><b>科研过程沉淀成 artifact。</b> 对话、文件、报告和证据链留在同一个研究对象里。</sub>
</td>
<td width="33%" valign="top">
<img src="../../resources/readme/xhs/xhs-09.png" alt="OpenScience GitHub 和网站入口" /><br/>
<sub><b>欢迎一起建设。</b> 访问 OpenScience.cc 或 GitHub ResearAI/OpenScience。</sub>
</td>
</tr>
</table>
...
安装后直接打开 OpenScience。现在不需要应用级登录，启动后会直接进入主页。桌面应用默认也会启动本机 WebUI，你可以在**设置 -> 远程连接 -> WebUI** 里查看浏览器访问地址。你可以新建或打开研究项目，然后选择**科学研究模式**、**医学循证模式**、**目标模式**或**知识沉淀模式**开始工作。

Linux 发行包是 Debian 安装包，可以这样使用：
...
如果你希望在 Linux 工作站、服务器或无图形环境中直接使用浏览器 WebUI，可以显式启动 WebUI。本机浏览器访问不需要开放远程访问；只有需要局域网、服务器或 SSH 隧道访问时才加 `--remote`：
...
本机打开 `http://localhost:25808`；开启远程访问并放行防火墙后，可以用 `http://<服务器IP>:25808` 访问。
...
桌面应用运行时，WebUI 默认可以在设置页查看和访问。如果你希望从源码启动一个不复用桌面数据目录的干净 WebUI 开发版本，可以使用：

```bash
DEEPORGANISER_DATA_DIR=/tmp/openscience-webui-clean bun run webui -- --port 25809
```

如果是在 Linux 服务器或容器里运行，不需要 Electron 桌面窗口，可以使用：

```bash
DEEPORGANISER_DATA_DIR=/var/lib/openscience-webui bun run webui:remote -- --port 25809
```

然后在服务器本机访问 `http://localhost:25809`，或者用 SSH 转发到自己的电脑：
...
```text
请帮我在这台电脑上安装并运行 OpenScience。仓库地址是 https://github.com/ResearAI/OpenScience 。请先检查我的系统环境，确认是否已有 git、bun，以及至少一个本地 coding agent（例如 Codex CLI、Claude Code 或 OpenCode）；如果缺少，请告诉我需要安装什么。然后克隆仓库，运行 `bun install --frozen-lockfile`，再运行 `bun run start` 启动桌面应用。桌面应用应该默认通过“设置 -> 远程连接 -> WebUI”提供浏览器访问入口。如果我希望启动干净 WebUI，请使用 `DEEPORGANISER_DATA_DIR=/tmp/openscience-webui-clean bun run webui -- --port 25809`；如果是在 Linux 服务器上运行，请使用 `DEEPORGANISER_DATA_DIR=/var/lib/openscience-webui bun run webui:remote -- --port 25809`，必要时用 `ssh -L 25809:127.0.0.1:25809 user@host` 转发端口。普通安装时不要运行 `bun run skills:science:materialize`，除非我是在维护 vendored skill pack。安装过程中不要删除或覆盖我的个人文件；如果遇到依赖、权限、端口、Electron 或原生模块问题，请一步步诊断，并在安全的情况下修复。启动成功后，请告诉我 OpenScience 已经打开，且现在不需要应用级登录，然后说明下一步应该去设置页哪里配置模型/API key 或本地 coding agent。
```
...
OpenScience 的长期科研工作流设计部分继承自 DeepScientist。如果 OpenScience 或科学研究模式对你的论文、报告或科研项目有实质帮助，也欢迎引用 DeepScientist 论文：
...
如果你希望交流 OpenScience、DeepScientist、AI Scientist 和科研智能体相关用法，可以扫码加入微信群。二维码会定期更新。

<p align="center">
  <img src="../../resources/readme/wechat/ai-scientist-wechat-group-8.jpg" alt="AI Scientist 科研讨论微信群" width="360" />
</p>
