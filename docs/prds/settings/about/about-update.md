...
1. 用户打开「设置 → 关于」页面
2. 页面顶部居中显示应用名 "DeepOrganiser"（h3 标题）
3. 下方显示应用描述（通过 i18n 系统，随语言设置变化）
4. 显示当前版本号 badge（格式 `v{x.y.z}`），版本号来自打包时的 `package.json`
5. 版本号旁有 GitHub 图标，点击在系统浏览器中打开项目仓库 `https://github.com/ResearAI/DeepOrganiser`
...
| 序号 | 标题     | 行为               | 目标                                                 |
| ---- | -------- | ------------------ | ---------------------------------------------------- |
| 1    | 帮助文档 | 打开外部链接       | `https://github.com/ResearAI/DeepOrganiser/wiki`     |
| 2    | 更新日志 | 打开外部链接       | `https://github.com/ResearAI/DeepOrganiser/releases` |
...
│                                                          │
│  AboutModalContent                                       │
│    ├─ CustomEvent('deeporganiser-open-update-modal')            │
