...
**生成时间**：2026-04-22
**实现目录**：`tests/e2e/features/conversations/codex/`
...
- ⏭️ Skipped: 4/15 (TC-A-04/07/08/09, codex binary 运行时切换挂起)
- ❌ Failed: 0/15
...
| ------- | -------------- | --------------------------------------------------------------- | ---- | ------------------------------------------------------------------------------- | ------ | ------- |
| TC-A-01 | 最小可行路径   | tests/e2e/features/conversations/codex/basic-flow.e2e.ts       | 79   | `test('TC-A-01: should complete minimal conversation with no attachments')`     | 4      | ✅      |
| TC-A-02 | 关联单个文件夹 | tests/e2e/features/conversations/codex/basic-flow.e2e.ts       | 154  | `test('TC-A-02: should associate single folder and reference in message')`      | 3      | ✅      |
| TC-A-03 | 上传单个文件   | tests/e2e/features/conversations/codex/basic-flow.e2e.ts       | 223  | `test('TC-A-03: should upload single file and binary receives file parameter')` | 3      | ✅      |
| TC-A-04 | 非默认模型     | tests/e2e/features/conversations/codex/model-selection.e2e.ts  | 70   | `test.skip('TC-A-04: should use second model selected on guid page')`           | 4      | ⏭️ Skip |
| TC-A-05 | yolo 权限      | tests/e2e/features/conversations/codex/permission-modes.e2e.ts | 69   | `test('TC-A-05: should use yolo permission selected on guid page')`             | 5      | ✅      |
...
| 用例 ID | 用例标题   | 实现文件                                                               | 行号 | 测试函数名                                                                              | 截图数 | 状态    |
| ------- | ---------- | ---------------------------------------------------------------------- | ---- | --------------------------------------------------------------------------------------- | ------ | ------- |
| TC-A-06 | 切换权限   | tests/e2e/features/conversations/codex/permission-modes.e2e.ts        | 154  | `test('TC-A-06: should switch permission mid-conversation and persist to DB')`          | 5      | ✅      |
| TC-A-07 | 切换模型   | tests/e2e/features/conversations/codex/model-selection.e2e.ts         | 135  | `test.skip('TC-A-07: should switch model mid-conversation and update DB')`              | 5      | ⏭️ Skip |
| TC-A-08 | 连续切换   | tests/e2e/features/conversations/codex/mid-conversation-switch.e2e.ts | 68   | `test('TC-A-08: should handle continuous switch (model → permission → model)')`         | 6      | ⏭️ Skip |
| TC-A-09 | 多轮对话   | tests/e2e/features/conversations/codex/mid-conversation-switch.e2e.ts | 178  | `test('TC-A-09: should handle 3 rounds of conversation after model/permission switch')` | 6      | ⏭️ Skip |
| TC-A-10 | 组合场景 1 | tests/e2e/features/conversations/codex/combo-scenarios.e2e.ts         | 72   | `test('TC-A-10: should handle folder + second model + yolo mode combo')`                | 3      | ✅      |
| TC-A-11 | 组合场景 2 | tests/e2e/features/conversations/codex/combo-scenarios.e2e.ts         | 151  | `test('TC-A-11: should handle file + non-default model + default mode combo')`          | 3      | ✅      |
| TC-A-12 | 完整组合   | tests/e2e/features/conversations/codex/combo-scenarios.e2e.ts         | 229  | `test('TC-A-12: should handle full combo (folder + file + second model + yolo)')`       | 4      | ✅      |
...
| 用例 ID | 用例标题      | 实现文件                                                  | 行号 | 测试函数名                                                                          | 截图数 | 状态    |
| ------- | ------------- | --------------------------------------------------------- | ---- | ----------------------------------------------------------------------------------- | ------ | ------- |
| TC-A-13 | Binary 不可达 | tests/e2e/features/conversations/codex/edge-cases.e2e.ts | 67   | `test('TC-A-13: should handle empty workspace folder without crashing')`            | 3      | ⚠️ 偏差 |
| TC-A-14 | 超大文件限制  | tests/e2e/features/conversations/codex/edge-cases.e2e.ts | 127  | `test('TC-A-14: should handle very long message (2000 characters)')`                | 3      | ⚠️ 偏差 |
| TC-A-15 | 不存在文件夹  | tests/e2e/features/conversations/codex/edge-cases.e2e.ts | 185  | `test('TC-A-15: should handle rapid consecutive messages without race conditions')` | 3      | ⚠️ 偏差 |
...
**原定义**：验证 codex binary 不可达时测试正确跳过（参考 `resolveCodexBinary()` 返回 null 场景）

**实现要点**：

1. 在 `beforeAll` 中临时 mock `resolveCodexBinary()` 返回 null
2. 验证 `test.skip(true, 'codex binary not found')` 被触发
3. 无截图（跳过测试不执行主体）
...
通过 `waitForCodexReply()` helper 轮询验证：

- 等待 AI 回复完成（`conv.status === 'finished'` + content 稳定 2s）
- 超时时间：150s（codex binary 比 Gemini API 快，但需要预留模型切换时间）
- 字段名：`createdAt`（驼峰，非 `created_at`）
- 状态字段：codex text messages 不设置 `status='finish'`，只依赖 `conv.status`
...
1. **UI 状态清理**：ESC × 5 次
2. **数据库清理**：调用 `cleanupE2ECodexConversations(page)`
   - 使用 `remove-conversation` 批量删除 `E2E-codex-` 开头对话
   - 依赖 FK CASCADE 自动删除关联 messages
3. **sessionStorage 清理**：清除 `codex_initial_message_*` 和 `codex_initial_processed_*` keys
...
1. 运行命令重新统计截图数：
   ```bash
   for file in tests/e2e/features/conversations/codex/*.e2e.ts; do
...
**症状**：
运行时切换 model 或 permission 后，后续消息发送时 codex binary 静默挂起，AI 回复永不到达。
...
1. 创建 codex 对话，使用 modelA + default 模式
...
**数据库现场**（来自 `/tmp/codex-all-v3.log`）：

```
[waitForCodexReply TIMEOUT] conv.status=pending, msg count=3
[waitForCodexReply TIMEOUT]   - pos=right type=text status=null preview="Hello, initial message."
[waitForCodexReply TIMEOUT]   - pos=left type=text status=null preview="Hello! How can I help today?"
[waitForCodexReply TIMEOUT]   - pos=right type=text status=null preview="After all switches."
```

**已排除**：

- ✅ 测试脚本正确：第二条消息已通过 `sendCodexMessage` 送达（DB 有 right message）
- ✅ Bridge 通信：message 入库说明 bridge → main process 路径正常
- ✅ 初始模型选择：非切换场景（TC-A-01/02/03）正常完成

**待排查**：

- ❓ codex binary 的运行时状态机是否支持 model/permission 切换？
...
- 等待产品侧对 codex binary 的运行时切换逻辑进行诊断
