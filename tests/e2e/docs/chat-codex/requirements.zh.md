# Codex (codex) E2E 测试需求

**版本**: v1.1（修订版）
**作者**: chat-codex-analyst
**日期**: 2026-04-22
**状态**: 已完成 Gate 1 + v1.1 修订（纠正 codex 模型来源）
...
用户从 **guid 首页** 选择 codex agent，配置上下文（文件/文件夹/模型/权限），发送消息，进入 **codex 对话页**，接收流式回复，并可在对话中切换模型/权限。

**关键路径**（源码追溯）：

1. **guid 首页** (`src/renderer/pages/guid/GuidPage.tsx:83-100`)
   - 用户点击 `AgentPillBar` 中 codex pill（`data-agent-backend="codex"`）
   - 可选：通过 `GuidModelSelector` 选择模型（ACP 模型列表）
   - 可选：通过 `AgentModeSelector` 选择权限模式
   - 可选：上传文件 / 关联文件夹（guid 页暂不支持，对话页支持）
   - 输入消息，点击发送 → 创建对话并导航到 `/conversation/codex/<id>`

2. **codex 对话页** (`src/renderer/pages/conversation/platforms/codex/CodexChat.tsx:27-54`)
   - 显示 `MessageList`（历史消息）
   - `CodexSendBox` 提供文件上传、文件夹关联、模型选择、权限选择
   - 发送消息后，前端通过 `ipcBridge.conversation.sendMessage.invoke()` 调用进程端

3. **进程端** (`src/process/task/CodexManager.ts:78-781`)
   - 创建 `CodexAgent` 实例（`src/process/agent/codex/index.ts:54-450`）
   - 启动 codex binary（stdin/stdout JSON Lines 协议）
   - 处理流式事件（`stream_start`, `text_delta`, `thinking`, `tool_request`, `stream_end` 等）
   - 权限确认逻辑（`auto_edit` / `yolo` 自动批准部分工具）

4. **后端持久化**
   - backend 独占 `deeporganiser.db`，Electron 不再直接访问 SQLite
...
| 维度           | 档位 | 说明                                   | 源码追溯                                                 |
| -------------- | ---- | -------------------------------------- | -------------------------------------------------------- |
| **文件夹关联** | 2 档 | 无关联 / 关联                          | `CodexSendBox.tsx:331-337` atPath 状态 + event listener |
| **文件上传**   | 2 档 | 无上传 / 上传                          | `CodexSendBox.tsx:103-125` file input handler           |
| **模型**       | 2 档 | 从 ACP 模型列表挑 2 个（推荐配置见下） | `GuidModelSelector.tsx` + `useGuidModelSelection.ts`     |
| **权限模式**   | 3 档 | default / auto_edit / yolo             | `agentModes.ts:65-69` codex 分支                        |
| **对话中切换** | 必测 | 切换模型 + 切换权限                    | `CodexModelSelector.tsx` + `AgentModeSelector`          |
...
- 发送消息时 `atPath = []`
- `workspace` 指向临时目录 `/tmp/e2e-chat-codex-<scenario>-<ts>/`

**档位 2 - 关联文件夹**:

- 通过 `emitter.emit('codex.selected.file', items)` 触发
- `atPath` 包含 `{path: '/tmp/...', name: 'folder-name', isFile: false}`
- 对话页显示蓝色 Tag（`CodexSendBox.tsx:423-446`）

**前置条件**:

- E2E 在 `/tmp/e2e-chat-codex-<scenario>-<ts>/` 创建临时文件夹
...
- E2E 创建测试文件 `/tmp/e2e-chat-codex-<scenario>-<ts>/test.txt`（内容: "E2E test file"）

**验证点**:

- 上传后显示文件预览卡片（`CodexSendBox.tsx:413-421`）
...
**重要纠正**: codex **不使用 ACP 模型列表**，而是使用用户在 Settings → Model 里配置的通用 provider 列表。

**源码追溯**:

- `src/renderer/pages/conversation/platforms/codex/useCodexModelSelection.ts:34-40`:
  ```typescript
...
- **codex**: 模型从 `ipcBridge.mode.getModelConfig` 读取（用户配置文件，排除 `gemini-with-google-auth`）
...
- codex **不支持 Google Auth**（`useCodexModelSelection.ts:36-40` 过滤）
...
| mode        | label             | 行为                                             | 源码追溯                                |
| ----------- | ----------------- | ------------------------------------------------ | --------------------------------------- |
| `default`   | Default           | 每次工具调用需确认                               | `CodexManager.ts:250-296` confirm 逻辑 |
| `auto_edit` | Auto-Accept Edits | 自动批准 edit / info 类工具，exec / mcp 仍需确认 | `CodexManager.ts:254-259`              |
| `yolo`      | YOLO              | 全自动批准所有工具                               | `CodexManager.ts:250-253`              |

**权限切换接口**:

- guid 页: `GuidActionRow.tsx:277-287` — `AgentModeSelector` 组件
- 对话页: `CodexSendBox.tsx:391-401` — 同样使用 `AgentModeSelector`
- 进程端: `CodexManager.ts:727-737` — `setMode()` 更新 DB + 发送 `set_mode` 到 binary
...
- 操作: 点击对话页 `CodexModelSelector` 按钮 → 选择不同模型
...
- 预期: 立即生效（`CodexManager.setMode()` 发送 `set_mode` 到 binary）
...
```typescript
const CODEX_BINARY_PATH = '/Users/zhoukai/.local/bin/codex';
```
...
const binary = await ipcBridge.fs.findCodexBinary.invoke();
if (!binary) {
  test.skip('codex binary not found in PATH or ~/.local/bin/codex, skipping E2E tests');
}
```

**源码追溯**: `src/process/agent/codex/binaryResolver.ts`

- 解析顺序: 环境变量 `AION_CLI_PATH` → `~/.deeporganiser/bin/aion-<platform>-<arch>` → 系统 PATH 中的 `aion` 命令

**验证命令**（team-lead 已确认）:

```bash
$ which codex
/Users/zhoukai/.local/bin/codex
```

**E2E 实现规范**:

```typescript
// tests/e2e/setup/codex.setup.ts
export async function checkCodexBinary(page: Page): Promise<boolean> {
  try {
    const binary = await invokeBridge(page, 'fs.findCodexBinary');
    if (!binary) {
      console.error('[E2E Setup] codex binary not found in PATH or ~/.local/bin/codex');
      return false;
    }
    console.log(`[E2E Setup] codex binary found: ${binary}`);
    return true;
  } catch (error) {
    console.error('[E2E Setup] Failed to check codex binary:', error);
    return false;
  }
}

// tests/e2e/specs/chat-codex/*.spec.ts
test.beforeAll(async ({ page }) => {
  const hasBinary = await checkCodexBinary(page);
  if (!hasBinary) {
    test.skip('codex binary not found, skipping E2E tests');
  }
});
```
...
- 错误信息示例: `codex binary not found in PATH or ~/.local/bin/codex`

---

### 3.2 Binary 启动与超时

**启动流程** (`src/process/agent/codex/index.ts:85-157`):
...
```typescript
test(
  'should start codex conversation',
  async ({ page }) => {
...
- 启动超时: 抛出 `Error('codex ready timeout (30s)')`
...
```
/tmp/e2e-chat-codex-<scenario>-<timestamp>/
├── test-file.txt          # 文件上传测试文件
├── test-folder/           # 文件夹关联测试目录
│   └── sample.md
└── .codex/               # codex session 文件（binary 自动创建）
```
...
afterEach(async ({ page }) => {
  ...
  try {
    ...
    await invokeBridge(page, 'db.exec', {
      sql: "DELETE FROM conversations WHERE name LIKE 'E2E-codex-%'"
    });

    // 3. 清理 FS（临时目录 + codex session 文件）
    ...
    const remaining = await invokeBridge(page, 'db.query', {
      sql: "SELECT COUNT(*) as count FROM conversations WHERE name LIKE 'E2E-codex-%'"
    });
...
| 资源类型              | 清理规则                                | 验证方式                      |
| --------------------- | --------------------------------------- | ----------------------------- |
| **DB conversations**  | `DELETE WHERE name LIKE 'E2E-codex-%'` | `SELECT COUNT(*)` 期望 0      |
| **DB messages**       | 级联删除（`ON DELETE CASCADE`）         | 自动清理                      |
| **FS 临时目录**       | `rm -rf /tmp/e2e-chat-codex-*`         | `fs.existsSync()` 期望 false  |
| **FS codex session** | 包含在临时目录内                        | 同上                          |
...
```typescript
const conversationName = `E2E-codex-${scenario}-${Date.now()}`;
// 示例: 'E2E-codex-no-attach-1745327890123'
```

**关键要求**:

- 前缀 **必须** 是 `E2E-codex-`（清理 SQL 依赖此前缀）
- 包含场景描述（便于日志追溯）
- 包含时间戳（避免重名）
...
| `name`       | TEXT        | 匹配 `'E2E-codex-*'` 模式             | -                          |
| `type`       | TEXT        | 固定 `'codex'`                        | -                          |
| `model`      | TEXT        | 模型 ID（如 `'claude-opus-4-7'`）      | `CodexManager.ts:108`     |
| `status`     | TEXT        | `'pending' \| 'running' \| 'finished'` | `CodexManager.ts:524-526` |
...
- `sessionMode`: `CodexManager.ts:740-747` — `saveSessionMode()`
- `lastTokenUsage`: `CodexManager.ts:432-449` — `saveContextUsage()`
...
- ✅ **已有**: `data-agent-pill="true"`, `data-agent-backend="codex"`, `data-agent-selected`
...
<div data-testid="codex-sendbox">
  {/* SendBox 根元素 */}
</div>

// src/renderer/pages/conversation/platforms/codex/CodexModelSelector.tsx
<Button data-testid="codex-model-selector">
  {/* 模型选择器按钮 */}
</Button>

// src/renderer/components/agent/AgentModeSelector.tsx（通用组件）
<Button data-testid={`agent-mode-selector-${backend}`}>
  {/* backend='codex' 时 → data-testid="agent-mode-selector-codex" */}
</Button>

// src/renderer/pages/conversation/platforms/codex/CodexSendBox.tsx（file input）
<input
  type="file"
  data-testid="codex-file-upload-input"
  multiple
  style={{ display: 'none' }}
/>
...
<Button
  data-testid="codex-attach-folder-btn"
...
```tsx
// 发送按钮
<Button data-testid="codex-send-btn" />

// 文件预览卡片
<FilePreview key={path} data-testid={`codex-file-preview-${idx}`} />

// 文件夹 Tag
<Tag key={item.path} data-testid={`codex-folder-tag-${idx}`} />

// 模型下拉菜单项
<Menu.Item key={modelId} data-testid={`codex-model-menu-item-${modelId}`} />

// 权限下拉菜单项
<Menu.Item key={mode} data-testid={`codex-mode-menu-item-${mode}`} />
```
...
```typescript
test('should complete codex conversation with no attachments', async ({ page }) => {
  // 1. 导航到 guid 页
  await page.goto('/#/guid');

  // 2. 选择 codex agent
  await page.click('[data-agent-backend="codex"][data-agent-selected="false"]');

  // 3. 输入消息（通过 Playwright locator）
  const textarea = page.locator('textarea[placeholder*="codex"]');
  await textarea.fill('Hello, codex!');

  // 4. 点击发送按钮
  await page.click('.send-button-custom'); // 或 [data-testid="codex-send-btn"]

  // 5. 等待导航到对话页
  await page.waitForURL(/\/conversation\/codex\/.+/, { timeout: 10000 });

  // 6. 提取 conversationId
  const url = page.url();
  const conversationId = url.match(/\/conversation\/codex\/(.+)/)?.[1];
...
| 启动超时（30s） | 抛出 `Error('codex ready timeout')`    | `index.ts:136-140`         |
| 进程崩溃        | 前端收到 `error` 事件，对话标记为 error | `CodexManager.ts:297-298` |
...
**场景**: 同时打开 2 个 codex 对话，轮流发送消息

**预期**: 每个对话独立维护 binary 进程 + session（`CodexManager` 实例独立）
...
**源码追溯**: `src/process/agent/codex/index.ts:116-119` — `try-catch` 包裹 `JSON.parse()`
...
**背景**: codex 使用用户配置的 provider 列表（非 ACP 探测），E2E 需依赖测试环境的 provider 配置
...
- 若无可用 provider: `test.skip('No available providers for codex, skipping E2E tests')`

**动态 model 选择**（E2E setup 实现）:

```typescript
// tests/e2e/setup/codex.setup.ts
export async function getCodexTestModels(page: Page): Promise<{
...
- E2E beforeAll 检查 `getCodexTestModels()` 返回值
...
**背景**: `CodexAgent.setConfig(model)` 发送 `set_config` 命令，但 binary 能力未验证
...
**背景**: 当前存储在内存（`CodexApprovalStore`），进程重启后失效
...
**背景**: 当前代码未显式处理（`CodexManager.ts:253-296` confirm 逻辑独立）
...
| 文档            | 路径                                              | 状态                        |
| --------------- | ------------------------------------------------- | --------------------------- |
| Gate 1 需求文档 | `tests/e2e/docs/chat-codex/requirements.zh.md`   | ✅ 完成（本文档）           |
| Gate 1 讨论记录 | `tests/e2e/docs/chat-codex/discussion-log.zh.md` | ✅ 完成（双 reviewer 审核） |
| Gate 2 测试用例 | `tests/e2e/docs/chat-codex/test-cases.zh.md`     | ⏳ 待 designer 起草         |
| Gate 3 实现映射 | `tests/e2e/docs/chat-codex/implementation.md`    | ⏳ 待 engineer 起草         |
...
**修订人**: chat-codex-analyst
**触发原因**: 用户指出调研错误 — codex 模型来源非 ACP 探测
...
2. **§8 议题 0（新增）** — E2E 环境 provider 配置前置条件
   ...
   - 动态 model 选择实现: `getCodexTestModels()` helper

3. **源码追溯补充**:
   - `src/renderer/hooks/agent/useModelProviderList.ts:30` — `ipcBridge.mode.getModelConfig.invoke()`
   - `src/renderer/pages/conversation/platforms/codex/useCodexModelSelection.ts:34-40` — 过滤 Google Auth

**影响范围**:

- Gate 2 用例设计: designer 需确认 TC-A-04（guid 页选模型）和 TC-A-07（对话中切换）的前置条件
- Gate 3 实现: engineer 需实现 `getCodexTestModels()` helper（动态查询 provider 列表）
...
| 文件                                                                          | 行号   | 关键功能                                                            |
| ----------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------- |
| `src/renderer/pages/guid/GuidPage.tsx`                                        | 83-100 | providerAgentKey 状态（codex/gemini）                              |
...
| `src/renderer/pages/conversation/platforms/codex/CodexChat.tsx`             | 19-57  | 对话页容器 + MessageList + SendBox                                  |
| `src/renderer/pages/conversation/platforms/codex/CodexSendBox.tsx`          | 88-459 | 发送框逻辑 + 文件附件 + 权限选择                                    |
| `src/renderer/pages/conversation/platforms/codex/CodexModelSelector.tsx`    | 19-135 | 模型选择器（对话页）                                                |
| `src/renderer/pages/conversation/platforms/codex/useCodexModelSelection.ts` | 24-73  | 模型选择 hook（过滤 google auth）                                   |
| `src/renderer/pages/conversation/platforms/codex/useCodexMessage.ts`        | 20-321 | 流式消息处理 + 工具状态                                             |
| `src/renderer/utils/model/agentModes.ts`                                      | 65-69  | codex 权限模式枚举                                                 |
| `src/process/task/CodexManager.ts`                                           | 78-781 | 进程管理 + 权限审批 + DB 持久化                                     |
| `src/process/agent/codex/index.ts`                                           | 54-450 | binary 启动 + stdin/stdout 协议                                     |
| `src/process/agent/codex/binaryResolver.ts`                                  | —      | binary 路径解析逻辑                                                 |
| `deeporganiser-core deeporganiser.db`                                                          | —      | conversations + messages 由 backend 独占持久化                      |
