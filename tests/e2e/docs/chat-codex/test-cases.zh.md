# Codex (codex) E2E 测试用例

**版本**: Gate 2 初稿
**作者**: chat-codex-designer
**日期**: 2026-04-22
**状态**: 待审核
...
1. **codex binary 可用**：通过 `ipcBridge.fs.findCodexBinary.invoke()` 验证，否则 skip 全部测试
2. **用户配置的模型列表至少 1 个可用 provider**：
   ...
   - 若无可用 provider：skip 全部 codex 测试，原因："No non-Google-Auth provider with apiKey configured"

3. **测试数据准备**：临时工作目录 `/tmp/e2e-codex-<timestamp>/`

**模型获取 helper**（engineer 实现）:

```typescript
// tests/e2e/helpers/chatCodex.ts
...
| 维度       | 可选值                     | 说明                                                                                               |
| ---------- | -------------------------- | -------------------------------------------------------------------------------------------------- |
| 关联文件夹 | 无 / 单 / 多               | `atPath` 数组（源码：`CodexSendBox.tsx:331-337`）                                                 |
...
| 对话中操作 | 切换模型 / 切换权限 / 无   | 对话页 `CodexModelSelector` / `AgentModeSelector`                                                 |

### 1.4 清理约定

**命名模式**: 所有测试对话命名为 `E2E-codex-<timestamp>-<scenario>`

**清理顺序**（每个用例 `afterEach` 执行）:

1. 停止 binary 进程：`ipcBridge.conversation.stopAgent.invoke(conversationId)`
2. 删除 DB 记录：`DELETE FROM conversations WHERE name LIKE 'E2E-codex-%'`（级联删除 messages）
3. 删除临时目录：`fs.rm('/tmp/e2e-codex-*', { recursive: true })`
4. 清理 sessionStorage：`sessionStorage.removeItem('codex_initial_message_*')` + `sessionStorage.removeItem('codex_initial_processed_*')`
...
- 同 §1.2 全局前置条件
- 调用 `getCodexTestModels(page)` 获取 `modelA`（默认模型）
...
2. 选择 codex agent（点击 `[data-agent-backend="codex"]`）
3. 确认权限选择器显示 `default`（`AgentModeSelector` 默认值）
4. 输入测试消息："Hello, codex! Please list files in current directory."
5. 点击发送按钮
6. 等待跳转至对话页（URL 匹配 `/conversation/codex/*`）
...
FROM conversations
WHERE name LIKE 'E2E-codex-%' AND id = ?;
-- 期望: type='codex', mode='default', status='finished'

-- 2. 验证用户消息
SELECT id, type, position, content, status
FROM messages
WHERE conversation_id = ? AND position = 'right';
-- 期望: type='text', position='right', json_extract(content, '$.content') 包含 "Hello, codex!"
...
- conversations name: `E2E-codex-<timestamp>-minimal-path`
- temp dir: `/tmp/e2e-codex-<timestamp>-tc-a-01/`（workspace）
- sessionStorage: `codex_initial_message_${conversationId}`, `codex_initial_processed_${conversationId}`
...
- 同 §1.2 全局前置条件
- 临时工作目录存在测试文件夹：`/tmp/e2e-codex-<timestamp>/test-folder/`
...
1. 创建临时目录 + 测试文件夹：
   ```bash
   mkdir -p /tmp/e2e-codex-<timestamp>/test-folder/
   echo "sample content" > /tmp/e2e-codex-<timestamp>/test-folder/sample.txt
   ```
2. 打开 guid 页，选择 codex agent
3. 从文件树选择 `test-folder/`（触发 `emitter.emit('codex.selected.file', [{ path, name, isFile: false }])`）
...
- conversations name: `E2E-codex-<timestamp>-folder-single`
- temp dir: `/tmp/e2e-codex-<timestamp>/`（包含 test-folder）
...
- 同 §1.2 全局前置条件
- 测试文件存在：`/tmp/e2e-test-file.txt`（内容："Test file content for codex E2E"）
...
1. 创建测试文件：
   ```bash
   echo "Test file content for codex E2E" > /tmp/e2e-test-file.txt
   ```
2. 打开 guid 页，选择 codex agent
3. 上传文件：
...
- conversations name: `E2E-codex-<timestamp>-file-single`
- temp dir: `/tmp/e2e-codex-<timestamp>/`（workspace）
...
- 同 §1.2 全局前置条件
- 调用 `getCodexTestModels(page)` 获取 `{ modelA, modelB }`
...
- conversations name: `E2E-codex-<timestamp>-model-second`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 打开 guid 页，选择 codex agent
2. 打开权限选择器（`AgentModeSelector`，`data-testid="agent-mode-selector-codex"`）
...
- conversations name: `E2E-codex-<timestamp>-yolo-mode`
- temp dir: `/tmp/e2e-codex-<timestamp>/`（包含 binary 创建的 test.txt）
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-codex-<timestamp>-auto-edit-mode`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
2. 等待首条 AI 回复完成
3. 点击对话页模型选择器（`CodexModelSelector`，`data-testid="codex-model-selector"`）
...
- conversations name: `E2E-codex-<timestamp>-switch-model`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-codex-<timestamp>-switch-permission-1`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
- conversations name: `E2E-codex-<timestamp>-switch-permission-2`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-codex-<timestamp>/folder-a/
   echo "content A" > /tmp/e2e-codex-<timestamp>/folder-a/file-a.txt
...
- conversations name: `E2E-codex-<timestamp>-combo-folder-file-model`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
...
- conversations name: `E2E-codex-<timestamp>-multi-files`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
...
1. 创建测试数据：
   ```bash
   mkdir -p /tmp/e2e-codex-<timestamp>/folder-x/ /tmp/e2e-codex-<timestamp>/folder-y/
   echo "X content" > /tmp/e2e-codex-<timestamp>/folder-x/x.txt
   echo "Y content" > /tmp/e2e-codex-<timestamp>/folder-y/y.txt
   ```
2. 打开 guid 页，选择 codex agent
...
- conversations name: `E2E-codex-<timestamp>-multi-folders`
- temp dir: `/tmp/e2e-codex-<timestamp>/`（包含 folder-x, folder-y）
- sessionStorage: 同 TC-A-01
...
- codex binary **不可用**（通过环境变量 `AION_CLI_PATH=/dev/null` 模拟）
...
2. 运行测试套件（或单个用例）
3. 验证测试被 skip（状态：skipped，原因："codex binary not found"）
...
- 控制台输出 skip 原因："codex binary not found, skipping E2E tests"
...
export async function checkCodexBinary(): Promise<boolean> {
  try {
...
test.beforeAll(async () => {
  const hasBinary = await checkCodexBinary();
  if (!hasBinary) {
...
- conversations name: `E2E-codex-<timestamp>-nonexistent-folder`
- temp dir: `/tmp/e2e-codex-<timestamp>/`
- sessionStorage: 同 TC-A-01
...
4. **并发对话场景**（暂缓至后续 Gate）：
   - 场景：同时打开 2 个 codex 对话，轮流发送消息 → 验证 binary 进程隔离
...
1. **chat-codex-engineer** review 本用例设计，评估实现工作量
...
| `src/renderer/pages/conversation/platforms/codex/CodexSendBox.tsx`          | 331-337  | `atPath` 状态（关联文件夹数组）                   |
| `src/renderer/pages/conversation/platforms/codex/CodexSendBox.tsx`          | 103-125  | 文件上传 handler                                  |
| `src/renderer/pages/conversation/platforms/codex/CodexSendBox.tsx`          | 206-212  | 发送消息（传递 `files` 参数）                     |
| `src/renderer/pages/conversation/platforms/codex/useCodexModelSelection.ts` | 36-40    | 过滤 Google Auth 模型                             |
| `src/renderer/utils/model/agentModes.ts`                                      | 65-69    | codex 权限枚举（`default`, `auto_edit`, `yolo`） |
| `src/process/task/CodexManager.ts`                                           | 250-259  | 权限模式自动批准逻辑                              |
| `src/process/task/CodexManager.ts`                                           | 727-737  | `setMode()` 持久化                                |
| `src/process/task/CodexManager.ts`                                           | 452-489  | missing finish fallback（15s 超时）               |
...
-- 清理所有 E2E 数据
DELETE FROM conversations WHERE name LIKE 'E2E-codex-%';
```
