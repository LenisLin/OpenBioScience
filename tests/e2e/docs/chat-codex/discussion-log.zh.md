# Codex (codex) E2E 测试 - 讨论记录

**项目**: DeepOrganiser E2E Coverage
**子组**: chat-codex
**日期**: 2026-04-22
...
**参与人**: chat-codex-analyst

**完成内容**:

1. 分析 codex 源码（guid 入口、对话页、进程端、权限枚举、DB schema）
2. 起草 `requirements.zh.md` 初稿
3. 识别 8 个重大不确定性（模型切换、权限存储范围、binary 来源等）

**待办事项**:

- [x] chat-codex-designer 审核需求，补充测试用例设计建议
- [ ] chat-codex-engineer 审核需求，评估技术可行性
...
4. **清理顺序**（避免外键冲突）:
   ...
   await db.exec("DELETE FROM conversations WHERE name LIKE 'E2E-codex-%'");

   // 3. 清理 FS
   await fs.rm('/tmp/e2e-codex-*', { recursive: true });
...
| 4. CI 环境 binary 来源？            | 跳过 CI E2E（C 选项） + 本地测试套件 | `test.beforeAll()` 检查 binary，不存在则 `test.skip('codex binary not found')`        |
...
1. **guid 页 data-testid 已有**（之前误判为 0 个）
   - 现有属性: `data-agent-pill="true"`, `data-agent-backend="codex"`, `data-agent-selected`
   - 位置: `AgentPillBar.tsx:79-82`

2. **对话页需新增 15+ testid**（否则无法定位元素）
   - **P0 优先级**（阻塞测试）: 5 个
     ```tsx
     [data-testid="codex-sendbox"]
     [data-testid="codex-model-selector"]
     [data-testid="agent-mode-selector-codex"]
...
3. **DB 清理需补充 helper**
   - 文件位置: `tests/e2e/helpers/chatCodex.ts`
   - 函数: `cleanupE2ECodexConversations()`, `getCodexMessages()` 等

4. **binary 已验证可用**
   - 版本: codex v0.1.12
   - 路径: `/Users/zhoukai/.local/bin/codex`
...
| 4. CI 环境 binary 来源？            | 短期 **CI skip**，长期 DevOps 配置       | `test.skip(() => resolveCodexBinary() === null, 'codex binary not found')`              |
...
**背景**: 当前存储在 `CodexApprovalStore`（内存），进程重启后失效
...
**背景**: E2E 需依赖 codex binary

**选项**:

- A) CI 预装（需 DevOps 配置）
- B) 测试前动态下载（需提供 URL + 版本锁定）
- C) 跳过 codex E2E（标记 skip + 原因）

**engineer 建议**: 先实现 **local E2E**，CI 策略延后

- **短期**: CI 跳过 codex E2E（`test.skip(() => resolveCodexBinary() === null, 'codex binary not found')`）
- **长期**: DevOps 配置预装 或 动态下载（需 team-lead 协调）

**team-lead 决策**: **选 C CI 跳过**（短期）

- **理由**: 长期 CI 策略延后，不阻塞本轮 E2E 交付
- **实施**: `test.skip(() => resolveCodexBinary() === null, 'codex binary not found')`
...
- [ ] chat-codex-engineer review 用例设计，评估实现工作量
...
```typescript
const PROVIDER_BASED_AGENTS = new Set(['gemini', 'codex']);
const isGeminiMode =
...
**结论**: **备注不成立** — `isGeminiMode` 对 `codex` 默认为 **true**（`PROVIDER_BASED_AGENTS` 包含 codex）

**建议调整**:

- 删除 TC-A-04 备注："若 guid 页模型选择器未启用...改为 TC-A-07"
- 保留 TC-A-04 操作步骤 2："打开模型选择器（`GuidModelSelector`），选择第二个模型"
- 补充 TC-A-04 前置条件："验证 `isGeminiMode=true`（codex 默认启用）"
...
- `CodexManager.ts:766-772` 调用 `agent.denyTool(callId, reason)` 发送 `tool_deny` 命令
...
- codex binary 解析在 **main process**（`binaryResolver.ts`），读取 `process.env.AION_CLI_PATH`
...
| 审核点          | 状态          | 调整建议                                                |
| --------------- | ------------- | ------------------------------------------------------- |
| 1. TC-A-04 备注 | ⚠️ 不成立     | 删除"改为 TC-A-07"备注，codex 默认启用模型选择器       |
