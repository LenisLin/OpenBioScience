...
```typescript
return path.join(getDataPath(), 'deeporganiser.db');
```

**userData directory** (`src/process/utils/configureChromium.ts:18-26`):

- Dev mode: `~/Library/Application Support/DeepOrganiser-Dev/` (macOS)
- Database: `{userData}/config/deeporganiser.db`
- Shared by all E2E tests

**Conflict scenario**: If Assistant tests and Skills tests run in parallel workers:

1. Both access same `deeporganiser.db` file
...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-state-'));
...
```typescript
DEEPORGANISER_EXTENSION_STATES_FILE: process.env.DEEPORGANISER_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `deeporganiser.db` → parallel execution impossible without refactoring.
...
1. Extend `DEEPORGANISER_E2E_TEST` to include worker ID:
   ```typescript
   DEEPORGANISER_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.DEEPORGANISER_E2E_TEST_WORKER_ID || '0';
   return `DeepOrganiser-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/DeepOrganiser-E2E-Worker-0/config/deeporganiser.db`
   - `~/Library/Application Support/DeepOrganiser-E2E-Worker-1/config/deeporganiser.db`
...
**Complexity**: High (requires main process to read `DEEPORGANISER_DATABASE_PATH`, conflicts with userData convention)
