...
### 2. Ensure `deeporganiser-core` is on PATH

The Electron main process spawns the `deeporganiser-core` binary during startup and
exposes its port to the renderer via `window.__backendPort`. The binary is
located via `which deeporganiser-core`, so it must be reachable from the `PATH`
...
```bash
# Install the backend binary (builds to ~/.cargo/bin/deeporganiser-core)
cd ../DeepOrganiser Core && cargo install --path crates/deeporganiser-app
...
| ---------------------------- | ----- | ------------------------ |
| `DEEPORGANISER_E2E_TEST`            | `1`   | App recognizes test mode |
| `DEEPORGANISER_DISABLE_AUTO_UPDATE` | `1`   | No update checks         |
| `DEEPORGANISER_DISABLE_DEVTOOLS`    | `1`   | No DevTools windows      |
| `DEEPORGANISER_CDP_PORT`            | `0`   | CDP disabled             |
...
```bash
# Clean via database
sqlite3 "~/Library/Application Support/DeepOrganiser-Dev/deeporganiser/deeporganiser.db" \
