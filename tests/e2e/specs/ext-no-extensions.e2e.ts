...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      DEEPORGANISER_EXTENSIONS_PATH: emptyExtensionsDir,
      DEEPORGANISER_EXTENSION_STATES_FILE: extensionStatesFile,
