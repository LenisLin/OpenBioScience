...
test.describe('Guid Agent Selection', () => {
  ...
  test('select Gemini agent via pill', async ({ page }) => {
    ...
    const codexPill = page.locator(agentPillByBackend('codex'));

    // Try gemini first, fallback to codex (fork gemini)
    const targetPill = (await geminiPill.isVisible().catch(() => false)) ? geminiPill : codexPill;
    const pillVisible = await targetPill.isVisible().catch(() => false);
    if (!pillVisible) {
      test.skip(true, 'Neither gemini nor codex pill available');
      return;
    }
