 ...
 * 1. seed a legacy `deeporganiser.db` using the pre-unification assistants schema
...
function resolveBackendBinary(): string {
  ...
  const candidates = [
    process.env.DEEPORGANISER_CORE_BINARY,
    path.join(projectRoot, '../DeepOrganiserCore/target/debug/deeporganiser-core'),
    path.join(os.homedir(), '.cargo', 'bin', 'deeporganiser-core'),
  ...
  throw new Error('DeepOrganiser Core binary not found for migration e2e');
}

function schemaPath(): string {
  return path.join(process.cwd(), '../aionCore/crates/deeporganiser-db/migrations/001_initial_schema.sql');
}

function querySqliteValue(dataDir: string, sql: string): string {
  const dbPath = path.join(dataDir, 'deeporganiser-backend.db');
  return execFileSync('sqlite3', ['-readonly', dbPath, sql], { encoding: 'utf8' }).trim();
}

function seedLegacyDatabase(dataDir: string): void {
  const legacyDbPath = path.join(dataDir, 'deeporganiser.db');
...
test.describe('Assistant Settings Migration', () => {
  ...
  async function startBackend(): Promise<void> {
    ...
    backend = spawn(binary, ['--local', '--port', String(MIGRATION_BACKEND_PORT), '--data-dir', dataDir], {
      ...
      env: {
        ...process.env,
        RUST_LOG: 'warn',
        DEEPORGANISER_EXTENSIONS_PATH: path.join(process.cwd(), 'examples'),
      },
    });
    try {
  ...
  test.beforeEach(async () => {
    dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-assistant-migration-'));
  ...
  test('legacy user assistant migrates into phase-1 defaults without losing existing config', async () => {
    ...
    expect(migrated?.preset_agent_type).toBe('codex');
    ...
    expect(detail.engine.agent_backend).toBe('codex');
