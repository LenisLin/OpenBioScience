...
import { getCodexTestModels, type TProviderWithModel } from '../helpers/chatCodex';
...
async function ensureCodexTestModels(page: Page): Promise<EnsuredCodexModels> {
  const existing = await getCodexTestModels(page);
  if (existing?.modelA) {
    return {
...
function locateConversationModelButton(page: Page) {
  return page
    .locator('[data-testid="codex-model-selector"], [data-testid="chat-model-selector"], .header-model-btn')
    .first();
}
...
test.describe('Assistant Settings Conversation Defaults', () => {
  ...
  test('fixed defaults are written into conversation assistant snapshots on create', async ({ page, electronApp }) => {
    ...
    try {
      ...
      const dbPath = path.join(userDataPath, 'deeporganiser', 'deeporganiser-backend.db');
  ...
  test('switching from auto to fixed seeds preferences and new conversations use fixed defaults', async ({
    ...
    const codexModels = await ensureCodexTestModels(page);
    ...
    try {
      await httpInvoke(page, 'PUT', `/api/assistants/${assistantId}`, {
        ...
        defaults: {
          model: {
            mode: 'fixed',
            value: codexModels.modelA.useModel,
          },
          permission: {
            mode: 'fixed',
            value: 'default',
      ...
      await httpDelete(page, `/api/assistants/${assistantId}`).catch(() => {});
      if (codexModels.cleanupProviderId) {
        await httpDelete(page, `/api/providers/${codexModels.cleanupProviderId}`).catch(() => {});
      }
    }
  });
