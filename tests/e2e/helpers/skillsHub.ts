 ...
 * (lines ~317-363) and backed by `crates/deeporganiser-extension/src/skill_routes.rs`.
 */
...
export function createTempExternalSource(sourceName: string): {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-external-'));
