...
/**
 * Mapping from legacy dotted IPC keys to DeepOrganiser Core HTTP routes.
...
export const HTTP_ROUTES: Record<string, HttpRoute> = {
  ...
  'conversation.update': {
    ...
    mapBody: (p) => ({
      ...(p.updates && typeof p.updates === 'object' ? (p.updates as Record<string, unknown>) : {}),
      merge_extra: p.merge_extra,
    }),
  },
  // Workspace / file-system routes (DeepOrganiser Core, --local mode: no auth).
