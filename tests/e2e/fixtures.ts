 ...
 *      (e.g. out/linux-unpacked/deeporganiser, out/mac-arm64/DeepOrganiser.app, out/win-unpacked/DeepOrganiser.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/DeepOrganiser.exe  or  out/win-x64-unpacked/DeepOrganiser.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'DeepOrganiser.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/DeepOrganiser.app/Contents/MacOS/DeepOrganiser  or  out/mac/DeepOrganiser.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'DeepOrganiser');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/deeporganiser  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['deeporganiser', 'DeepOrganiser']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    DEEPORGANISER_EXTENSIONS_PATH: process.env.DEEPORGANISER_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
