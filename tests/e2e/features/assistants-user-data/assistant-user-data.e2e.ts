 ...
 * so seeding a legacy `deeporganiser-config.txt` before launch and observing
...
function querySqliteIds(dataDir: string, sql: string): string[] {
  const dbPath = path.join(dataDir, 'deeporganiser.db');
...
test.describe('Assistant User Data Migration (T5)', () => {
  ...
  test.describe('Migration contract (sibling backend)', () => {
    ...
    test.beforeEach(async () => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-migrate-'));
      await startBackend();
    });
