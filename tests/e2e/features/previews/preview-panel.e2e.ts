 ...
 *
 * Covers the four user-visible preview flows that run against DeepOrganiser Core
...
function makeTempFile(ext: string, body: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-preview-e2e-'));
...
function makeExternalWorkspaceFile(ext: string, body: string): { filePath: string; workspace: string } {
  const dir = fs.mkdtempSync(path.join(EXTERNAL_WORKSPACE_ROOT, 'deeporganiser-preview-e2e-'));
  const file = path.join(dir, `sample.${ext}`);
  fs.writeFileSync(file, body);
