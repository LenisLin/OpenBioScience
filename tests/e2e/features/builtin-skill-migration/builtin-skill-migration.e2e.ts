 ...
 */
const AUTO_INJECT_EXPECTED_NAMES = ['deeporganiser-skills', 'cron', 'officecli', 'skill-creator'] as const;
 ...
 */
const AUTO_INJECT_DIR_NAMES = ['deeporganiser-skills', 'cron', 'office-cli', 'skill-creator'] as const;
...
function resolveBackendBinary(): string {
  const candidates = [process.env.DEEPORGANISER_CORE_BINARY, path.join(os.homedir(), '.cargo', 'bin', 'deeporganiser-core')].filter(
    (x): x is string => typeof x === 'string' && x.length > 0
  );
  for (const c of candidates) {
    if (fs.existsSync(c)) return c;
  }
  throw new Error('DeepOrganiser Core binary not found. Set DEEPORGANISER_CORE_BINARY or install to ~/.cargo/bin/deeporganiser-core.');
}
...
test.describe('Built-in Skill Migration (T3)', () => {
  ...
  test('S7: builtin skills in /api/skills expose an absolute, readable location for export', async ({ page }) => {
    ...
    const targetDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-s7-export-'));
    try {
  ...
  //
  // Run against a sibling `deeporganiser-core` process on port 25903 against a
  ...
  test.describe('Cold-start invariants (sibling backend)', () => {
    ...
    async function startBackend(): Promise<void> {
      ...
      const logPath = path.join(dataDir, 'sibling-deeporganiser-core.log');
      const logFd = fs.openSync(logPath, 'a');
      const parentEnv = { ...process.env };
      // Scrub any env vars that would leak main-Electron backend state.
      delete parentEnv.DEEPORGANISER_EXTENSIONS_PATH;
      delete parentEnv.DEEPORGANISER_EXTENSION_STATES_FILE;
      delete parentEnv.DEEPORGANISER_E2E_TEST;
      delete parentEnv.DEEPORGANISER_CDP_PORT;
      delete parentEnv.DEEPORGANISER_BUILTIN_SKILLS_PATH;
    ...
    test.beforeEach(() => {
      dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'deeporganiser-e2e-builtin-skill-'));
    });
    ...
    test('S8: legacy {cacheDir}/builtin-skills/ is gone after the current Electron boot', async () => {
      ...
      // `~/.deeporganiser-config` tree. Failing that we at least assert the
      ...
      const candidates = [
        path.join(os.homedir(), '.deeporganiser-config', 'builtin-skills'),
        path.join(os.homedir(), '.deeporganiser-config-dev', 'builtin-skills'),
      ];
      ...
      test.info().annotations.push({
        ...
        description:
          survivors.length === 0
            ? 'no legacy builtin-skills cache dirs detected under ~/.deeporganiser-config*'
