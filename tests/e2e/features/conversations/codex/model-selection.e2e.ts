/**
 * Codex Chat E2E Tests - Model Selection (P0 + P1)
 *
 * Test Cases Covered:
 ...
 * - Model options: data-testid="codex-model-option-{modelId}"
 */
...
test.describe('Codex Chat - Model Selection (P0 + P1)', () => {
  ...
  test.skip('TC-A-04: should use second model selected on guid page', async ({ page }) => {
    // SKIP: Pending codex binary investigation - modelB switch causes silent hang on subsequent messages
    ...
    if (!preconditions.models!.modelB) {
      test.skip(true, 'Need 2nd codex-compatible model, modelB is null');
    }

    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-model-second`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-04/01-guid-page-initial.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-04/02-conversation-created.png`);

      // Step 3: Send message
      await sendCodexMessage(page, conversationId, 'Say hi in one word.');
      await takeScreenshot(page, `chat-codex/tc-a-04/03-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForCodexReply(page, conversationId);
      await takeScreenshot(page, `chat-codex/tc-a-04/04-reply-completed.png`);
  ...
  test.skip('TC-A-07: should switch model mid-conversation and update DB', async ({ page }) => {
    ...
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-switch-model`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-07/01-conversation-created.png`);

      // Step 2: Send first message
      await sendCodexMessage(page, conversationId, 'Hello, please respond.');

      // Step 3: Wait for first AI reply
      await waitForCodexReply(page, conversationId);

      // Screenshot 02: first reply completed
      await takeScreenshot(page, `chat-codex/tc-a-07/02-first-reply.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-07/03-model-selector-open.png`);

      const secondModelOption = page.locator(
        `[data-testid="codex-model-option-${preconditions.models!.modelB.useModel}"]`
      );
      ...
      await sendCodexMessage(page, conversationId, 'What model are you using now?');

      // Step 7: Wait for second AI reply
      await waitForCodexReply(page, conversationId);

      // Screenshot 04: second reply completed
      await takeScreenshot(page, `chat-codex/tc-a-07/04-second-reply.png`);
