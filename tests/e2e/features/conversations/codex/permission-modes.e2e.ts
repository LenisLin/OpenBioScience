/**
 * Codex Chat E2E Tests - Permission Modes (P0 + P1)
 *
 * Test Cases Covered:
 ...
 * - Mode options: data-testid="codex-mode-option-{mode}"
 */
...
test.describe('Codex Chat - Permission Modes (P0 + P1)', () => {
  ...
  test('TC-A-05: should use yolo permission selected on guid page', async ({ page }) => {
    ...
    const conversationName = `E2E-codex-${timestamp}-mode-yolo`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-05/01-guid-page.png`);

      // Step 2: Select codex agent
      await selectCodexAgent(page);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-05/02-mode-selector-open.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-05/03-yolo-selected.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-05/04-reply-completed.png`);
      ...
      // 2. Verify mode from conversation.extra.sessionMode (codex doesn't use ACP bridge)
      const extra =
  ...
  test('TC-A-06: should switch permission mid-conversation and persist to DB', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-switch-mode`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-06/01-conversation-created.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-06/02-first-reply.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-06/03-mode-selector-open.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-06/04-mode-switched.png`);

      // Step 6: Send second message
      await sendCodexMessage(page, conversationId, 'What mode are we using now?');

      // Step 7: Wait for second AI reply
      await waitForCodexReply(page, conversationId);

      // Screenshot 05: second reply completed
      await takeScreenshot(page, `chat-codex/tc-a-06/05-second-reply.png`);
