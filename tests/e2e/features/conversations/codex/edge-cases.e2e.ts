/**
 * Codex Chat E2E Tests - Edge Cases (P2 Priority)
 *
 * Test Cases Covered:
...
test.describe('Codex Chat - Edge Cases (P2)', () => {
  ...
  test('TC-A-13: should skip when codex binary is not reachable', async ({ page }) => {
    // This test verifies the skip logic in beforeAll when resolveCodexBinary() returns null
    ...
    await takeScreenshot(page, `chat-codex/tc-a-13/01-binary-available.png`);
  });
  ...
  test('TC-A-14: should show error when uploading file exceeding 100MB limit', async ({ page }) => {
    ...
    const conversationName = `E2E-codex-${timestamp}-large-file`;
    ...
    try {
      ...
      await page.goto(`${page.url().split('#')[0]}#/guid`);
      await takeScreenshot(page, `chat-codex/tc-a-14/01-before-upload.png`);

      // Step 2: Try to create conversation with 100MB file in workspace
      // Note: codex binary may reject large files or UI may block upload
      // We expect either bridge error or UI error message
      let errorOccurred = false;
      let conversationId = '';

      try {
        conversationId = await createCodexConversationViaBridge(page, {
        ...
        await sendCodexMessage(page, conversationId, `Read the file: ${largeFilePath}`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-14/02-after-upload-attempt.png`);
      ...
      // - If no error: file may be accessible (codex handles large files)
  ...
  test('TC-A-15: should show error when associating deleted folder path', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-deleted-folder`;
    ...
    try {
      ...
      await page.goto(`${page.url().split('#')[0]}#/guid`);
      await takeScreenshot(page, `chat-codex/tc-a-15/01-before-association.png`);
      ...
      await takeScreenshot(page, `chat-codex/tc-a-15/02-after-association-attempt.png`);
      ...
      // - If no error: codex may accept path but fail on access (deferred validation)
