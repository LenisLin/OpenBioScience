/**
 * Codex Chat E2E Tests - Basic Flow (P0 Priority)
 *
 * Test Cases Covered:
 ...
 *
 * Prerequisites:
 * - codex binary available (via ipcBridge.fs.findCodexBinary)
 * - User logged in
 * - At least 1 ACP model available (filtered Google Auth)
 *
 * Data-testid references:
 * - AgentPillBar: data-agent-backend="codex"
 * - AgentModeSelector: data-testid="agent-mode-selector-codex"
 * - CodexSendBox: data-testid="codex-sendbox"
 * - FileAttachButton: data-testid="codex-attach-folder-btn"
 */
...
test.describe('Codex Chat - Basic Flow (P0)', () => {
  // Set longer timeout for codex tests (binary calls can be slow)
  test.setTimeout(240_000); // 4 minutes — allow 150s waitForCodexReply + buffer

  let preconditions: { binary: string | null; models: CodexTestModels | null };

  // Check codex binary and provider availability before all tests
  ...
  test.afterEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      // Clear codex-specific sessionStorage keys
  ...
  test('TC-A-01: should complete minimal conversation with no attachments', async ({ page }) => {
    ...
    const conversationName = `E2E-codex-${timestamp}-minimal-path`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-01/01-guid-page-initial.png`);

      // Step 2: Create conversation via bridge (uses prioritized codex-compatible provider)
      ...
      await takeScreenshot(page, `chat-codex/tc-a-01/02-conversation-created.png`);

      // Step 3: Send simple message
      await sendCodexMessage(page, conversationId, 'Say hi in one word.');
      await takeScreenshot(page, `chat-codex/tc-a-01/03-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForCodexReply(page, conversationId);
      await takeScreenshot(page, `chat-codex/tc-a-01/04-reply-completed.png`);
      ...
      expect(conversation.type).toBe('codex');
      ...
      // Note: message.status is not set to 'finish' for codex text messages (only conv.status matters)
  ...
  test('TC-A-02: should associate single folder and reference in message', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-folder-single`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-02/01-guid-page-before-create.png`);
      ...
      await sendCodexMessage(page, conversationId, 'What files are in the attached folder?');

      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-codex/tc-a-02/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForCodexReply(page, conversationId);

      // Screenshot 03: AI reply completed
      await takeScreenshot(page, `chat-codex/tc-a-02/03-reply-completed.png`);
  ...
  test('TC-A-03: should upload single file and binary receives file parameter', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-file-single`;
    ...
    try {
      ...
      await fs.writeFile(testFilePath, 'Test file content for codex E2E');

      // Screenshot 01: before creating conversation
      await takeScreenshot(page, `chat-codex/tc-a-03/01-before-create.png`);
      ...
      // with workspace containing the test file, which codex can access
      const conversationId = await createCodexConversationViaBridge(page, {
        name: conversationName,
        workspace: tempWorkspace.path,
        provider: preconditions.models!.modelA,
      ...
      // Screenshot 02: message sent
      await takeScreenshot(page, `chat-codex/tc-a-03/02-message-sent.png`);

      // Step 4: Wait for AI reply
      await waitForCodexReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-codex/tc-a-03/03-reply-completed.png`);
      ...
      // 2. Verify AI reply mentions file content (codex can read workspace files)
      const aiMessages = messages.filter((m) => m.position === 'left' && m.type === 'text');
      expect(aiMessages.length).toBeGreaterThanOrEqual(1);
      ...
      // Note: Since codex has workspace access, it should be able to read the file
