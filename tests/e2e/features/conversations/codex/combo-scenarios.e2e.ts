/**
 * Codex Chat E2E Tests - Combo Scenarios (P1)
 *
 * Test Cases Covered:
...
test.describe('Codex Chat - Combo Scenarios (P1)', () => {
  ...
  test('TC-A-10: should handle folder + second model + yolo mode combo', async ({ page }) => {
    ...
    const conversationName = `E2E-codex-${timestamp}-combo-folder-model-yolo`;
    ...
    try {
      ...
      await takeScreenshot(page, `chat-codex/tc-a-10/01-before.png`);
      ...
      const modelOptions = page.locator('[data-testid^="codex-model-option-"]');
      ...
      await takeScreenshot(page, `chat-codex/tc-a-10/02-model-switched.png`);

      // Step 5: Send message
      await sendCodexMessage(page, conversationId, 'List files in the attached folder.');
      await waitForCodexReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-codex/tc-a-10/03-reply.png`);
  ...
  test('TC-A-11: should handle file + non-default model + default mode combo', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-combo-file-model-default`;
    ...
    try {
      ...
      // Screenshot 01: before creation
      await takeScreenshot(page, `chat-codex/tc-a-11/01-before.png`);
      ...
      if (!preconditions.models!.modelB) {
        test.skip(true, 'Need 2nd codex-compatible model for combo test');
      }
      ...
      // Screenshot 02: after model switch
      await takeScreenshot(page, `chat-codex/tc-a-11/02-model-switched.png`);

      // Step 5: Send message about file
      await sendCodexMessage(page, conversationId, 'Read the content of test-file.txt in the workspace.');
      await waitForCodexReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-codex/tc-a-11/03-reply.png`);
      ...
      // 2. Verify mode is default (from conversation.extra.sessionMode, codex doesn't use ACP bridge)
  ...
  test('TC-A-12: should handle full combo (folder + file + second model + yolo)', async ({ page }) => {
    const timestamp = Date.now();
    const conversationName = `E2E-codex-${timestamp}-full-combo`;
    ...
    try {
      ...
      // Screenshot 01: before creation
      await takeScreenshot(page, `chat-codex/tc-a-12/01-before.png`);
      ...
      // Screenshot 02: after model switch
      await takeScreenshot(page, `chat-codex/tc-a-12/02-model-switched.png`);

      // Step 5: Send message about folder and files
      await sendCodexMessage(page, conversationId, 'List all files in the attached folder and read their contents.');
      await waitForCodexReply(page, conversationId);

      // Screenshot 03: reply completed
      await takeScreenshot(page, `chat-codex/tc-a-12/03-reply.png`);
      ...
      // 2. Verify mode is yolo (from conversation.extra.sessionMode, codex doesn't use ACP bridge)
      expect(extra.sessionMode).toBe('yolo');
