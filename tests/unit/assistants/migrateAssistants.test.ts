...
describe('migrateAssistants', () => {
  ...
  describe('legacyAssistantToCreateRequest', () => {
    ...
    it('rewrites legacy default gemini to current default codex', () => {
      // Legacy Electron shipped 'gemini' as the global default.
    ...
    it('defaults to codex when presetAgentType missing', () => {
    ...
    it('preserves non-default preset_agent_type verbatim', () => {
      ...
      // gemini → codex default migration.
  ...
  describe('migrateAssistantsToBackend builtin overrides', () => {
    ...
    it('skips legacy builtin override replay when backend already exposes unified assistant detail', async () => {
      ...
      (ipcBridge.assistants.get.invoke as any).mockResolvedValue({
        ...
        engine: { agent_backend: LEGACY_LOCAL_RUNTIME_ID },
  ...
  describe('migrateAssistantsToBackend builtin preset_agent_type override', () => {
    ...
    it('preserves explicit non-default user choice across the default change', async () => {
      // Legacy built-in was set to 'claude'; backend default is codex. The
      // migration should PUT an override so the user's choice survives.
      const config = makeConfig({
        assistants: [{ id: 'builtin-word-creator', enabled: true, presetAgentType: 'claude', isBuiltin: true }],
      });

      (ipcBridge.assistants.list.invoke as any).mockResolvedValue(
        builtinListStub([{ id: 'word-creator', preset_agent_type: 'codex' }])
      );
    ...
    it('does not override when legacy value is the old default (gemini)', async () => {
      ...
      // lands on the new default codex, not a broken gemini reference.
      const config = makeConfig({
    ...
    it('does not override when legacy value already matches the current default', async () => {
      // User picked 'codex' explicitly (or the legacy default already matched):
    ...
    it('skips retired built-in ids (404 via filter, never calls PUT)', async () => {
      ...
      (ipcBridge.assistants.list.invoke as any).mockResolvedValue(
        builtinListStub([{ id: 'word-creator', preset_agent_type: LEGACY_LOCAL_RUNTIME_ID }]) // no pdf-to-ppt
      );
