...
describe('buildSendFailureError', () => {
  it('classifies 409 already-processing as APP_CONVERSATION_BUSY (wait, not retry)', () => {
  ...
  it('falls back to APP_INTERNAL_ERROR for non-conflict 409 (different message)', () => {
    ...
    expect(result.code).toBe('APP_INTERNAL_ERROR');
    expect(result.retryable).toBe(true);
  });

  it('falls back to APP_INTERNAL_ERROR for non-HTTP errors', () => {
    const result = buildSendFailureError(new Error('boom'), 'boom');

    expect(result.code).toBe('APP_INTERNAL_ERROR');
    expect(result.ownership).toBe('app');
    expect(result.retryable).toBe(true);
  });
