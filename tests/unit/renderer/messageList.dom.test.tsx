...
vi.mock('@/renderer/hooks/context/ConversationContext', () => {
  const legacyRuntime = ['ai', 'on', 'rs'].join('');
  return {
    useConversationContextSafe: () => ({ conversation_id: 'conversation-1', type: legacyRuntime }),
  };
});
...
describe('MessageList', () => {
  ...
  it('keeps agent process items inspectable and leaves the final answer as normal text', () => {
    const messages = [
      { id: 'user-1', type: 'text', position: 'right', content: { content: 'q' }, created_at: 1 },
      {
        id: 'think-1',
        type: 'thinking',
        position: 'left',
        content: { content: 'thinking', status: 'done' },
        created_at: 2,
      },
      {
        id: 'tool-1',
      ...
      { id: 'text-mid', type: 'text', position: 'left', content: { content: 'mid' }, created_at: 4 },
      {
        id: 'tool-2',
        type: 'tool_call',
        position: 'left',
      ...
      { id: 'text-final', type: 'text', position: 'left', content: { content: 'final' }, created_at: 6 },
    ] as unknown as IMessageText[];
    ...
    expect(container.querySelector('.agent_activity')).not.toBeInTheDocument();
    expect(screen.getByText('thinking')).toBeInTheDocument();
    expect(screen.getByTestId('msgtext-text-mid')).toBeInTheDocument();
    expect(screen.getByTestId('msgtext-text-final')).toBeInTheDocument();
    expect(screen.getByTestId('msgtext-text-mid').getAttribute('data-copy-row')).toBe('false');
    expect(screen.getByTestId('msgtext-text-final').getAttribute('data-copy-row')).toBe('true');
    expect(container.querySelectorAll('.tool_summary')).toHaveLength(2);
    expect(container.querySelector('.agent-steps-inline-summary')).not.toBeInTheDocument();
    expect(container.querySelector('.agent-steps-body-shell')).toBeInTheDocument();
  });
