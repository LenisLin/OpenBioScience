...
describe('Windows ARM64 installer hardening', () => {
  ...
  it('fails the ARM64 installer when required runtime files are missing after install', () => {
    ...
    expect(arm64NsisScript).toContain('DEEPORGANISER_VERIFY_REQUIRED_FILE');
    expect(arm64NsisScript).toContain('$INSTDIR\\DeepOrganiser.exe');
    expect(arm64NsisScript).toContain('$INSTDIR\\ffmpeg.dll');
    expect(arm64NsisScript).toContain('$INSTDIR\\vulkan-1.dll');
    expect(arm64NsisScript).toContain('$INSTDIR\\resources\\bundled-deeporganiser-core\\win32-arm64\\deeporganiser-core.exe');
    expect(arm64NsisScript).toContain(
      '$INSTDIR\\resources\\bundled-deeporganiser-core\\win32-arm64\\managed-resources\\node\\node-v24.11.0-win-arm64\\node.exe'
    );
  ...
  it('runs a real Windows ARM64 install smoke check instead of only checking unpacked contents', () => {
    ...
    expect(arm64SmokeStep).toContain('$env:LOCALAPPDATA\\\\Programs\\\\DeepOrganiser\\\\DeepOrganiser.exe');
    expect(arm64SmokeStep).toContain('resources\\\\bundled-deeporganiser-core\\\\win32-arm64\\\\deeporganiser-core.exe');
  });
});
