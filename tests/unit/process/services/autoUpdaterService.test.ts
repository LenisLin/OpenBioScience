...
const appMock = vi.hoisted(() => ({
  ...
  getPath: vi.fn(() => '/tmp/deeporganiser-test'),
  exit: vi.fn(),
}));
...
describe('AutoUpdaterService', () => {
  beforeEach(() => {
    ...
    delete process.env.DEEPORGANISER_FORCE_DEV_AUTO_UPDATE;
    delete process.env.DEEPORGANISER_DEBUG_AUTO_UPDATE_CURRENT_VERSION;
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'DeepOrganiser-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
  ...
  it('configures electron-updater to read stable metadata from the configured update endpoint', async () => {
    const { autoUpdaterService } = await import('@/process/services/autoUpdaterService');
  ...
  it('enables forced updater checks in unpacked dev builds when requested', async () => {
    process.env.DEEPORGANISER_FORCE_DEV_AUTO_UPDATE = '1';
  ...
  it('overrides the updater current version only for forced unpacked dev checks', async () => {
    process.env.DEEPORGANISER_FORCE_DEV_AUTO_UPDATE = '1';
    process.env.DEEPORGANISER_DEBUG_AUTO_UPDATE_CURRENT_VERSION = '2.1.12';
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'DeepOrganiser-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'DeepOrganiser-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
