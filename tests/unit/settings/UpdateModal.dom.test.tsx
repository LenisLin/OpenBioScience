...
describe('UpdateModal manual install fallback', () => {
  beforeEach(() => {
    ...
    mocks.updateCheckMock.mockResolvedValue({
      ...
      data: {
        ...
        latest: {
          ...
          name: 'v2.1.14',
          body: 'notes',
          htmlUrl: 'https://github.com/ResearAI/DeepOrganiser/releases/tag/v2.1.14',
          prerelease: false,
          draft: false,
          assets: [],
          recommendedAsset: {
    ...
    mocks.updateDownloadMock.mockImplementation(async (request: UpdateDownloadRequest) => {
      ...
      mocks.manualProgressHandler?.({
        ...
        totalBytes: 123,
        percent: 100,
        file_path: '/tmp/DeepOrganiser-2.1.14-mac-arm64.dmg',
      });
      return {
        success: true,
        data: {
  ...
  it('keeps fast manual download completion matched to the caller-provided download id', async () => {
    ...
    act(() => {
      window.dispatchEvent(new Event('deeporganiser-open-update-modal'));
    });
