...
describe('MarkdownViewer', () => {
  ...
  it('renders fenced Zig code without loading a Shiki language chunk into the page', () => {
    render(<MarkdownViewer content={'```zig\nconst value = 1;\n```'} />);

    expect(screen.getByText('const value = 1;')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Copy code' })).toBeInTheDocument();
    expect(screen.queryByText(/source\.zig|entity\.name\.type\.zig/)).not.toBeInTheDocument();
  });
});
