...
describe('selectRecentLogFiles', () => {
  it('returns every file from the N most recent non-empty days', () => {
    const files = [
      ...
      { path: '/a/2026-05-22.deeporganiser-core.log', mtime: Date.UTC(2026, 4, 22, 11), size: 200 },
...
describe('captureBackendStartupFailure', () => {
  it('captures and flushes a dedicated backend startup failure with diagnostics', async () => {
    ...
    const error = new Error('DeepOrganiser Core failed to start within timeout') as Error & {
    ...
    expect(scopeSetContext).toHaveBeenCalledWith(
      'deeporganiser_core_install_diagnostics',
  ...
  it('sets flattened incomplete-installation tags for update-related missing directory resources', async () => {
    ...
    try {
      const error = new Error('DeepOrganiser Core startup failed while resolving backend binary') as Error & {
        details?: Record<string, unknown>;
      };
      error.details = {
        ...
        resourcesPath: 'C:\\Users\\alice\\AppData\\Local\\Programs\\DeepOrganiser\\resources',
      ...
      expect(scopeSetTag).toHaveBeenCalledWith(
        'deeporganiser.backend_startup.incomplete_installation_kind',
        'missing_directory_resources'
      );
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.missing_bundled_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.missing_runtime_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.missing_binary', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.missing_hub_dir', 'true');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.last_update_status', 'quit-and-install');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.seconds_since_quit_and_install', '46');
      expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.install_path_kind', 'user_local_programs');
      expect(scopeSetContext).toHaveBeenCalledWith(
        'deeporganiser_core_startup_classification',
        expect.objectContaining({
          incompleteInstallationKind: 'missing_directory_resources',
          missingBundledDeepOrganiserCoreDir: true,
  ...
  it('sets bucketed health polling tags for backend startup timeouts', async () => {
    ...
    expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.health_polling_delayed', 'true');
    expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.health_attempts_bucket', '1');
    expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.health_attempt_deficit_bucket', '76-150');
    expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.health_timeout_overrun_bucket', 'over_60s');
    expect(scopeSetTag).toHaveBeenCalledWith('deeporganiser.backend_startup.health_max_attempt_gap_bucket', '0ms');
  });
});
...
describe('initSentry beforeSend', () => {
  ...
  it('drops backend-port secondary errors after backend startup already failed', () => {
    ...
    const event = {
      exception: {
        values: [
          {
            value: '[WebUI] Cannot start: DeepOrganiser Core is not running (globalThis.__backendPort unset)',
          },
        ],
      },
    };
  ...
  it('keeps the primary backend startup failure even when its details contain secondary text', () => {
    ...
    const event = {
      tags: {
        'deeporganiser.failure': 'backend_startup',
      },
      exception: {
        values: [
          {
