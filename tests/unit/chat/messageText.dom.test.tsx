...
vi.mock('@/renderer/pages/conversation/Messages/components/MessageOutputFiles', () => ({
  __esModule: true,
  default: () => null,
}));
...
describe('MessageText attachment paths', () => {
  ...
  it('renders context compaction markers as a centered divider', () => {
    const message: IMessageText = {
      id: 'msg-context-compacted',
      msg_id: 'msg-context-compacted',
      conversation_id: 'conv-1',
      type: 'text',
      position: 'left',
      createdAt: Date.now(),
    ...
    render(
      <ConversationProvider value={{ conversationId: 'conv-1', workspace: '/workspace/demo', type: 'codex' }}>
        <MessageText message={message} />
      </ConversationProvider>
    );

    expect(screen.getByTestId('context-compacted-divider')).toHaveTextContent('Context Compacted');
