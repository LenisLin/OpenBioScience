...
vi.mock('electron', () => ({
  ...
  app: {
    getPath: vi.fn(() => '/tmp/deeporganiser-test-logs-nonexistent'),
...
describe('feedback logs', () => {
  it('collects top-level frontend logs and nested backend logs through the IPC handler', async () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'deeporganiser-feedback-bridge-'));
    try {
  ...
  it('collects the same recent three log days used by user feedback reports', () => {
    const logsDir = mkdtempSync(path.join(tmpdir(), 'deeporganiser-feedback-logs-'));
    try {
      writeFileSync(path.join(logsDir, '2026-05-25.log'), 'today frontend\n');
      writeFileSync(path.join(logsDir, '2026-05-25.deeporganiser-core.log'), 'today backend\n');
      writeFileSync(path.join(logsDir, `2026-05-24.${LEGACY_LOCAL_RUNTIME_ID}.log`), 'yesterday rust\n');
