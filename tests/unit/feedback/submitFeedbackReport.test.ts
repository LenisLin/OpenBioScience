...
describe('submitFeedbackReport', () => {
  ...
  it('submits a user-feedback event with tags, extra context, logs, and attachments', async () => {
    ...
    await submitFeedbackReport({
      ...
      collectLogs: true,
      description: '  DeepOrganiser Core   cannot start  ',
      extra: {
      ...
      moduleLabel: 'DeepOrganiser installation is incomplete',
      tags: {
        'deeporganiser.installation_integrity.report_source': 'backend_startup_failure',
      },
    });
    ...
    expect(sentryMocks.setTag).toHaveBeenCalledWith(
      'deeporganiser.installation_integrity.report_source',
      'backend_startup_failure'
    );
    expect(sentryMocks.captureEvent).toHaveBeenCalledWith(
      {
        level: 'info',
        message: 'DeepOrganiser installation is incomplete: DeepOrganiser Core cannot start',
        extra: {
          description: 'DeepOrganiser Core cannot start',
          installation_integrity: {
            source: 'backend_startup_failure',
