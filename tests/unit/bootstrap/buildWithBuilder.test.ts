...
describe('build-with-builder', () => {
  it.each(['x64', 'arm64'])('uses exact app process checks in the Windows %s NSIS include', (arch) => {
    ...
    expect(script).toContain('${DEEPORGANISER_APP_EXECUTABLE_FILENAME}');
  ...
  ])('prepares bundled DeepOrganiser Core for $expectedPlatform-$expectedArch with args $args', ({ args, expectedPlatform, expectedArch }) => {
    const tempDir = mkdtempSync(join(tmpdir(), 'deeporganiser-build-test-'));
...
function recordPrepareCall(options) {
  const callsPath = process.env.DEEPORGANISER_PREPARE_CALLS_FILE;
  const calls = fs.existsSync(callsPath) ? JSON.parse(fs.readFileSync(callsPath, 'utf8')) : [];
  calls.push(options ?? null);
  fs.writeFileSync(callsPath, JSON.stringify(calls));
  return { prepared: true, dir: 'mock-bundled-deeporganiser-core', sourceType: 'mock' };
}

Module._load = function patchedLoad(request, parent, isMain) {
  if (request === './prepareDeepOrganiserCore' || request.endsWith('/prepareDeepOrganiserCore')) {
    return recordPrepareCall;
  }

  if (request.endsWith('packages/shared-scripts/src/prepare-deeporganiser-core.js')) {
    return { prepareDeepOrganiserCore: recordPrepareCall };
  }

  if (request === './resolveDeepOrganiserCoreVersion.js' || request.endsWith('/resolveDeepOrganiserCoreVersion.js')) {
    return { resolveDeepOrganiserCoreVersion: () => 'v-test' };
  }
...
`,
    ...
    try {
      const result = spawnSync(process.execPath, ['scripts/build-with-builder.js', ...args], {
        ...
        env: {
          ...process.env,
          DEEPORGANISER_PREPARE_CALLS_FILE: callsPath,
      ...
      const calls = JSON.parse(readFileSync(callsPath, 'utf8')) as Array<{ platform?: string; arch?: string } | null>;
      expect(calls).toContainEqual(expect.objectContaining({ platform: expectedPlatform, arch: expectedArch }));
    } finally {
