...
describe('classifyBackendStartupFailure', () => {
  it('classifies missing GLIBC symbols as an incompatible backend runtime', () => {
    ...
    error.details = {
      stage: 'early_exit',
      stderrTail:
        "/opt/DeepOrganiser/resources/bundled-deeporganiser-core/linux-x64/deeporganiser-core: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.34' not found\n" +
        "/opt/DeepOrganiser/resources/bundled-deeporganiser-core/linux-x64/deeporganiser-core: /lib/x86_64-linux-gnu/libc.so.6: version `GLIBC_2.32' not found",
    };
  ...
  it('classifies packaged app resources missing from installation as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      missingPwaDir: true,
      missingResources: ['bundled-deeporganiser-core/', 'bundled-deeporganiser-core/win32-x64/'],
  ...
  it('classifies packaged runtime directories without the backend binary as incomplete installation', () => {
    ...
    expect(classifyBackendStartupFailure(error)).toEqual({
      ...
      incompleteInstallationKind: 'missing_directory_resources',
      missingBackendBinary: true,
      missingBundledDeepOrganiserCoreDir: false,
      missingHubDir: false,
      missingPetStatesDir: false,
      missingPwaDir: false,
      missingResources: ['bundled-deeporganiser-core/win32-x64/managed-resources/', 'bundled-deeporganiser-core/win32-x64/deeporganiser-core.exe'],
      missingRuntimeDir: false,
    });
  });

  it('classifies packaged macOS architecture mismatches separately from generic startup failures', () => {
    const error = new Error('DeepOrganiser package architecture does not match this Mac') as Error & {
      details?: Record<string, unknown>;
    };
    error.details = {
