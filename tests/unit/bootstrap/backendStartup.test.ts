...
describe('startBackendOrExit', () => {
  ...
  it('captures startup failure and exits without registering a backend port by default', async () => {
    const error = new Error('DeepOrganiser Core failed to start within timeout');
    const calls: string[] = [];
    const onStarted = vi.fn();
    ...
    expect(logError).toHaveBeenCalledWith('[DeepOrganiser] Failed to start DeepOrganiser Core:', error);
  ...
  it('captures startup failure without dialog or exit when exitOnFailure is disabled', async () => {
    const error = new Error('DeepOrganiser Core exited before health check passed');
    const onStarted = vi.fn();
    const captureFailure = vi.fn();
    const exitApp = vi.fn();
    const logError = vi.fn();
  ...
  it('does not capture or exit when backend startup is cancelled by shutdown', async () => {
    const error = new Error('DeepOrganiser Core startup cancelled');
