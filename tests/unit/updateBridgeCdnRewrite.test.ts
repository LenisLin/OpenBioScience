...
const makeGitHubReleaseResponse = () => [
  {
    ...
    name: 'v1.9.22',
    body: 'release notes',
    html_url: 'https://github.com/ResearAI/DeepOrganiser/releases/tag/v1.9.22',
    published_at: '2026-04-29T00:00:00Z',
    prerelease: false,
    draft: false,
    assets: [
      {
        name: 'DeepOrganiser-1.9.22-mac-arm64.dmg',
      ...
      {
        name: 'DeepOrganiser-1.9.22-win-x64.exe',
        browser_download_url:
          'https://github.com/ResearAI/DeepOrganiser/releases/download/v1.9.22/DeepOrganiser-1.9.22-win-x64.exe',
        size: 456,
        content_type: 'application/vnd.microsoft.portable-executable',
      },
      {
        name: 'DeepOrganiser-1.9.22-linux-amd64.deb',
        browser_download_url:
          'https://github.com/ResearAI/DeepOrganiser/releases/download/v1.9.22/DeepOrganiser-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge CDN URL rewriting', () => {
  ...
  it('rewrites asset.url to the configured update path and keeps GitHub URL in fallbackUrl', async () => {
  ...
  it('uses the normalized version (no v prefix) in the configured update path', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => makeGitHubReleaseResponse(),
    });
