 ...
 * DeepOrganiser Core binary using the user-provided fixtures
 * (`/Users/zhoukai/Downloads/deeporganiser-config.txt` + `Archive/*.md`).
 *
 ...
 *
 *   1. Decode the legacy `deeporganiser-config.txt` exactly as ConfigStorage would.
 *   2. Stage `Archive/*.md` as `<userData>/config/assistants/<id>.<locale>.md`.
 *   3. Spawn a real DeepOrganiser Core bound to a throw-away data-dir.
...
const FIXTURE_CONFIG = '/Users/zhoukai/Downloads/deeporganiser-config.txt';
...
function resolveBackendBinary(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '../../../DeepOrganiserCore/target/debug/deeporganiser-core'),
  ].filter((x): x is string => typeof x === 'string' && x.length > 0);
  for (const c of candidates) {
    if (existsSync(c)) return c;
  }
  throw new Error('DeepOrganiser Core binary not found (set DEEPORGANISER_CORE_BINARY or build it)');
}
...
/**
 * Decode an `deeporganiser-config.txt` exactly the way `JsonFileBuilder` does:
...
describeIfFixtures('migrateAssistantsToBackend (real fixture)', () => {
  ...
  beforeEach(async () => {
    dataDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'deeporganiser-migrate-fixture-'));
